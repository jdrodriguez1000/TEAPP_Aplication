...
| | |
|---|---|
| **paso** | 8 de 9 — ✅ **Condición de `[D-063]` CUMPLIDA; `T-078` SIGUE ABIERTA A PROPÓSITO — la llave todavía no llegó al servidor.** Vigésimo octavo tramo (`[S-045]`), después del cierre que dejó `[S-044]` (`32f3314`). `ask_anthropic` se ejecutó por primera vez contra la red real: corrida A (llave de `teapp-measure`) → puerta 3, `requests-limit=50`; corrida B (llave de `Default`) → puerta 0, `requests-limit=1000`. `[D-063]` ampliada con la tabla, dos hipótesis muertas y la regla 7 enmendada (cuatro caracteres finales, solo en terminal, nunca en el repo). `[A-027]` nació y murió el mismo día: el "algo más" que usa la llave de `Default` es el repositorio del curso —21 archivos `.py`, 8 niveles—, ascendida a `[D-065]`: se crea `teapp-server`, llave propia, antes de correr `install.sh`. El usuario ya la creó en `Default`; su comprobación contra la red dio 10×`529` (saturación de Anthropic, confirmada con control al lado — la llave del laboratorio también falló), sigue sin verse morder. `[L-050]` nueva: el comentario de `check_api_key.py:62-63` dice que el modelo da igual y es falso — el 50 es la firma del laboratorio para `claude-opus-5` específicamente; cambiar `MODEL` en el paso 9 deja al portero mudo. `[L-049]` nueva: `T-074`, cerrada desde el 2026-08-10, viajó viva dos traspasos seguidos; corregida en `progress.md` y en la cola de `[S-044]`. Suite no se corrió hoy — último número medido: 410, sobre `32f3314`. Ningún `.ts`/`.js` en el diff — solo `_persistence/`; paso 2b no aplica (confirmado: `compilar: 0`, `comparar: 0`) |
| **última sesión** | 2026-08-13 (vigésimo octavo tramo) |
| **siguiente acción** | 🔑 **`T-078`: comprobar `teapp-server` contra la red real** en cuanto Anthropic deje de responder `529` — tiene que dar puerta 0. Con eso corre `install.sh` en la máquina y ahí sí se cierra `T-078`. Después: `T-079` reformulada — decidir qué hacer con el timeout de 10 s que hoy no puede disparar. ⚠️ Sigue pendiente `T-086` (hora de las lecturas de costo de AWS), y queda anotado para el paso 9: corregir el comentario de `MODEL` en `check_api_key.py` (`[L-050]`) |
