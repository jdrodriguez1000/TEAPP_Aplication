...
| | |
|---|---|
| **paso** | 8 de 9 — Trigésimo noveno tramo (`[S-056]`), décimo cierre del ciclo, después del cierre que dejó `[S-055]` (`46cce85`). **`T-094` CERRADA: auditoría de `[D-077]` respondida midiendo, no razonando.** Los tres hallazgos de la auditoría externa se confirmaron sobre el commit `46cce85`: `GRAMMAR_RUBRIC` creció 678 → 1.016 chars (+49,9%) por `[D-066]`/`[D-067]`, tokens de entrada medidos subieron 247 → 361 (+46,2%), y `COST_PER_CALL_USD` seguía en `0,00234` — H-1 bloqueante, corregido en `[D-078]` (`COST_PER_CALL_USD` sube a `0,00304`, marcado DERIVADO con etiqueta de tres partes). H-2 (no bloqueante): test nuevo `test_the_cap_still_lets_the_whole_run_through` cruza `MAX_CALLS_PER_RUN` con `TARGET_SAMPLES`, suite 439 → **440 passed**. H-3 (no bloqueante): la báscula reusa la conexión y producción no — anotado en `[A-030]`, no tumba el veredicto (2,6 s de holgura contra un handshake de décimas). Dos correcciones al propio encargo del auditor, comprobadas aquí: la rama B de `T-095` no puede usar `client.usages` porque `measure_tutor.py` no persiste nada en disco (sustituido por comprobar `cache_control` en el código — no aparece en ningún `.py`); y `T-086` no se salda con esta lectura, porque pide la próxima lectura de AWS, y Anthropic es otro bolsillo (`[A-024]`). **`T-095` (encargo 3): el criterio queda sellado en `[D-079]`, antes de abrir la consola** — banda `$0,156–$0,205` (barre todos los repartos entrada/salida), se lee el espacio `teapp-measure` y no el total de la organización, cuatro ramas A/B/C/D ya escritas, hora UTC antes de leer. `[L-059]` nueva: dos números contradictorios cabían dentro de la misma entrada de `decisions.md`, a cincuenta líneas — la cercanía no protege, la regla que sale es escribir los derivados como la operación con sus entradas visibles. Paso 2b confirmado: `compilar: 0`, `comparar: 0` — ningún `.ts` en el diff. `tasks.md`: `T-094` pasa a ✅ |
| **última sesión** | 2026-08-14 (trigésimo noveno tramo) |
| **siguiente acción** | `T-095`: leer la consola de Anthropic (acción del estudiante, en el navegador) y comparar el cargo de las 60 llamadas de `T-093` contra la banda **$0,156–$0,205** sellada en `[D-079]` — abrir esa entrada antes de mirar la pantalla, no después; si la consola aún no muestra datos, anotar hora UTC y dejar `T-095` abierta, no tratarlo como cero. Quedan abiertas del paso 8: `T-081` (renombrar `request_sent`, aplazada a propósito), `T-089` (mensaje inseguro de `install.sh`), `T-090` (valorar si el paso 8 cierra del todo). ⚠️ Sigue pendiente `T-086` (hora de las lecturas de costo de AWS, paso 7 — NO la salda una lectura de Anthropic) y `T-088` (comentario de `MODEL` en `check_api_key.py`, para cuando toque el paso 9) |
