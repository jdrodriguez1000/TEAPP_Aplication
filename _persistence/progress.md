...
| | |
|---|---|
| **paso** | 8 de 9 — 🔄 **`T-076` sigue A MEDIAS, pero la suite volvió a arrancar: 380 tests pasando.** Vigésimo primer tramo (`[S-038]`), después del cierre que dejó `[S-037]`. Dos arreglos en `app/tools.py`, salidos de una auditoría externa y comprobados contra la documentación del SDK: `TIMEOUT_SECONDS = 8.0` pasado al cliente (el defecto del SDK son 10 min) y la rama del veredicto vacío ya no cobra siempre — `request_sent=not refused_before_output`, exigiendo `stop_reason == "refusal"` **y** `content` vacío (`[D-054]`). Los tres archivos de tests rotos por el `FAKE_VERDICT` borrado en `[S-037]` quedaron arreglados: `tests/fake_tutor.py` nuevo (todo lo que finge ser Claude — veredicto de mentira, `FakeClient`, excepciones del SDK fabricadas) y un maniquí `autouse` en `conftest.py`. Cinco guardianes nuevos vistos en ROJO por sabotaje: quitar el timeout, cobrar siempre el veredicto vacío, la regla corta del auditor, reordenar los `except`, reordenar `TutorReply` en `english_tutor.py:53`. `[L-039]` nueva: el guion de sabotaje escribió con `write_text()` en Windows y convirtió LF a CRLF en tres archivos — contenido idéntico, bytes distintos, reparado. `app/api.py` sigue SIN TOCAR: toda la maquinaria de `request_sent` existe y está probada, pero nadie la usa — la cuota no se devuelve nunca |
| **última sesión** | 2026-08-10 (vigésimo primer tramo) |
| **siguiente acción** | 🚨 **`app/api.py` tiene que cazar `TutorUnavailableError`, mirar `request_sent` y llamar a `quota.refund`.** Es lo único que falta para cerrar `T-076`: la excepción ya se lanza con la información correcta (`[D-050]`, `[D-051]`, `[D-054]`), pero hoy nadie la atrapa y la cuota jamás se devuelve, ni siquiera cuando la petición nunca salió. Después: `T-077`, `T-078`, `T-079`. ⚠️ **Sobre el estado de la máquina de producción: no se afirma aquí sin comprobarlo** (`[L-038]`) |
