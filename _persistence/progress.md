...
| | |
|---|---|
| **paso** | 🏁 **8 de 9 sigue CERRADO (`[D-081]`), listo para cruzar al paso 9. Hoy no se cruzó: la sesión fue de respuesta a auditoría externa sobre lo ya cerrado.** Cuadragésimo cuarto tramo (`[S-061]`), primer cierre del 2026-08-17, después del cierre que dejó `[S-060]` (`13d2d79`). Tres hallazgos (H-1, H-2, H-3), verificados contra el disco antes de aplicarlos, más un cuarto que salió al corregir el tercero. **H-1 → `T-099` CERRADA:** el disparador del paso 9 deja de ser un comentario y pasa a ser un test — `tests/test_check_api_key.py` clavaba solo `LAB_REQUESTS_PER_MINUTE == 50`; ahora clava el par `(MODEL, LAB_REQUESTS_PER_MINUTE)`. Visto morder: con `MODEL="claude-sonnet-5"` la suite da `1 failed, 439 passed`; restaurado, 440 passed. Ver `[D-082]`, `[L-068]`. **H-3 → `T-100` CERRADA:** `PI-6` y `PI-7` entran en `CLAUDE.md` desde el verbatim de `GUIDE.md` §11.i (repo supervisor), no desde una paráfrasis — la paráfrasis perdía el actor ("del humano"), la voz imperativa y los porqués. Ver `[D-083]`, `[L-069]`. **H-2 (al aplicar H-3) → `T-101` CERRADA:** la tabla de citas de `CLAUDE.md` mandaba los `[LM.nn]` a `Edu_TripleS/PROGRESO.md`; viven en `LESSONS.md` (48 encabezados `### LM.n`, 0 en `PROGRESO.md`). Corregido en TRES sitios: `CLAUDE.md`, `[L-034]` y la fila `[S-033]` de este mismo archivo — los tres nacidos el 2026-08-09 en el arreglo `c96c7d7`/`7b30eb3`. Ver `[L-070]`. 📅 **Además, corregida una fecha inventada en 40 líneas de 6 archivos:** toda la sesión `[S-060]` decía 2026-08-15; sus commits son del 2026-08-14 (confirmado contra `git log` y por el usuario — no hubo sesión el 15). Ver `[L-069]`. Suite: **440 passed**, corrida cuatro veces. Código tocado: `deploy/check_api_key.py` (6 líneas, comentario) y `tests/test_check_api_key.py` (23 inserciones/8 borrados, un test reescrito). Paso 2b no aplica: ningún `.ts` en el diff |
| **última sesión** | 2026-08-17 (cuadragésimo cuarto tramo, `[S-061]`) |
| **siguiente acción** | 🏁 **El paso 8 sigue cerrado; cruzar al paso 9 ("Observabilidad y evals con rúbrica", `_context/roadmap.md:23`) sigue siendo la acción pendiente.** 🔻 **DISPARADOR — antes de cambiar `MODEL`, CADA VEZ:** leer en la consola el límite por minuto de ese modelo en `teapp-measure` y ponerlo en `LAB_REQUESTS_PER_MINUTE` **en el mismo cambio** — y desde hoy (`T-099`) esto ya no depende solo de leer un comentario: un `MODEL` sin su límite pone la suite ROJA. `[D-049]` mete el descenso a **Sonnet 5 y Haiku 4.5** dentro del paso 9, así que el disparador salta al menos dos veces. ⏳ **Sigue pendiente y sin bloquear: leer el SALDO PREPAGADO actual** en la consola de Anthropic (un clic, $0), con la hora UTC anotada ANTES del número (`[D-079]`) — el 6,55 US$ de `[D-057]` es del 2026-08-11. 🚨 Se lee ANTES del próximo bucle de llamadas. Quedan abiertas del paso 8: `T-081` (renombrar `request_sent`, aplazada a propósito), `T-096` (prioridad baja, ~5 llamadas sin dueño en `teapp-measure`). ⚠️ Sigue pendiente `T-086` (hora de las lecturas de costo de AWS, paso 7) |
