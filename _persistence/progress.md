...
| | |
|---|---|
| **paso** | 8 de 9 — ✅ **`T-084` CERRADA; `T-078` sigue bloqueada, ahora por una decisión nueva (`T-085`); `T-079` A MEDIAS y cambió de forma.** Vigésimo sexto tramo (`[S-043]`), después del cierre que dejó `[S-042]` (`89d00fd`). `T-084` cerrada con `[D-061]`: espacio de trabajo `teapp-measure` creado, freno de velocidad para `claude-opus-5` en `50/20.000/5.000` (peticiones, tokens de entrada, tokens de salida por minuto) — 5%/4%/6% de lo que la consola de esta cuenta reporta que hereda de `Default` (la documentación general decía otra cosa; ganó la consola). Verificado en vivo: llamada mínima con la llave nueva devolvió `requests-limit: 50` en las cabeceras, costó 10 tokens de entrada + 4 de salida. `[D-061]` deja abierto si además hace falta un tope de GASTO para el espacio — es la `T-085` nueva. `[L-046]`: nueve `529 Overloaded` seguidos salieron al verificar la llave — no fallo del proyecto, saturación de Anthropic; y con `MAX_RETRIES = 0` cada intento así se cobra de la cuota de 20 sin dar veredicto. `[L-045]`: el plan de `T-079` (23 peticiones a la vez contra un pool de 20) es un fósil — hoy `TUTOR_POOL_SIZE = 40`, así que 23 entrarían sin cola y no medirían nada; y el timeout de 10 s no puede disparar ni por cola ni por modelo lento con el cliente cortando a los 8,0 s. Ningún `.ts` en el diff — solo `_persistence/`, paso 2b no aplica |
| **última sesión** | 2026-08-12 (vigésimo sexto tramo) |
| **siguiente acción** | 💰 **`T-085`: decidir si `teapp-measure` lleva tope de GASTO, medido contra el saldo real de $6,55** (no contra el tope mensual de $500 de la organización). Va antes de `T-078`, porque `T-078` es lo que le da al saldo un segundo consumidor sin freno propio. Después: `T-078` (llave de servir al servidor) y `T-079` reformulada — ya no cronometrar, sino decidir qué hacer con un timeout de 10 s que hoy no puede disparar. ⚠️ **Sobre la máquina de producción de AWS: la lectura de hoy (`Costo Acumulado Mensual` = 1,12 US$) quedó sin hora anotada** — ver `T-086` |
