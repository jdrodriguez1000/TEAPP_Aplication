...
| | |
|---|---|
| **paso** | 8 de 9 — ✅ **`T-076`, `T-077` y `T-080` CERRADAS; `T-079` A MEDIAS.** Vigésimo cuarto tramo (`[S-041]`), después del cierre que dejó `[S-040]` (`1365ed1`). Dos rondas de auditoría externa: `[C-008]` nueva (medir y servir salen del mismo saldo; hoy inofensivo porque el servidor tiene la llave vacía, `T-078` quita esa protección — se decide con `T-082` antes de `T-078`) y `[L-042]` nueva (el 504 decide la devolución con `future.cancel()`, que no responde "¿se facturó?"; ventana de 2 s entre el timeout del cliente, 8,0 s, y el de la ruta, 10 s — señalado, sin arreglar a propósito). `measure_tutor.py` nuevo: diez llamadas reales a `judge_grammar` con `claude-opus-5`, primer gasto real del proyecto — 1,72 s / 3,33 s mediana / 4,72 s la peor de diez; 247,2 tokens de entrada y 44,3 de salida por práctica. `[A-010]` CERRADA, ascendida a `[D-058]`: $0,02 en consola vs $0,0234 por precio de lista, coinciden — $0,00234/práctica, $8,44 en 180 días por persona a tope; **el saldo de $6,55 no cubre a una persona a tope, se acaba a los 140 días**; el tope de 20 se queda igual, frena el abuso, no describe el uso. `[A-011]` se RETIRÓ y se REABRIÓ el mismo día: el guion cronometró solo `judge_grammar`, no la cola del pool + `respond()` entero que sí cronometra `TUTOR_TIMEOUT_SECONDS`; queda encogida. Reencuadre que se queda: el timeout del cliente (8,0 s) mide un subconjunto del de la ruta (10 s) y es más chico, así que sin cola el de la ruta nunca puede disparar — respalda no tocar el 8,0. Suite de 381 a **387 pasando**, confirmado en este cierre. Paso 2b: `.js` compilado, al día (`compilar: 0`, `comparar: 0`) — no había ningún `.ts` en el diff |
| **última sesión** | 2026-08-11 (vigésimo cuarto tramo) |
| **siguiente acción** | ⏳ **Leer el saldo actual en la consola de Anthropic — pendiente que CADUCA.** Hoy es el único día en que `$6,55 − saldo` corresponde exactamente a los diez consumos de `T-079` y a nada más; en cuanto `T-078` ponga la llave en el servidor y alguien practique, deja de ser irrepetible. Se pidió al usuario en esta sesión y no llegó a contestarse. Después: `T-082` — decidir cómo se separan medir y servir (`[C-008]`), **antes** de `T-078`. Luego `T-078` y terminar `T-079` cronometrando `/practice` entera con el pool cargado (`[A-011]`, aparejo de `[L-013]`). ⚠️ **Sobre la máquina de producción de AWS: no se afirma nada, no se tocó** — la única medida externa (timeout, sin RST) la hizo la terminal auditora, no este proyecto |
