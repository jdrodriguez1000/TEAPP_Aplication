...
| | |
|---|---|
| **paso** | 8 de 9 — ✅ **`T-076`, `T-077`, `T-080`, `T-082` y `T-083` CERRADAS; `T-079` A MEDIAS.** Vigésimo quinto tramo (`[S-042]`), segundo tramo del 2026-08-11 después del cierre que dejó `[S-041]` (`5815132`). `T-082` cerrada con `[D-059]`: medir y servir se parten en DOS capas — corte duro en `measure_tutor.py` (capa 1) + espacio de trabajo propio para medir, con llave y límite de velocidad (capa 2). Descartado fiarlo al tope de gasto por espacio de trabajo (documentación de Anthropic: es reparto del mismo techo de la organización, no un bolsillo aparte) y descartado Claude Platform on AWS (factura a mes vencido, sin saldo, contra la regla 5). `T-083` nueva y cerrada el mismo día con `[D-060]`: el tope de la báscula ahora sale del saldo ($0,25/tanda ÷ $0,00234/llamada = 106 llamadas), con `CallBudget` que cobra ANTES de llamar dentro de `RecordingClient`. Hallazgo de camino (`[L-044]`): el `MAX_CALLS = 10` que ya existía salía de `len(SENTENCES)`, no de una medición, y lo hacía cumplir un recorte de lista que con el tope real no podía cortar nunca. 8 tests nuevos en `tests/test_measure_tutor.py`, tres sabotajes vistos en ROJO (cobrar después de llamar, contador por cliente, `>` en vez de `>=`). Suite de 387 a **395 pasando**, confirmado en este cierre. `[C-008]` cerrada A MEDIAS: el fallo mudo queda tapado por el corte duro, pero la partición del saldo sigue sin existir — medir y servir además comparten la llave hasta que la capa 2 se construya. Paso 2b: `.js` compilado, al día (`compilar: 0`, `comparar: 0`) — no había ningún `.ts` en el diff |
| **última sesión** | 2026-08-11 (vigésimo quinto tramo) |
| **siguiente acción** | 🆕 **Acción del usuario en el navegador: `T-084`, crear el espacio de trabajo para MEDIR** — con su llave propia y su límite de VELOCIDAD (capa 2 de `[D-059]`). 🚨 **Bloquea `T-078`:** `[D-059]` asignó la llave nueva a medir y la de hoy (`T-075`) a servir, pero la llave nueva nace en `T-084`; si `T-078` corre antes, la única llave que hay viaja al servidor y medir y servir pasan a compartir también la llave, perdiendo la revocación por separado y la contabilidad por `workspace_id`. Después de `T-084`: `T-078` (poner la llave de servir en el servidor) y terminar `T-079` cronometrando `/practice` entera con el pool cargado (`[A-011]`, aparejo de `[L-013]`). ⚠️ **Sobre la máquina de producción de AWS: no se afirma nada, no se tocó** en este tramo |
