...
| | |
|---|---|
| **paso** | 8 de 9 — Trigésimo séptimo tramo (`[S-054]`), octavo cierre del 2026-08-13, después del cierre que dejó `[S-053]` (`a0ccb1f`). **Ronda corta de preparación, sin auditoría y sin gastar un centavo: el criterio de `T-093` queda fijado en el CÓDIGO antes de correr la tanda.** `[D-074]` nueva: 60 frases (no 40), tasa de corte aceptada del 5%, y tres veredictos escritos a ciegas. El 60 sale de la regla de tres — con cero cortes observados, `3/n` da 7,5% con n=40 (afirma menos de lo exigido, no concluye) y 5,0% con n=60 (coincide con el criterio); coste estimado ~$0,14 en vez de ~$0,09. `measure_tutor.py` gana `TARGET_SAMPLES = 60`, `ACCEPTED_CUT_RATE = 0.05`, `CUT_THRESHOLD_SECONDS` (leído de `tools.TIMEOUT.read`, no copiado) y `ROUTE_THRESHOLD_SECONDS = 9.5`; `verdict_for()` nueva imprime uno de tres veredictos (VERDE: 0 de 60 por encima del corte → `[A-011]` se cierra; ÁMBAR: alguna corta pero ninguna pasa de 9,5 s → reequilibrar fases; ROJO: alguna pasa de 9,5 s → el presupuesto de la ruta está mal), y avisa si la tanda no completa las 60 —el veredicto no se puede escribir en `[A-011]` sin la N entera—. `SENTENCES` pasa de 10 a 60 frases DISTINTAS (repetir mediría la caché de Anthropic, no el tiempo real). Suite: **427 passed**, sin tests nuevos en este diff — las tres ramas de `verdict_for` se probaron a mano con datos falsos, sin llamar a Anthropic. Paso 2b no aplica: ningún `.ts`/`.js` en el diff (solo `measure_tutor.py` y `_persistence/decisions.md`). `tasks.md`: `T-093` sigue 🔲, ahora anotada como PREPARADA — falta correr la tanda, primer gasto real de una medición planificada del proyecto |
| **última sesión** | 2026-08-13 (trigésimo séptimo tramo) |
| **siguiente acción** | `T-093`: correr `measure_tutor.py` de verdad contra `claude-opus-5` — 60 frases, ~$0,14, con el criterio ya escrito en `verdict_for()`. Si sale VERDE, `[A-011]` se cierra sin techos inventados; si ÁMBAR, reequilibrar fases dentro de los 10 s; si ROJO, el presupuesto de la ruta es el que está mal. Quedan abiertas del paso 8: `T-081` (renombrar `request_sent`, aplazada a propósito), `T-089` (mensaje inseguro de `install.sh`), `T-090` (valorar si el paso 8 cierra del todo). ⚠️ Sigue pendiente `T-086` (hora de las lecturas de costo de AWS, paso 7) y `T-088` (comentario de `MODEL` en `check_api_key.py`, para cuando toque el paso 9) |
