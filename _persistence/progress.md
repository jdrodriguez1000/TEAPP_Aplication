...
| | |
|---|---|
| **paso** | 8 de 9 — 🎉 **`T-019` CERRADA — el marcador cuenta aciertos, no prácticas.** Trigésimo tramo (`[S-047]`), después del cierre que dejó `[S-046]` (`811d436`). `judge_grammar` cambió de contrato: devuelve `GrammarVerdict(correct, message)` en vez de `str`, con `split_verdict` leyendo una primera línea fija (`OK`/`FIX`) que se recorta y deniega el punto por defecto si el formato falla (`[D-066]`, `[D-067]`). `read_score`/`add_point` sustituidas por `read_counters`/`record_practice` (`Counters(score, practice)`), escritura atómica de los dos números juntos. `api.py`, `frontend/app.ts`, `index.html` y `app.js` (recompilado) muestran las cuatro piezas. Marcadores viejos borrados en local — formato incompatible (`[D-068]`). `deploy/README.md` gana la sección "Actualizar una máquina que YA está montada". **Verificado contra el modelo real, desde el navegador:** frase correcta → Score 1 / Practice 1; frase mala → Score 1 / Practice 2, sin subir (`[D-069]`). Suite: 425 tests pasando (eran 410); `npx tsc --noEmit` sin errores. `[A-001]` y `[A-028]` retiradas de `assumptions.md`, ascendidas a `[D-066]` y `[D-069]`. `constraints.md` y `lessons.md` sin cambios hoy, revisados contra el diff — al día. Paso 2b confirmado: `compilar: 0`, `comparar: 0`. **`T-091` nueva:** falta llevar este cambio al servidor — `git pull`, borrar `data/users/*.json` viejos, reiniciar `teapp` |
| **última sesión** | 2026-08-13 (trigésimo tramo) |
| **siguiente acción** | 📌 **`T-091`: desplegar el trabajo de hoy en el servidor** — `git pull` en `/opt/teapp`, borrar `data/users/*.json` (formato viejo incompatible con `practice` exigido, `[D-068]`), `systemctl restart teapp`, y confirmar una práctica real desde el navegador. Quedan además abiertas del paso 8: `T-079` (a medias, decidir qué hacer con el timeout de 10 s), `T-081` (renombrar `request_sent`, aplazada a propósito), `T-089` (mensaje inseguro de `install.sh`), `T-090` (valorar si el paso 8 cierra del todo). ⚠️ Sigue pendiente `T-086` (hora de las lecturas de costo de AWS, paso 7) y `T-088` (comentario de `MODEL` en `check_api_key.py`, para cuando toque el paso 9) |
