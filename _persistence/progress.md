...
| | |
|---|---|
| **paso** | 7 de 9 — 🚀 **T-073 CERRADA: la máquina se apaga sola con una pieza de systemd, no con un disparo único.** **Decimoquinto tramo (`[S-032]`):** `deploy/teapp-shutdown.service`/`.timer` (nuevos), instalados por `install.sh` sección 4b — `OnCalendar=23:00:00 UTC`, `Persistent=false` explícito, sin `[Install]` para que `install.sh` no la arranque a mitad de instalación. `[D-046]` nueva (systemd sobre `cron`, por la zona horaria) y luego actualizada con la medición real en la máquina (17:37–17:50 UTC): `git pull` `0dfdbba`→`afe2eab`, `install.sh` código 0, temporizador armado (`Sun 2026-08-09 23:00:00 UTC`, `LAST`/`PASSED` vacíos), disparo único desarmado con medición antes/después, `list-timers` releído después de cancelar. `[L-033]` nueva: el rodeo del DNS perdió la palabra `SSH` al recontarse — por la IP se entra por SSH, por navegador y `curl` por el nombre. `deploy/console_steps.md` gana el Paso 5b: el guion de encender la máquina cada mañana, con cinco controles. De 351 a **360** tests (`tests/test_deploy_shutdown.py`, cuatro guardianes vistos ROJO). 🚨 **`T-074` PENDIENTE Y VENCE HOY A LAS 23:00 UTC (18:00 Colombia):** es lo único que quedó sin medir de la pieza — que dispare de verdad y la instancia pase a `stopped` en la consola de AWS, con la hora exacta anotada. Mañana la máquina hay que **encenderla a mano** desde la consola antes de trabajar |
| **última sesión** | 2026-08-09 (decimoquinto tramo) |
| **siguiente acción** | 🚨 **T-074 — mirar el temporizador disparar de verdad, hoy 2026-08-09 a las 23:00 UTC (18:00 Colombia):** con alguien viendo la consola de AWS, confirmar que la instancia pasa a `stopped` sola y anotar la hora exacta. Es comportamiento, no configuración — lo único que la pieza de `T-073` no pudo medirse a sí misma. Al día siguiente, encender la máquina a mano desde la consola antes de trabajar, y correr el Paso 5b de `deploy/console_steps.md` (cinco controles, el quinto —que el temporizador se haya rearmado solo— es el que no tiene síntoma si falla). Después: `T-066` (dos dispositivos), `T-067` (gasto real bajo la ventana de `[D-045]`, tres tarifas). **Entrar por SSH usando la IP fija `32.199.55.191`**, no el nombre — el DNS falla intermitente y la causa está en el cliente (`[L-033]`) |
