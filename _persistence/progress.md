...
| | |
|---|---|
| **paso** | 8 de 9 — Cuadragésimo primer tramo (`[S-058]`), tercer cierre del 2026-08-14, después del cierre que dejó `[S-057]` (`4a9baad`). **`T-095` CERRADA: el estudiante leyó la barra del día 14 en "Costo diario de tokens" → `$0,18`.** Cae dentro de la banda sellada `$0,156–$0,205` (RAMA A de `[D-079]`, la pre-comprometida). `COST_PER_CALL_USD` pasa de DERIVADO a **MEDIDO** en el comentario de `measure_tutor.py` (`$0,18 / 60 = $0,00300`, intervalo `[$0,00292, $0,00308]` por el redondeo al céntimo de la consola); **el valor de la constante no cambia, se queda en `0,00304`** — cae dentro del intervalo y sigue siendo la calibración de un freno, así que se elige el lado alto. `[D-058]` queda confirmada en su mecánica: el modelo de tarifas acertó dos veces sobre números leídos (semana `$0,2004`/`$0,20`, día `$0,1828`/`$0,18`). `MAX_CALLS_PER_RUN` sigue en 82. 🚨 **Hallazgo propio del tramo — `[L-060]` nueva:** la lectura NO discriminó entre las dos predicciones selladas (`$0,180–$0,190` derivación completa y `$0,177–$0,187` por resta); las dos se cumplen porque la consola redondea al céntimo y `$0,18` abarca `[$0,175, $0,185]`, pisando las dos franjas — la zona discriminante medía tres milésimas y el instrumento no resuelve menos de diez. Regla: sellar la predicción es la mitad del método, la otra mitad es comprobar que el instrumento tiene resolución para decidirla. Archivos tocados: `measure_tutor.py` (solo el comentario), `_persistence/decisions.md` (DESENLACE dentro de `[D-079]`), `_persistence/lessons.md` (`[L-060]`), `_persistence/tasks.md` (`T-095` → ✅). Suite: 440 passed, corrida por esta terminal tras los cambios. Paso 2b confirmado en este cierre: `compilar: 0`, `comparar: 0` — ningún `.ts` en el diff |
| **última sesión** | 2026-08-14 (cuadragésimo primer tramo, tercer cierre del día) |
| **siguiente acción** | `T-096` (prioridad baja: localizar ~5 llamadas sin dueño en `teapp-measure`, $0,018 de excedente semanal). Quedan abiertas del paso 8: `T-081` (renombrar `request_sent`, aplazada a propósito), `T-089` (mensaje inseguro de `install.sh`), `T-090` (valorar si el paso 8 cierra del todo). ⚠️ Sigue pendiente `T-086` (hora de las lecturas de costo de AWS, paso 7 — NO la salda una lectura de Anthropic) y `T-088` (comentario de `MODEL` en `check_api_key.py`, para cuando toque el paso 9) |
