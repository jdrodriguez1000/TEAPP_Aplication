...
| | |
|---|---|
| **paso** | 8 de 9 — 🎉 **`T-091` CERRADA — el marcador de aciertos (T-019) ya está en el servidor.** Trigésimo primer tramo (`[S-048]`), segundo tramo del 2026-08-13, después del cierre que dejó `[S-047]` (`76e9bee`). En `/opt/teapp`: `git pull` (`699f2b2`→`76e9bee`), `rm -f data/users/*.json` (obligatorio por `[D-068]`), `systemctl restart teapp`. **Verificado contra la máquina viva:** `https://teapp.duckdns.org/` → `200`; página servida con `<span id="practice">`; `app.js` servido con `practiceBox`; práctica real desde el navegador con frase incorrecta → `Words: 4 · Score: 1`, el `Score` no subió. **`[L-051]` nueva (de la sesión principal):** el navegador mostró la pantalla vieja cacheada mientras los números ya llegaban frescos — el despliegue estaba bien, el molde era el viejo; diagnosticado con `curl` a la línea de contadores, confirmado en incógnito. Único archivo del diff: `_persistence/lessons.md`. Suite: 425 tests pasando (sin cambio, no se tocó código). Paso 2b confirmado: `compilar: 0`, `comparar: 0` |
| **última sesión** | 2026-08-13 (trigésimo primer tramo) |
