...
| | |
|---|---|
| **paso** | 8 de 9 — ✅ **`T-076`, `T-077` y `T-080` CERRADAS.** Vigésimo tercer tramo (`[S-040]`), después del cierre que dejó `[S-039]`. `T-080` (acción del usuario): la consola de Anthropic mostró saldo prepagado de 6,55 US$, recarga automática DESACTIVADA, límite mensual de 500 US$ puesto por Anthropic — `[A-024]` era **falsa**, retirada de `assumptions.md`, vive en `[D-057]` (rectificada el mismo día por el usuario: el límite se queda en 500, no se baja a 10, porque el saldo muerde muchísimo antes; con disparador escrito para cuando se recargue saldo). `T-076`: `app/api.py` ya caza `TutorUnavailableError` — 503, `TUTOR_UNAVAILABLE_MESSAGE`, `quota.refund(user)` solo si `not error.request_sent`. Seis tests nuevos en `tests/test_api.py`, dos vistos en ROJO por sabotaje (`if False`/`if True` en el `refund`). `T-077`: `FAKE_VERDICT` ya no existía (se había borrado en `[S-037]`); quedaban comentarios desactualizados en seis archivos diciendo que el tutor seguía siendo falso, corregidos; el docstring de `app/english_tutor.py` se REESCRIBIÓ con precisión (la secuencia fija es encargo de `scope.md`, no deuda). **Corrida en vivo (PI-4), coste cero:** app local con llave de API inválida, puerto 8099 — `/practice` → 503 con el mensaje correcto, cuota gastada y devuelta (`used: 0`), marcador **ni se creó**. `[L-041]` nueva: el log de esa corrida trae un `request_id` de Anthropic junto a `salio: no` — la petición sí salió y aun así la cuota se devuelve correctamente, porque el campo decide "¿se facturó?", no "¿salió el paquete?"; el nombre `request_sent` se quedó atrás y no se renombró hoy a propósito (PI-3) — queda como `T-081`. Suite de 381 a **387 pasando**, confirmado corriendo `python -m pytest -q` en este cierre |
| **última sesión** | 2026-08-11 (vigésimo tercer tramo) |
| **siguiente acción** | `T-078` — que `ANTHROPIC_API_KEY` llegue al servidor por `install.sh`, con permisos cerrados y sin escribirse nunca en el repo. Después, `T-079` — medir `[A-010]` y `[A-011]` con el modelo real: ya NO está bloqueada (`T-080` la desbloqueó), el freno vivo es el saldo prepagado de 6,55 US$ (`[D-057]`), y es posible que la medición se quede corta por falta de saldo — eso es el freno funcionando, no un fallo. ⚠️ **Sobre el estado de la máquina de producción de AWS: no se afirma aquí sin comprobarlo** (`[L-038]`) — no se tocó en esta sesión |
