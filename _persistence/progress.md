...
| | |
|---|---|
| **paso** | 7 de 9 — la cuenta de AWS sigue abierta desde `T-057` (2026-08-06). El experimento de `[A-018]` (alarma de facturación) sigue madurando: la Elastic IP reservada en `us-east-1` sigue sin instancia y sin asociar, cobrando por existir. ⚠️ Nada de `deploy/` se ha corrido nunca — sigue sin haber máquina EC2. **Séptimo tramo (`[S-024]`):** `T-072` cerrada — la báscula de `T-054` (`measure_body.py`) era el camino que escribía en `data/` real fuera de pytest: desvió `accounts.ACCOUNTS_FILE` y se olvidó de `USERS_DIR` y `QUOTA_DIR` (`[L-023]`). Arreglo estructural, no del script: `TEAPP_DATA_DIR` (`[D-037]`) — ruta absoluta obligatoria, sin valor por defecto, carpeta que debe existir, log en INFO al arrancar. De 329 a **342** tests verdes |
| **última sesión** | 2026-08-06 (séptimo tramo) |
| **siguiente acción** | Esperar la lectura del experimento de `[A-018]` (factura + bandeja) antes de tocar la nube de nuevo. Cuando llegue el momento de `T-059` (segunda mitad — lanzar la EC2), releer antes la lista "ESTO NUNCA SE TOCA" de `T-068` en `deploy/console_steps.md`, minutos antes del primer clic |
