...
| | |
|---|---|
| **paso** | 🚀 **PASO 9 ARRANCADO (`[D-085]`), por observabilidad — orden decidido por el usuario: observabilidad → evals con rúbrica → descenso de modelo → seguridad.** Cuadragésimo quinto tramo (`[S-062]`), segundo cierre del 2026-08-17, después del cierre que dejó `[S-061]` (`41c753f`). Sesión de dos partes. **Parte 1 — el saldo:** saldo leído `$6,24` (16:05 UTC), descarga el disparador de `[D-081]` (`[D-084]`). Descuadre de `$0,02–0,095` **resuelto sin bicho**: la cuenta también paga el estudio de programación con agentes de IA del usuario — tercer inquilino del saldo, invisible en la vista de COSTO (`[C-009]` nueva). `T-096` cerrada sin bicho. `[L-071]` (el total cuadraba, el desglose no) y `[L-072]` (cuatro lecturas de consola para algo que estaba a una pregunta) nuevas. **Parte 2 — la traza (observabilidad):** `[D-085]` decide el orden del paso 9 y el reparto de tres filas (traza operativa en `data/`, material de evals aplazado, `_persistence/` sin frases de personas nunca — asciende a `PI-8` en `CLAUDE.md` + casilla en `protocol-close`). `[D-086]` fija los dos frenos ANTES de escribir código: el test prueba comportamiento (mover `TEAPP_DATA_DIR`), no forma; y un fallo al escribir la traza no propaga (no le cuesta la práctica a nadie) y no se calla (log de prosa). **Código nuevo:** `app/trace.py` (el cuaderno, `record()`), `config.trace_file()` (cuarto sitio colgado de `require_data_dir()`), `TutorReply.correct` (mitad de máquina del veredicto), cableado en `app/api.py` (reloj antes del `submit`, `try/except` alrededor de `trace.record`). **Tests nuevos:** `tests/test_trace.py` (NUEVO, 6 tests), más en `test_api.py`, `test_config.py`, `test_english_tutor.py`, `test_tools.py`. `[L-073]` nueva, en tres mitades: el campo `correct` nació sin guardián (saboteado con `correct=True` dio **447 passed** en verde); el arreglo estructural es clavar el CONJUNTO de campos (`{campo.name for campo in fields(...)}`), no el campo suelto; aplicado a las tres clases que viajan en bloque (`TutorReply`, `GrammarVerdict`, `Counters`). Suite: **452 passed**, corrida en este cierre. Meta-controles 11/11. Paso 2b no aplica: ningún `.ts` en el diff. 🔻 **Pendiente vivo, con disparador:** la traza no se ha visto escribir con el servidor levantado y una llamada real — disparador = la primera llamada real del descenso de modelo de `[D-049]` |
| **última sesión** | 2026-08-17 (cuadragésimo quinto tramo, `[S-062]`) |
| **siguiente acción** | 🚀 **Seguir dentro del paso 9, por observabilidad.** El reparto del tiempo por fase (`connect`/`write`/`pool`/`read`, `app/tools.py`) sigue sin registrarse en la traza — es el campo que `[D-085]` señala como el que de verdad decide si el descenso de modelo (`[D-049]`) ayuda. 🔻 **Pendiente vivo con disparador, no bloqueante:** ver la traza escribir con el servidor levantado y una llamada real — se dispara con la primera llamada real del descenso de modelo (`[D-049]`), mirando que `trace.jsonl` tenga su línea con el `model` nuevo dentro. Quedan abiertas del paso 8: `T-081` (renombrar `request_sent`, aplazada a propósito). ⚠️ Sigue pendiente `T-086` (hora de las lecturas de costo de AWS, paso 7) |
