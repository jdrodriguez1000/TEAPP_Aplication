...
| | |
|---|---|
| **paso** | 🏁 **PASO 9 CERRADO, y con él TEAPP TERMINADA — quincuagésimo sexto tramo (`[S-073]`), 2026-08-19, después del cierre que dejó `[S-072]` (`e8eb8bf`).** `[D-105]` (firmada por el usuario, ya venía escrita por la sesión principal, revisada contra el diff): las dos condiciones de `[D-103]` quedan CUMPLIDAS. **Condición 1 —vara discriminante escrita, etiquetada y corrida— CUMPLIDA con `T-112`:** salió fallida (0 desacuerdos de 30, tramo `0-2`), pero la condición pedía el ejercicio, no que mordiera; no se reabre. **Condición 2 —traza vista escribir en producción— CUMPLIDA el 2026-08-19 a las 18:16 UTC:** EC2 real, `https://teapp.duckdns.org/`, llamada real a `claude-opus-5`, `~$0,0034`. Antes hubo que desplegar: la máquina corría `699f2b2` (2026-08-13), anterior a `app/trace.py` (`c292210`, 2026-08-17); resuelto con `git pull` + `systemctl restart teapp` (40 commits, sin reinstalar). La línea de traza (doce campos, ninguno con la frase de la persona — `PI-8`/`[D-085]` en producción) demuestra además que el código propio tarda 2 ms de 3,47 s de práctica (0,06%): toda la espera es el modelo. **Se RENUNCIA a `[D-049]`** (descenso a Sonnet 5 y Haiku 4.5): no se hace, ni hoy ni después — TEAPP es educativa, no comercial, y `[D-105]` deja escrito qué SÍ sería obligatorio en una app comercial (cinco puntos). Marca nueva en el encabezado de `decisions.md`: `~~D-nnn~~` ⛔ **RENUNCIADA** (tercera, junto a 🔻 SUPERADA y ✅ CUMPLIDA), porque las dos existentes habrían mentido sobre `~~D-049~~`. `[A-031]` nueva (sesión principal): el marcador de `jorge` bajó de `score: 9` (`T-078`, 2026-08-13) a `score: 6` (traza de hoy) — un marcador solo sube; la mitad benigna (`score < practice` por prácticas falladas) está confirmada por el usuario, la bajada no, se supone un borrado de marcadores en algún ensayo (`deploy/README.md:98`), anotada en vez de resuelta por decisión del usuario al cerrar el paso 9. Deuda nueva registrada dentro de `[D-105]`, ahora con tarea propia (`T-113`): dos constantes de modelo (`app/tools.py:41` y `deploy/check_api_key.py:104`) y solo la segunda la clava un test — cambiar el modelo de la app es hoy un cambio MUDO; queda DORMIDA al renunciar a `[D-049]`. **Suite: 580 passed**, corrida en este cierre con `.venv/Scripts/python.exe -m pytest -q` (58,59 s) — el test que fallaba en `[S-072]` (`test_the_archived_replies_are_backed_up_by_git`) ya pasa. **Cero código tocado en el diff de hoy** (PI-3): solo `_persistence/assumptions.md` y `_persistence/decisions.md`, ya escritos por la sesión principal antes de este cierre. Paso 2b: `.js` al día, `tsc` compiló sobre `tsconfig.json` y coincide byte a byte con `app/static/app.js` — sin cambios de `.ts` en el diff de hoy |
| **última sesión** | 2026-08-19 (quincuagésimo sexto tramo, `[S-073]`) |
| **siguiente acción** | 🏁 **No hay siguiente paso de producto: el roadmap (0..9) está completo y TEAPP queda cerrada en `claude-opus-5`.** Si se retoma el proyecto, lo primero que hay que mirar es `T-113` (constante de modelo sin test propio, dormida) antes de tocar `MODEL_NAME` o `MODEL` en ningún archivo. Deuda pequeña que sigue abierta, sin decidir el cierre: `T-108` (rendija del portero de corpus/replies, `*.jsonl` vs `*.json`), `T-103` (`tutor_started[0]` vacío en el camino del timeout), `T-081` (renombrar `request_sent`). ⚠️ Sigue pendiente `T-086` (hora de las lecturas de costo de AWS, paso 7) — anterior al cierre del paso 9, no lo bloqueó |

## Índice

| id | fecha | qué avanzó | paso |
|---|---|---|---|
| S-073 | 2026-08-19 | Quincuagésimo sexto tramo, después del cierre que dejó `[S-072]` (`e8eb8bf`). 🏁 **PASO 9 CERRADO, y con él TEAPP TERMINADA.** `[D-105]` (firmada por el usuario, ya venía escrita por la sesión principal al arrancar este cierre — revisada contra el diff, al día, índice y entrada juntos): las dos condiciones de `[D-103]` quedan CUMPLIDAS y se RENUNCIA a `~~D-049~~` (el descenso a Sonnet 5 y Haiku 4.5 no se hace). **Condición 1 CUMPLIDA con `T-112`:** corrió, salió fallida (0 de 30 desacuerdos, tramo `0-2` de `[D-104]`), y la condición pedía el ejercicio —escrita, etiquetada, corrida— no que mordiera; no se reabre, el aprendizaje quedó en `[L-088]`. **Condición 2 CUMPLIDA el 2026-08-19 18:16 UTC:** la traza se vio escribir en la EC2 real, entrando por `https://teapp.duckdns.org/`, con llamada real a `claude-opus-5` (`~$0,0034`). Antes hubo que desplegar (la máquina corría `699f2b2`, anterior a `app/trace.py` nacido en `c292210`); resuelto con `git pull` + `systemctl restart teapp`, 40 commits, sin reinstalar (`requirements.txt`/`.env.example` sin cambios). La línea, doce campos, ninguno con la frase de la persona (`PI-8`/`[D-085]` verificados en PRODUCCIÓN); las cuentas cuadran (`3,47 − 0,0 − 3,468 = 0,002`); y mide que de 3,47 s de práctica, 3,468 son el modelo — el código propio tarda 2 ms, el 0,06%, holgando el freno de `[A-011]`. **Por qué se renuncia:** TEAPP es educativa, no comercial — sin clientes ni factura a escala no hay presión que justifique el descenso; el aprendizaje central del arco de evals ya está cobrado en `[L-088]`. `[D-105]` deja escrito, sin ambigüedad, qué SÍ sería obligatorio en una app comercial: vara discriminante antes de cambiar de modelo, medir forma y veredicto por separado, distinguir corregir-de-más de perdonar, medir coste real (no de lista), regresión automática en cada cambio de modelo. **Marca nueva en `decisions.md`:** `~~D-nnn~~` ⛔ **RENUNCIADA**, tercera junto a 🔻 SUPERADA y ✅ CUMPLIDA — ninguna de las dos existentes servía para `~~D-049~~` (no la superó otra decisión, no se cumplió). **`[A-031]` nueva** (sesión principal, ya escrita al arrancar este cierre): el marcador de `jorge` bajó de `score: 9` (`T-078`, 2026-08-13) a `score: 6` (traza de hoy) — un marcador solo sube; la mitad benigna (`score` < `practice` por prácticas falladas con respuesta incorrecta) confirmada por el usuario, la BAJADA no explicada, se supone un borrado de marcadores en algún ensayo (`deploy/README.md:98` documenta el `rm`), anotada en vez de resuelta por decisión del usuario al cerrar el paso 9. **Deuda DORMIDA hallada al preparar `[D-105]`, con tarea propia nueva `T-113`:** dos constantes de modelo — `app/tools.py:41` (`MODEL_NAME`, la app) sin ningún test que la clave (`test_tools.py`, `test_trace.py`, `test_eval_rubric.py` la DERIVAN) contra `deploy/check_api_key.py:104` (`MODEL`, el portero) que sí la clava `test_check_api_key.py:91` — cambiar el modelo de la app es hoy un cambio MUDO. Queda dormida al renunciar a `[D-049]`, se reabre si alguien vuelve a tocar el modelo. **Tareas del índice tocadas en este cierre:** `T-102` pasa a ✅ (traza vista en producción); `T-112` pasa a ✅ y NO se reabre (condición 1 cumplida con resultado fallido documentado); `T-113` nueva y 🔲. `T-108`, `T-103`, `T-081` siguen 🔲, fuera del criterio de cierre por `[D-103]`, deuda pequeña que no decide el cierre. **Suite: 580 passed**, corrida en este cierre con `.venv/Scripts/python.exe -m pytest -q` (58,59 s) — el rojo de `[S-072]` (`test_the_archived_replies_are_backed_up_by_git`) ya está verde. **Cero código en el diff de hoy** (PI-3): solo `_persistence/assumptions.md` y `_persistence/decisions.md`, y ambos ya estaban escritos por la sesión principal antes de que arrancara este cierre — este tramo solo actualiza `progress.md` y `tasks.md` y hace el commit. `constraints.md` sin cambios en el diff de hoy — revisado, al día. `PI-8` sobre el diff de `_persistence/` de hoy: sin frases de personas — el diff de `assumptions.md` y `decisions.md` habla de trazas, marcadores, constantes y renuncias de alcance, ninguna cita textual de quien practica la app. Paso 2b: `tsc` compiló sobre `tsconfig.json` y `app/static/app.js` coincide byte a byte con lo generado — al día, sin cambios de `.ts` en el diff de hoy | 9 |
| S-072 | 2026-08-19 | Quincuagésimo quinto tramo, después del cierre que dejó `[S-071]` (`fcd2d2d`). **`T-112` corrió y salió FALLIDA.** 30 frases discriminantes añadidas a `measure_tutor.SENTENCES` (posiciones 61-90), barajadas y sin marca de brazo. El humano etiquetó las 30 a mano: 15 `correct`, 15 `wrong`, 0 `unclear`, 0 notas. Juez: 30 llamadas a `claude-opus-5`, tanda `pick`, `$0,1026`, archivo `eval_replies_claude-opus-5_2026-08-19_run-df89768f_pick.jsonl` archivado en `_persistence/replies/`, los dos porteros en verde. Cruce con las 60 representativas excluidas: **0 desacuerdos de 30** — cae en el tramo `0-2` de `[D-104]` = 🔴 fracaso, la vara no discrimina y se reescribe. **`[D-104]` ya venía escrita por la sesión principal:** el sello de `T-112` con cinco tramos escritos antes de correr, reparto 15/15 sellado aparte con sha256 en `_persistence/seals/hard_arms_sealed.json`, dos desenlaces (apertura del sello y resultado del juez), y una ENMIENDA a `[D-103]` — su afirmación de que las dos condiciones del paso 9 "se pagan con el mismo gasto" es FALSA, porque `app/trace.py` no escribe `sentence`, `reply` ni el número de frase (`[D-085]`, `PI-8`); `T-102` se paga aparte, ~`$0,01`. **`[L-088]` ya venía escrita:** la lección del fracaso — el aviso llegó gratis, una hora antes que la factura (el cruce diseño↔etiqueta humana dio 30/30, y eso decía "inequívoca para dos lectores", no "difícil para el juez"); la pista sin sellar apunta a FORMA BAJO CARGA (2/30 rompieron `too_many_sentences` contra 0/60), no a rareza gramatical. **`PI-6`, tres tests tocados con autorización explícita del usuario del 2026-08-19, razón escrita dentro de cada uno:** `test_the_budget_is_exactly_the_number_of_sentences` → `test_a_full_run_costs_what_we_decided_to_pay` (número a mano, `MAX_CALLS * COST_PER_CALL_USD <= 0.31`, guarda dinero no frases); `test_no_numbers_means_all_sixty` → `test_no_numbers_means_all_of_them` (derivado de `len(SENTENCES)`); el `"61"` de `test_a_bad_number_stops_before_calling_anybody` → `str(len(SENTENCES) + 1)`. Criterio firmado: se deriva cuando el test afirma una relación, se escribe a mano cuando afirma una decisión. Archivos nuevos: `_persistence/seals/` (`hard_arms_sealed.json` + `README.md`) y `tests/test_seals.py` (3 tests, dos sabotajes vistos morder). Corrección de coste anotada por la sesión principal: se usó por error `$0,00304` (caducado) en vez de `$0,00342` (`[D-096]`) en algún cálculo intermedio; cifras buenas: corrida de 60 = `$0,2052`, de 90 = `$0,3078`, pick de 30 = `$0,1026`. **Suite: 580** — corrida en este cierre con `.venv/Scripts/python.exe -m pytest -q`: 579 passed + 1 failed (`test_the_archived_replies_are_backed_up_by_git`, exige que el `.jsonl` de respuestas esté trackeado por Git; se pone verde al hacer `git add` en este commit, es la forma esperada del test, no un bicho). `decisions.md` y `lessons.md` ya venían escritas por la sesión principal (`[D-104]`, enmienda a `[D-103]`, `[L-088]`) — revisadas contra el diff, al día, índice y entrada juntos. `assumptions.md` y `constraints.md` sin cambios en el diff de hoy — revisados, al día. `PI-8` sobre el diff de `_persistence/` de hoy: sin frases de personas — las 30 frases discriminantes son inventadas para romper al juez (subjuntivos, colectivos en singular, preposiciones típicas de error), no texto de quien practica la app. Paso 2b: `tsc` compiló sobre `tsconfig.json` y `app/static/app.js` coincide byte a byte con lo generado — al día, sin cambios de `.ts` en el diff. **Tareas:** `T-108` sube de rendija teórica a CRUZADA Y MEDIDA (la propia sesión ejecutora la cruzó al primer intento, sin saber que existía, al escribir el sello `.json` en `_persistence/seals/` que ningún `glob *.jsonl` mira); `T-112` deja de estar `🔒 Bloqueada por T-109` y de decir "sin firma ni prioridad decidida" — corrió, salió fallida, se reabre con el diagnóstico de `[L-088]` dentro | 9 |
| S-071 | 2026-08-19 | Quincuagésimo cuarto tramo, después del cierre que dejó `[S-070]` (`799cc00`). Tramo corto, sin código, solo `_persistence/tasks.md` y `_persistence/decisions.md`. **Corrección de estado:** `T-109` seguía `🔲` en la fila del índice y en la línea `Estado:` de su entrada de ancla, estando cerrada y commiteada en `799cc00` — pasada a `✅` en los dos sitios. Es el bicho que `[T-098]` y `[L-062]` describen (el estado se pudre por detrás de las entradas): habría ofrecido `T-109` como trabajo por hacer en el próximo arranque. **`[D-103]` firmada por el usuario:** el paso 9 no tenía criterio de cierre escrito (`roadmap.md:23` le dedica una línea, `scope.md` no lo menciona) y se descubrió buscándolo. Ahora el paso 9 cierra con DOS condiciones: (1) existe una vara DISCRIMINANTE escrita, etiquetada y corrida; (2) la traza se ha visto escribir con servidor levantado y modelo real. `T-103` y `T-108` quedan fuera a propósito — deuda pequeña que no decide el cierre. Las dos condiciones se pagan con el mismo gasto si `T-112` corre con el servidor levantado, lo que cerraría `T-102` de paso. **Suite: 577 passed**, confirmado corriendo `python -m pytest -q` en este cierre. `decisions.md` ya venía escrita por la sesión principal (`[D-103]`, índice y entrada juntos) — revisada contra el diff, al día. `assumptions.md`, `constraints.md` y `lessons.md` sin cambios en el diff de hoy — revisados, al día. `PI-8` sobre el diff de `_persistence/` de hoy: sin frases de personas — el diff habla del criterio de cierre del paso 9 y del estado de `T-109`, nada de texto de quien practica la app. Paso 2b: `tsc` compiló sobre `tsconfig.json` (sin pasarle archivos por línea de comandos, para que tome el `include`) y `app/static/app.js` coincide byte a byte — al día, sin cambios de `.ts` en el diff. **Tareas:** `T-109` corregida a ✅ en índice y entrada (ya estaba cerrada desde `[S-070]`, solo faltaba el estado) | 9 |
| S-070 | 2026-08-19 | Quincuagésimo tercer tramo, después del cierre que dejó `[S-069]` (`b727ae9`). **`[D-102]` firmada, con TRES enmiendas del mismo día:** el nombre del corpus deja de llevar la huella de `GRAMMAR_RUBRIC` y pasa a llevar un SELLO DE CORRIDA — `sha256(huella_rúbrica + huella_frases + huella_detector)[:8]` — porque `T-112` cambia exactamente el conjunto de frases y el nombre viejo no se habría movido. La promoción a `_persistence/corpus/` pasa de comparar cuatro ejes a comparar uno. `~~D-092~~` tachada en índice y cuerpo, con lo que sobrevive citado desde `[D-102]`. **Primera enmienda:** la redacción inicial sellaba los TEXTOS crudos, no las huellas — con eso el cruce nombre↔fila era imposible; corregido a hashear las tres huellas, así `name_matches_rows` puede recalcular el sello desde la fila. **Segunda enmienda:** la autorización preveía que `test_the_archived_name_agrees_with_its_rows` "siguiera verde" como garantía; se saboteó `name_matches_rows` saltándose los nombres legado y los 574 tests siguieron en verde sin enterarse — de ahí `[L-087]`, tapado con `test_a_legacy_name_that_lies_about_the_rubric_is_still_caught`. **Tercera enmienda:** `detector_fingerprint()` hasheaba el archivo `rubric_check.py` pero no sus tres constantes (`MAX_SENTENCES`, `VERDICT_CORRECT`, `VERDICT_WRONG`); corregido a hashear fuente + valores. **`T-109` CERRADA en sus dos mitades:** (b) `save_replies` pasa de `open("w")` a `open("x")`, se niega con `SystemExit` si el archivo existe — el escape es borrar a mano, no un flag; (a) `replies_file()` compone el nombre con `run_seal()`, las tres huellas viajan además en cada fila. ⚠️ **La justificación con la que `T-109` llegó de prioridad era FALSA:** decía que dos corridas enteras el mismo día pisarían "la única copia archivada del 58/58"; medido contra el disco, `save_replies` escribe en `data/` (`config.require_data_dir()`) y `cross_check.py` lee de `_persistence/replies/` — dos carpetas distintas, y la archivada ya está en Git desde `[D-099]`. Enmendado en `[T-109]` de `tasks.md`, no reescrito aparte. **`T-110` ABSORBIDA por `T-109`** — estado nuevo `🔗 absorbida` en la leyenda de `tasks.md`: no descartada (el hallazgo es correcto) ni hecha (era una de tres piezas sin sellar, no la única). `T-108` sigue pendiente, alcance ampliado de un sitio a TRES (`replies.py:83`, `tests/test_eval_rubric.py:512`, `tests/test_replies.py:25`) — el tercero lo encontró la sesión ejecutora. `T-112` queda DESBLOQUEADA (el bloqueo era el nombre que no distinguía exámenes). **Autorización `PI-6` firmada por el usuario:** jubila tres tests, no dos — el tercero (`tests/test_eval_rubric.py:545`) no se ponía rojo, quedaba verde y hueco. **Seis sabotajes vistos morder** además del de `[L-087]`: `"x"`→`"w"`, reventar después de truncar, el conjunto de frases fuera del sello, el detector fuera del sello, saltarse la generación legado, quitar las constantes del sello del detector. **Suite: 577 passed**, confirmado corriendo `.venv/Scripts/python.exe -m pytest -q` en este cierre. `decisions.md` y `lessons.md` ya venían escritas por la sesión principal — revisadas contra el diff, al día (índice y entrada juntos en `D-102`, `~~D-092~~`, `L-087`). `assumptions.md` y `constraints.md` sin cambios en el diff de hoy — revisados, al día. `PI-8` sobre el diff de `_persistence/` de hoy: sin frases de personas — el diff habla de huellas, sellos y constantes de detector, nada de texto de quien practica la app. Paso 2b no aplica: ningún `.ts`/`.js` en el diff de hoy. ⚠️ **Nota de reparto:** esta sesión principal escribió `tasks.md` (normalmente del cierre) para que la justificación falsa de `T-109` no durmiera en el disco hasta el cierre — candidata a cambiar la regla de `CLAUDE.md`, no excepción a repetir. **Tareas:** `T-109` pasa a ✅; `T-110` pasa a 🔗; `T-108` gana alcance; `T-112` se desbloquea | 9 |
| S-069 | 2026-08-18 | Quincuagésimo segundo tramo, después del cierre que dejó `[S-068]` (`f7a64ad`). **`[D-099]` firmada (commit `010c8e5`): el corpus de las 60 respuestas del juez se ARCHIVA en `_persistence/replies/` y se BORRA de `data/`** — las dos mitades del cruce de `T-111` vivían en regímenes opuestos (`data/` sin copia y fuera de Git) y la desprotegida era la que costó dinero. Orden invertido respecto al propuesto: copiar → verificar en Git → borrar, porque `mv` como primera acción deja un solo instante con una sola copia en el mundo. `replies.py` + `tests/test_replies.py` (12 tests): conjunto de campos CERRADO, y el docstring declara que `reply` —la prosa entera del archivo— no lo audita nadie. El rojo existió antes del `git add` (`test_the_archived_replies_are_backed_up_by_git`). `L-084` nueva: un argumento de seguridad de dos mitades ciertas puede no tocar la superficie expuesta. **`[D-100]` firmada (commit `546a67a`): el sello de `T-111` escrito ANTES de calcular** — la REGLA de exclusión (no la lista: se excluye la fila cuyo contenido o veredicto se expuso antes de etiquetar; salen 54 y 55, entra la 37), denominador 58 con el de 60 al lado, cuentas crudas antes que porcentaje, las dos predicciones (`56/58`) marcadas como CONTAMINADAS porque un agregado se coló con otra etiqueta, y la tabla 2×2 (perdonar / corregir de más) en vez de un solo número. **`T-111` CERRADA (commit `ed8d162`): el cruce da 58 de 58 sobre las 58 selladas, 60 de 60 sin excluir** — cero veces el juez perdonó, cero veces corrigió de más, cero formato roto. `cross_check.py` + `tests/test_cross_check.py` (7 tests) nuevos, sobre `_persistence/labels/` y `_persistence/replies/`, usando `app.tools.split_verdict`. Saboteado antes de creerlo: voltear una etiqueta da 57/58, voltear una respuesta da 57/58 — la casilla se mueve donde toca. `L-086` nueva: un 100% aquí no mide al juez, mide el EXAMEN — `SENTENCES` (importada de `measure_tutor.py`, escrita para medir al tutor, no al juez) no discrimina, el eval quedó saturado desde que se reutilizó el archivo, y el día que `[D-049]` baje el modelo esta vara no verá la caída hasta que sea enorme. **`L-085` corregida en el mismo diff, no reescrita aparte:** la "tercera pieza" que decía que se inventó un `54` en boca de otra terminal queda como que NINGUNA de las dos predicciones (ni el `54` del usuario ni el `56` de la auditoría) llegó a disco — un sello que vive en el chat no es un sello; se sellaron las reglas (`D-100`) y no los números que esas reglas protegían. **`D-101` nueva, sin commitear al arrancar este cierre:** la próxima vara de frases se declara HOY, antes de escribirla, como DISCRIMINANTE (busca dónde se rompe el corrector) y no representativa (parecerse a lo que escribe quien practica) — la pregunta viva es `[D-049]`. `[D-049]` gana una condición: no se toca `MODEL` hasta que exista una vara capaz de bajar, porque un eval en el techo da 100 antes y 100 después del cambio y no sirve de freno de regresión. `[L-083]` sigue viva sin portero que la cierre: quien escriba las frases discriminantes será quien las etiquete, con el veredicto fresco en la cabeza. **Suite: 570 passed**, confirmado corriendo `.venv/Scripts/python.exe -m pytest -q` en este cierre. `decisions.md` y `lessons.md` ya venían escritas por la sesión principal (los tres commits del día más la corrección de `L-085` y `D-101` sin commitear) — revisadas contra el diff, al día (índice y entrada juntos en `D-099`, `D-100`, `D-101`, `L-084`, `L-085`, `L-086`). `assumptions.md` y `constraints.md` sin cambios en el diff de hoy — revisados, al día. `PI-8` sobre el diff de `_persistence/` de hoy: sin frases de personas — el diff habla de las 60 frases de `measure_tutor.SENTENCES` (corpus inventado) y de reglas de exclusión, nada de texto de quien practica la app. Paso 2b no aplica: `frontend/app.ts` sin cambios desde `76e9bee`, ningún `.ts`/`.js` en el diff de hoy. **Tareas:** `T-111` pasa a ✅; `T-109` deja de ser tarea de fondo del paso 9 y apunta a `_persistence/replies/` con prioridad; tarea nueva sin numerar (escribir el conjunto discriminante) | 9 |
| S-068 | 2026-08-18 | Quincuagésimo primer tramo, después del cierre que dejó `[S-067]` (`8a7c44f`). **`[D-097]` firmada: el etiquetado manual de las 60 frases vive en `_persistence/labels/`, HERMANA de `corpus/` y no dentro** — criterios de entrada opuestos (`corpus/` guarda lo que ya no es producción y un test lo hace cumplir; las etiquetas nacen contra la rúbrica viva y valen mientras viva) y descartado también `data/` (disco sin copia, fuera de Git, y el trabajo humano no se vuelve a pagar). Commit `206ca43`: `labels.py` + `tests/test_labels.py` (17 porteros) + `_persistence/labels/README.md` + esqueleto de 60 filas con `verdict: null`. El portero valida FORMA —`sentence` contra `SENTENCES`, `verdict` contra el conjunto cerrado, prosa libre confinada a un único campo `note` que ningún programa audita— no criterio. **`T-106` CERRADA** en commit `28ed31b`: las 60 frases etiquetadas a mano por el humano — 27 `correct`, 33 `wrong`, 0 `unclear`, 0 notas; ningún veredicto lo puso una terminal. El portero cazó la fila 21 corrupta (texto pegado al editar: `"I did not saw him yeshagmasterday"`) y se restauró el texto contra `SENTENCES` sin tocar el veredicto — el reparto 27/33 es idéntico antes y después, prueba de que solo se tocó el texto de esa fila. `[D-098]` firmada: la vara del etiquetado es el inglés escrito de libro, no el que un nativo aceptaría de oído — deducida de las etiquetas (frases 22, 30 y 54 salieron `wrong`, tres de tres), no preguntada; la frase 55 se revisó bajo esa vara y **no** cambió (el retroceso de tiempo tras `told` es opcional cuando el estado sigue vigente). `L-082` nueva: citar un mecanismo por su nombre no comprueba su alcance (dos veces el mismo día: `T-108` señalada como bloqueante sin serlo, `rubric_check.learner_message` señalado como lector reutilizable cuando el bueno es `tools.split_verdict`). `L-083` nueva: ilustrar `unclear` con las frases 54 y 55 del propio conjunto ancló esas etiquetas antes de que existieran — el archivo tiene portero, el chat no. Cero `unclear` y cero notas en las 60: queda escrito que eso no se auditó. **Suite: 551 passed**, según el mensaje de ambos commits — no se corrió de nuevo en este cierre porque el árbol ya estaba limpio y sin diff que verificar. `decisions.md` y `lessons.md` ya venían escritas por la sesión principal dentro de los dos commits del día —revisadas contra `git show --stat`, al día (índice y entrada juntos en `D-097`, `D-098`, `L-082`, `L-083`). `assumptions.md` y `constraints.md` sin cambios en el diff de hoy —revisados, al día. `PI-8` sobre `_persistence/labels/sentence_labels.jsonl`: sin frases de personas — las 60 son de `measure_tutor.SENTENCES`, el corpus inventado, no texto de quien practica la app. Paso 2b no aplica: `git status` salió limpio al arrancar este cierre, sin diff que compilar ni comparar. **Sin diff propio que commitear en este cierre** —el trabajo del día ya estaba en `206ca43` y `28ed31b`, ambos sin subir a `origin` al empezar | 9 |
| S-067 | 2026-08-18 | Quincuagésimo tramo, después del cierre que dejó `[S-066]` (`165f415`). **`D-095` (commit `165f415`, ya cerrado al arrancar este tramo):** el nombre del archivo de respuestas de `eval_rubric.py` se calcula con `len(records)` (lo que llegó), no con `calls = len(plan)` (lo planeado) — evita que una tanda cortada se guarde con nombre `full`, y evita que `save_replies` (abre en `"w"`) borre una línea base entera con una corrida cortada del mismo día. Test nuevo `test_a_run_that_is_cut_short_is_named_by_what_arrived`, primero del proyecto que entra en `main()`. `L-080` nueva: un aviso en consola no protege un archivo — cuando el informe y el nombre se contradicen, sobrevive el nombre. **En este tramo, recogido sin commitear:** `measure_tutor.py` — `COST_PER_CALL_USD` de `0,00304` (caducado) a **`0,00342`** (`D-096`), medido con la factura real de **$0,20** de la corrida de línea base de hoy: 60 llamadas, **entera, 60/60, cero cortes** — confirmado que no hubo otra llamada contra la cuenta ese día. `MAX_CALLS_PER_RUN` de 82 a 73; el margen al acantilado (`int(0,25/x) >= 60`) de veintidós llamadas de sobra a trece. `tests/test_measure_tutor.py`: solo el EJEMPLO del docstring de `test_the_cap_still_lets_the_whole_run_through` cambia (82/veintidós → 73/trece); el `assert` **no se tocó** — distinción de `L-078`. El archivo de la corrida, `data/eval_replies_claude-opus-5_2026-08-18_rubric-bbf4be38_full.jsonl` (60 líneas), se queda en `data/` — ignorado por `.gitignore`, correcto; no se promueve a `_persistence/corpus/` porque `[D-092]` cuelga esa promoción del commit que mueva `MODEL` o `GRAMMAR_RUBRIC`, y ninguno se movió hoy. `L-081` nueva: la huella del corpus (`rubric_fingerprint()`) sella `tools.GRAMMAR_RUBRIC` — la pregunta que se le hace al modelo —, no `rubric_check.py` — la báscula que corrige la respuesta —; comprobado corriendo el detector de hoy contra el corpus congelado del 17: 10 filas guardadas como rotas, 1 rota con el detector de hoy, nueve discrepancias, todas `too_many_sentences`. El caso concreto que lo destapó sí estaba cubierto por la huella (`MAX_SENTENCES` vive dentro de `GRAMMAR_RUBRIC`); el agujero es real igual, por construcción de `rubric_fingerprint()`. **Tareas nuevas:** `T-109` (dos corridas enteras el mismo día se pisan — modelo/fecha/huella idénticos, `save_replies` abre en `"w"`, el juez no es determinista; sellada en el cuerpo de `165f415`) y `T-110` (candidata, no decidida: huella de `rubric_check.py` en la fila del corpus, no un quinto eje en el nombre — la firma el humano). Pendiente antes de `T-106`: decidir dónde vive el archivo de etiquetado a mano de las 60 frases (disco sin copia, fuera de Git, deja de tener precio en cuanto lleve juicios humanos). **Suite: 534 passed**, confirmado corriendo la suite en este cierre. `decisions.md` y `lessons.md` ya venían escritas por la sesión principal antes de este cierre —revisadas contra el diff, al día (índice y entrada juntos en `D-096` y `L-081`). `assumptions.md` y `constraints.md` sin cambios en el diff de hoy — revisados, al día, sin rastro de `COST_PER_CALL_USD`/`MAX_CALLS_PER_RUN`. `PI-8` sobre el diff de `_persistence/` de hoy: sin frases de personas — el diff habla de constantes de coste y de la huella del corpus, nada de texto de quien practica. Paso 2b: hay `frontend/app.ts` en el repo pero sin cambios en el diff de hoy — comprobado igual (`tsc` compila, `app/static/app.js` coincide byte a byte): al día | 9 |
| S-066 | 2026-08-18 | Cuadragésimo noveno tramo, después del cierre que dejó `[S-065]` (`9844eac`). **`T-107` CERRADA — el corpus del eval ya no se borra en cada corrida.** `eval_rubric.py` gana `rubric_fingerprint()` (`sha256` de `GRAMMAR_RUBRIC` ya montado, 8 caracteres) y `replies_file(picked)` con CUATRO ejes en el nombre —modelo, fecha, huella de rúbrica y marca `full`/`pick`—, más `sentences_are_invented()` como cerradura de `PI-8`. Las 10 filas del diagnóstico del 2026-08-17 se PROMOVIERON de `data/` a `_persistence/corpus/`, con `README.md` explicando la carpeta y un portero de tres tests (`glob` sobre `*.jsonl`, corren en cada `pytest`). `[D-092]` (los cuatro ejes y el disparador pegado al commit que mueve `MODEL`/`GRAMMAR_RUBRIC`) y `[D-093]` (la cerradura, AMPLIADA el mismo día con el portero porque la cerradura sola seguía siendo un acto de acordarse). `[L-079]` nueva: un criterio de conservación evaluado DESPUÉS deja la evidencia esperando en el sitio menos duradero — la propuesta descartada de esta sesión. **`T-105` CERRADA — corrector de rúbrica cableado a producción.** `app/tools.py`: `GrammarVerdict.correct: bool` pasa a `outcome: str` (tres estados: `correct`/`wrong`/`bad_format`) más `broken: frozenset[str]`; `correct` queda como propiedad derivada, no campo. `split_verdict` compone los tres estados y llama a `rubric_check.check_reply(answer)` con import diferido dentro de la función (arriba sería ciclo, `[D-091]`). `app/english_tutor.py`: `TutorReply` gana `outcome` y `broken`, subidos sin tocarse desde el veredicto; `correct` de `TutorReply` se queda —es el que da el punto en el marcador—. `app/api.py` y `app/trace.py`: la traza deja de escribir `correct` y escribe `outcome` + `broken`. `[D-094]` nueva: el motivo es que `correct=False` mezclaba dos causas opuestas —el alumno se equivocó / el juez rompió el formato— como el mismo dato (`[D-089]`, `LM.15`). **Suite: 516 → 533 passed**, confirmado corriendo la suite en este cierre. Sabotajes vistos morder en ambas tareas (cuatro en `T-105`, tres en el portero de `T-107`). `decisions.md` y `lessons.md` ya venían escritas por la sesión principal antes de este cierre —revisadas contra el diff en este cierre, al día (índice y entrada juntos en las cuatro filas nuevas). `assumptions.md` y `constraints.md` sin cambios en el diff de hoy —revisados, al día. `PI-8` sobre el diff de `_persistence/` de hoy: sin frases de personas —las 10 filas promovidas son de `measure_tutor.SENTENCES`, comprobado fila por fila y por el portero nuevo. Paso 2b no aplica: ningún `.ts`/`.js` en el diff | 9 |
| S-065 | 2026-08-17 | Cuadragésimo octavo tramo, después del cierre que dejó `[S-064]` (`df616dd`). **`T-104` CERRADA, en las dos mitades que estaban abiertas esperando firma — las dos firmadas por el usuario.** `[D-090]`: el tope de `GRAMMAR_RUBRIC`/`rubric_check.MAX_SENTENCES` sube de dos a tres frases, y el motivo escrito **no** es el de `[D-089]` (que se leyó entero y quedó desmentido: la rúbrica pide dos cosas para `FIX`, no tres — lo cálido es tono, no un renglón), sino que el "dos" duplicaba `"never correct more than one thing at a time"` y que `too_many_sentences` salía roja 18/60 con Opus 5 — un detector saturado deja de avisar cuando a un modelo más pequeño se le vaya la forma (`[D-049]`). `[D-091]`: se endurece la rúbrica prohibiendo TODAS las comillas en vez de afinar el corrector — la corrección no trae delimitador (cinco formas distintas de introducirla en las nueve `FIX` de disco), así que la opción fina sería una heurística sobre el fraseo del modelo, y el fraseo es justo lo que `[D-049]` va a mover; `MAX_SENTENCES` pasa a vivir SOLO en `app/tools.py` (antes dos números vigilados por dos comentarios). **Visto morder tres veces más** (prompt a mano, comilla condicionada, copia del monedero restaurada) y **al revés** (tope aflojado a cuatro → 3 rojos). Suite: 513 → **516 passed**, corrida en este cierre. `[D-089]` queda ENMENDADA en su sitio (arriba de la entrada y en el índice): su hipótesis final quedó desmentida y su "línea base de 60" es falsa por omisión — nunca se guardó (`[L-076]`). `[L-076]` nueva: el instrumento contó 60 respuestas y no guardó ninguna; el arreglo llegó tarde y en modo sobrescribir (`open("w")`), así que cubre una corrida y no dos. `[L-077]` nueva: `COST_PER_CALL_USD` estaba duplicado en `eval_rubric.py`, tres líneas bajo el comentario que decía "se importa, no se copia" — arreglado importándolo, atado con `test_the_wallet_is_imported_not_copied`. `[L-078]` nueva: la pregunta que separa arreglar un ejemplo de ablandar un listón es mirar el `assert` — si sigue igual o más exigente, se arregló el ejemplo. `measure_tutor.py`: `COST_PER_CALL_USD` marcado CADUCADO hacia el lado malo (más tokens de salida con tres frases, el freno de la tanda deja pasar de más), sin inventar número nuevo (regla 6). **`T-107` nueva, con prioridad sobre la corrida de 60:** `replies_file()` (`eval_rubric.py:188`) da un nombre fijo y `save_replies` (línea 243) abre en `"w"` — una tanda de 10 vuelve a borrar una línea base de 60. No se arregla pasando a `"a"`: el nombre tiene que distinguir modelo y fecha, que es lo que `[D-049]` va a mover tres veces; de paso permite guardar las tres líneas base (Opus/Sonnet/Haiku) a la vez. `T-106` pasa a BLOQUEADA por `T-107` — etiquetar 60 frases cruza sesiones y el corpus lo puede borrar cualquier corrida. `T-105` sigue libre: cablear el corrector a `app/api.py` no toca rúbrica ni corpus. `decisions.md` y `lessons.md` ya venían escritas por la sesión principal antes de este cierre — revisadas contra el diff en este cierre, al día (índice y entrada juntos en las seis filas nuevas/enmendadas). `assumptions.md` y `constraints.md` sin cambios en el diff de hoy — revisados, al día. Paso 2b no aplica: ningún `.ts`/`.js` en el diff | 9 |
| S-064 | 2026-08-17 | Cuadragésimo séptimo tramo, después del cierre que dejó `[S-063]` (`25332da`). **Dos frentes.** **Frente 1 — corrección de fecha:** `[L-069]` (tercera mitad) documenta que la regla "la fecha se copia de `git log`" duró 4 h 47 min: entre `41c753f` (10:54) y `25332da` (15:41) entraron 16 apariciones de `2026-08-18` en 6 archivos de TEAPP —`decisions.md` ×6, `lessons.md` ×3 (con el título de `[L-074]` dentro), `progress.md` ×3, `app/english_tutor.py` ×2, `app/trace.py`, `tests/test_trace.py`—, más 1 en `Edu_TripleS/PROGRESO.md`. Las 17 corregidas a 2026-08-17, verificado contra `git log` (los commits del día son del 17). **Frente 2 — arranque de los evals del paso 9, por la forma de la rúbrica:** `[D-089]` nueva — se empieza por las CUATRO promesas de `GRAMMAR_RUBRIC` que comprueba un programa sin opinar (primera línea `OK`/`FIX`, sin markdown/comillas, dos frases como mucho, sin fuga de la palabra clave), no por si el veredicto acierta. **Código nuevo:** `app/rubric_check.py`, `eval_rubric.py`, `tests/test_rubric_check.py`, `tests/test_eval_rubric.py` — cero cambios en `app/tools.py` de producción. **Tests: 456 → 512 passed**, confirmado corriendo `.venv/Scripts/python.exe -m pytest -q` en este cierre. Trece sabotajes vistos en rojo. **Dos corridas reales pagadas** (números completos en `[D-089]`, no repetidos aquí): línea base de 60 llamadas a Opus 5 (~$0,18) — limpias 40 de 60, `too_many_sentences` falla 18 veces y `bad_first_line` en 0, lo que acusa a la rúbrica y no al modelo — y una corrida de diagnóstico de 10 frases (~$0,03) que confirmó el patrón (aliento + corrección + explicación = tres frases) y cazó un falso positivo del propio instrumento con comillas que nombran, no que envuelven una corrección. `[L-075]` nueva: `_leaks_keyword` tenía el docstring con la regla correcta y la línea de debajo hacía exactamente lo prohibido (`.upper()` marcaba el `ok` de *"that's ok"* como fuga) — escritos en el mismo minuto, lo cazó el test, no la relectura. `[D-088]` nueva: `T-103` se PARA con disparador de acción, igual que `T-102` — el camino que dejaría `tutor_started[0]` vacío no tiene hoy quien lo recorra, así que un `if` protector sería `[L-048]` (guardián que se cumple solo); firmada por el usuario tras dos votos técnicos coincidentes, sin tocar código. `decisions.md` y `lessons.md` ya venían escritas por la sesión principal antes de este cierre — revisadas contra el diff en este cierre, al día, sin faltantes. `assumptions.md` y `constraints.md` sin cambios en el diff de hoy — revisados, al día. Paso 2b no aplica: ningún `.ts`/`.js` en el diff. 🔻 **Abierta, esperando firma del usuario:** subir el tope a TRES frases en la rúbrica y afinar la comprobación de comillas — preguntado al final de la sesión, sin contestar, nada tocado. Sin cablear a producción: cablear el corrector (que la ruta lo llame y la traza apunte el fallo de formato) y etiquetar las 60 frases para medir si el veredicto acierta quedan como los dos próximos pasos, ninguno decidido | 9 |
| S-063 | 2026-08-17 | Cuadragésimo sexto tramo, después del cierre que dejó `[S-062]` (`c292210`). **El tiempo de la traza se reparte en TRES —`queue_seconds` + `model_seconds` + `rest_seconds` = `seconds`—, y NO en las cuatro fases `connect`/`write`/`pool`/`read` de `app/tools.py`, que una auditoría externa demostró que no se pueden medir.** Verificado en el disco, en las dos librerías del `.venv`: `httpx 0.28.1` y `httpx2 2.9.1`, ambas `_client.py:157`, dan un solo `elapsed` — el total de la respuesta, no un dato por fase; aquellas cuatro son TOPES que se le entregan a la librería, no cronómetros. `[D-087]` nueva, con el porqué de TRES y no dos (la cola no sale por resta: `total − modelo` mezclaría cola con código propio y el descenso de modelo de `[D-049]` parecería inútil cuando el culpable fuera la cola) y dónde se mide cada uno (`queue_seconds` en `app/api.py`, cierre por petición `respond_marking_the_start`; `model_seconds` en `english_tutor.respond`, reloj alrededor de `judge_grammar(sentence)`; `rest_seconds` CALCULADO en `trace.record`, nunca recibido). `[D-085]` queda **ENMENDADA en el sitio, no matizada debajo:** su frase *"la arquitectura ya piensa en fases; el registro no las escribe"* era cierta palabra por palabra y engañaba en conjunto — la propuesta de las cuatro fases nació de leerla. `TutorReply.model_seconds` nuevo; docstring de la clase corregido ("en tres piezas separadas" → ya eran cinco campos desde `[D-066]`), igual que la cabecera de `tests/test_trace.py` (enumeraba tres tests de cinco). `[L-074]` nueva: dos frases CIERTAS —el docstring viejo y la frase de `[D-085]`— llevaron a dos conclusiones falsas el mismo día, sin ningún dato mal; las dos describían un estado ANTERIOR que se quedó escrito. **Suite: 452 → 456 passed**, confirmado corriendo `.venv/Scripts/python.exe -m pytest -q` en este cierre. El alambre de `[L-073]` (conjunto de campos de `TutorReply` clavado) se puso rojo solo al añadir el campo nuevo — segunda vez que muerde. Tres sabotajes vistos en rojo antes de aceptar los tests: `model_seconds=0.0` clavado (`assert 0.0 >= 0.05`), reloj abrazando `respond` entera (`assert 0.151 < 0.15`), `queue_seconds=0.0` clavado (`assert 0.0 >= 0.2`). `decisions.md` y `lessons.md` ya venían escritas por la sesión principal antes del cierre (`[D-087]` nueva, `[D-085]` enmendada en el sitio y en el índice, `[L-074]` nueva) — revisadas contra el diff en este cierre, al día, sin faltantes. `assumptions.md` y `constraints.md` sin cambios en el diff de hoy — revisados, al día. Paso 2b no aplica: ningún `.ts`/`.js` en el diff. ⚠️ **Pendiente avisado en el código, no cerrado:** en el camino del timeout con `attempt.cancel() == True` la lista de la marca de cola queda vacía; ese camino no escribe traza hoy, pero un futuro `IndexError` ahí perdería la fila entera por un campo — el aviso está escrito encima de la línea en `app/api.py`. Sigue viva `T-102` (ver la traza con servidor real, disparador = descenso de modelo de `[D-049]`) | 9 |
| S-062 | 2026-08-17 | Cuadragésimo quinto tramo, segundo cierre del 2026-08-17, después del cierre que dejó `[S-061]` (`41c753f`). **Sesión de dos partes.** **Parte 1 — respuesta a auditoría externa sobre el saldo:** leído `$6,24` (16:05 UTC, `[D-084]`), descargando el disparador de `[D-081]`. Un descuadre de `$0,02–0,095` que ninguna barra diaria explicaba se **resolvió sin bicho**: la cuenta también paga el estudio de programación con agentes de IA del usuario — es un tercer inquilino del saldo, invisible en la vista de COSTO (`[C-009]` nueva). `T-096` cerrada sin bicho: los `$0,10` del 1 de agosto son anteriores a `[D-001]`. `[L-071]` (el total cuadraba, el desglose no — cuadrar contra un agregado no es cuadrar) y `[L-072]` (cuatro lecturas de consola para algo que estaba a una pregunta) nuevas. **Parte 2 — arranque del paso 9, por observabilidad:** orden decidido por el usuario — observabilidad → evals con rúbrica → descenso de modelo → seguridad (`[D-085]`, contra la propuesta de la sesión de empezar por evals, que descartaba la observabilidad sin decirlo). `[D-086]` fija los dos frenos de la traza ANTES de la primera línea de código: el test prueba COMPORTAMIENTO (mover `TEAPP_DATA_DIR` y ver que la ruta se mueve), no la forma del código; y un fallo al escribir la traza NO propaga (no le cuesta la práctica a nadie) y NO se calla (queda en el log de prosa). **Código:** `app/trace.py` (NUEVO — el cuaderno, `record()`), `config.trace_file()` (cuarto sitio colgado de `require_data_dir()`), `TutorReply.correct` (la mitad de máquina del veredicto — nunca el texto, que puede citar la frase de la persona), cableado en `app/api.py`. **Tests:** `tests/test_trace.py` (NUEVO), más añadidos en `test_api.py`, `test_config.py`, `test_english_tutor.py`, `test_tools.py`. **`PI-8` nueva en `CLAUDE.md`** (ninguna frase de una persona usuaria entra en `_persistence/`, nunca) + casilla obligatoria en el Paso 5 de `protocol-close` y línea en el reporte del Paso 7 — se estrena en este mismo cierre. `[L-073]` nueva, tres mitades: el campo `correct` nació SIN guardián (saboteado con `correct=True` clavado, la suite dio **447 passed** en verde); el arreglo estructural es clavar el CONJUNTO de campos de una clase (`{campo.name for campo in fields(...)} == {...}`), no el campo suelto — visto morder con un sexto campo de fuga; aplicado a las tres clases que viajan en bloque a un sitio donde nadie las mira campo a campo (`TutorReply`, `GrammarVerdict`, `Counters`). Suite: **452 passed**, corrida en este cierre. Meta-controles: **11/11** (`check_no_data_writes.py` + `check_no_network.py`). Paso 2b no aplica: ningún `.ts` en el diff. 🔻 **Pendiente vivo, con disparador, decisión del usuario:** la traza no se ha visto escribir con el servidor levantado y una llamada real al modelo — disparador = la primera llamada real del descenso de modelo (`[D-049]`), mirando que `trace.jsonl` tenga su línea con el `model` nuevo dentro | 9 |
| S-061 | 2026-08-17 | Cuadragésimo cuarto tramo, primer cierre del 2026-08-17, después del cierre que dejó `[S-060]` (`13d2d79`). **Sesión de respuesta a auditoría externa** sobre el paso 8 ya cerrado, sin cruzar al paso 9. Tres hallazgos, los tres verificados contra el disco antes de aplicarlos, más un cuarto que salió al corregir el tercero. **H-1 → `T-099` cerrada:** `tests/test_check_api_key.py` clavaba solo `LAB_REQUESTS_PER_MINUTE == 50`; el escenario exacto que `[D-081]` mandó vigilar —cambiar `MODEL` sin tocar el 50— dejaba la suite en verde. Ahora clava el par `(MODEL, LAB_REQUESTS_PER_MINUTE)`; visto morder con `MODEL="claude-sonnet-5"` (`1 failed, 439 passed`), restaurado a 440. `[D-082]`, `[L-068]` nuevas. **H-3 → `T-100` cerrada:** `PI-6` (test rojo se arregla el código; tocarlo exige autorización explícita del humano) y `PI-7` (pedir el refactor de forma explícita) entran en `CLAUDE.md` desde el verbatim de `GUIDE.md` §11.i, después de comprobar que la paráfrasis perdía el actor, la voz y los porqués. `[D-083]`, `[L-069]` nuevas. **Hallazgo derivado → `T-101` cerrada:** al citar `LM.nn` en `PI-6`/`PI-7` se detectó que la tabla de citas de `CLAUDE.md` apuntaba a `PROGRESO.md` en vez de `LESSONS.md` — puntero que nació falso el 2026-08-09, no que caducó. Corregido en `CLAUDE.md`, `[L-034]` y `progress.md` (`[S-033]`). `[L-070]` nueva. **Fecha inventada corregida:** `[S-060]` entero decía 2026-08-15; era el quinto y último cierre del **14** (commits `abd1a87`, `a54baa3`, `13d2d79`, confirmado contra `git log` y por el usuario). Se propagó a 40 líneas en 6 archivos, incluidas las dos skills del protocolo — corregida entera, no solo la cabecera. `protocol-close` y `[L-067]` corregidos de paso: el criterio de "falta la fila del día" pasa de comparar FECHA a comparar ID, porque el caso que originó `[L-067]` tenía varios cierres el mismo día y la fecha no distinguía tramos. Código tocado: `deploy/check_api_key.py` (comentario, 6 líneas) y `tests/test_check_api_key.py` (un test reescrito, 23 ins/8 borr). Suite: **440 passed**, corrida cuatro veces. Paso 2b no aplica: ningún `.ts` en el diff | 8 |
| S-060 | 2026-08-14 | Cuadragésimo tercer tramo, **quinto y último cierre del 2026-08-14** (la fila decía *"primero del 2026-08-15"*; no hubo sesión el 15 — fecha corregida el 2026-08-17 contra `git log`, ver `[L-069]`), después del cierre que dejó `[S-059]` (`6c7b5a7`). 🏁 **PASO 8 CERRADO con `[D-081]`**, tras mirar las cuatro pendientes una por una como mandaba `[D-080]`: `T-089` ✅ (cerrada el 14 midiendo), `T-079` 🟡 (baja de ✅ a condición viva), `T-081` 🔲 (aplazada con motivo, PI-3), `T-088` ✅ (**desarmada**). 🔑 **La sesión entera nació de una auditoría de apertura que desmintió el resumen de arranque:** `T-090` se ofreció como pendiente estando decidida y commiteada en `[D-080]` desde el día anterior, y `~~A-024~~` viajó como *"sin comprobar"* estando retirada desde el 11. **`T-088` era el hallazgo gordo:** su ficha decía *"corregir cuando toque el paso 9"* y resultó estar **ARMADA para** el paso 9 — `anthropic-ratelimit-requests-limit` es por modelo, así que cambiar `MODEL` sin tocar `LAB_REQUESTS_PER_MINUTE` hace que `main()` devuelva `EXIT_OK` **aceptando la llave del laboratorio sin dar error**; denegar por defecto convertido en aceptar por accidente. Arreglada con dos comentarios y cero lógica. **`T-098` nueva y cerrada el mismo día:** el guion de arranque leía la prosa en vez del campo de estado y no se saltaba lo tachado — arreglado en `protocol-start` y `session-starter`, y **verificado corriendo el arranque TRES veces**, cada corrida destapando lo que la anterior tapaba (convención de tachado ausente en `decisions.md`; el disparador del paso 9 perdido y colgado de la tarea equivocada; y el roadmap desmintiendo que el paso 9 sea *"bajar a Haiku"* — es *"Observabilidad y evals con rúbrica"*, `_context/roadmap.md:23`). Adoptada la convención `~~D-nnn~~` con dos marcas (🔻 SUPERADA / ✅ CUMPLIDA), aplicada a `~~D-071~~` y `~~D-080~~`. **Cinco lecciones nuevas, `[L-062]`–`[L-066]`**, todas de la misma familia: una afirmación que fue cierta y a la que nadie fue a apagar. Código tocado: solo `deploy/check_api_key.py`, solo comentarios. Suite: **440 passed**, corrida dos veces. Cinco commits, todos subidos a `origin` (`a54baa3`). Paso 2b no aplica: ningún `.ts` en el diff | 8 |
| S-059 | 2026-08-14 | Cuadragésimo segundo tramo, cuarto cierre del 2026-08-14, después del cierre que dejó `[S-058]` (`954b296`). **`T-089` CERRADA — `install.sh` recomendaba `sudo VAR=... bash` para la llave, y la cabecera del propio archivo la traía COMPLETA a tres líneas de un aviso que decía "NUNCA como argumento".** Medido en la EC2 (18:54 UTC): el argumento de `sudo` es visible en `ps aux` para cualquier usuario, dueño `root`. No hubo que rotar la llave (0 rastro en los dos `.bash_history`). Arreglo: `export`/`read -r -s`/`sudo -E`, medido que sobrevive a `env_reset`. `[L-061]` nueva. **`T-097` nueva y cerrada** (número propio, a pedido del usuario): la misma forma retirada de los tres sitios que la traían sin secreto — la plantilla, no la fuga. **`T-090` decidida — el paso 8 NO cierra** (razón: `T-089` resultó ser seguridad real, no cosmética); decisión anotada en `[D-080]` (commit `6c7b5a7`, posterior a este sello) — 🔴 **este renglón decía "sin anotar, queda para el usuario"; corregido el 2026-08-14, ver `[L-062]`.** `T-079` pasa a primera tarea de mañana, sin tocar hoy. **Corrección de arrastre:** el cierre anterior atribuyó la lectura de `T-095` a AWS; era Anthropic (`teapp-measure`) — error solo en el chat, los archivos ya estaban bien. Suite: 440 passed (heredado, sin cambio de código). Paso 2b confirmado: `compilar: 0`, `comparar: 0` — ningún `.ts` en el diff | 8 |
| S-058 | 2026-08-14 | Cuadragésimo primer tramo, tercer cierre del 2026-08-14, después del cierre que dejó `[S-057]` (`4a9baad`). **`T-095` CERRADA — barra del día 14 leída: `$0,18`, RAMA A de `[D-079]`.** `COST_PER_CALL_USD` pasa de derivado a MEDIDO (comentario, intervalo `[$0,00292, $0,00308]`); la constante se queda en `0,00304`, lado alto del intervalo, sigue siendo el freno. `[D-058]` confirmada en su mecánica dos veces sobre números leídos. `[L-060]` nueva: la lectura no discriminó entre las dos predicciones selladas, porque la consola redondea al céntimo y la franja discriminante era más fina que la resolución del instrumento. Suite: 440 passed. Ver entrada en `decisions.md` [D-079] (sección DESENLACE) | 8 |
| S-057 | 2026-08-14 | Cuadragésimo tramo, segundo cierre del 2026-08-14, después del cierre que dejó `[S-056]` (`7ed93ea`). **`T-095` ejecutada por el estudiante — lectura PARCIAL, sigue 🟡 ABIERTA.** Consola de `teapp-measure`, 15:08 UTC: día 14 limpio al token (`21.668 \| 2.959`, idéntico a `T-093`); semana leída `$0,20` valida las tarifas unitarias sobre un mix distinto (7,1:1 contra 5,6:1 de `[D-058]`). Falta el costo del día 14 aislado — el `$0,20` es de "últimos 7 días", sigue derivado, la regla 6 impide cerrar con él. Dos predicciones selladas antes de ese clic ($0,18–$0,19 y, más fuerte, $0,177–$0,187 por resta). `T-096` nueva, prioridad baja. Sin código tocado; sin cambio en la suite (440, heredado de `[S-056]`). Ver entrada en `decisions.md` [D-079] | 8 |
| S-056 | 2026-08-14 | Trigésimo noveno tramo, después del cierre que dejó `[S-055]` (`46cce85`). **`T-094` CERRADA — auditoría de `[D-077]` respondida midiendo.** Tres hallazgos confirmados (H-1 bloqueante: `COST_PER_CALL_USD` caducado, corregido en `[D-078]`; H-2: test nuevo cruza `MAX_CALLS_PER_RUN`/`TARGET_SAMPLES`, suite 440 passed; H-3: báscula reusa conexión, anotado en `[A-030]`) y dos correcciones al propio encargo del auditor. `T-095` sellada de antemano en `[D-079]` (banda `$0,156–$0,205`). `[L-059]` nueva. Ver entrada | 8 |
| S-055 | 2026-08-14 | Trigésimo octavo tramo, después del cierre que dejó `[S-054]` (`464cdf9`). **`T-093` corrida de verdad — `[A-011]` muere al tercer intento, con condición.** Auditoría externa sobre `464cdf9` encontró tres defectos en el criterio antes de gastar (`[D-075]`); deuda de la sesión 71 cerrada con un assert nuevo (`[D-076]`); 60 llamadas reales a `claude-opus-5`, 0 por encima del corte de 6,5 s, VERDE (`[D-077]`, cierre condicionado). Suite: 439 passed. Ver entrada | 8 |
| S-054 | 2026-08-13 | Trigésimo séptimo tramo, octavo cierre del 2026-08-13, después del cierre que dejó `[S-053]` (`a0ccb1f`). **Ronda de preparación, sin auditoría y sin gastar: el criterio de `T-093` queda fijado en el código antes de medir.** `[D-074]` nueva: 60 frases (regla de tres: 3/60 = 5,0%, coincide con la tasa de corte aceptada), tasa de corte del 5%, tres veredictos a ciegas (`verdict_for()`). `measure_tutor.py` pasa de 10 a 60 frases distintas. Suite: 427 passed. Ver entrada | 8 |
| S-053 | 2026-08-13 | Trigésimo sexto tramo, séptimo cierre del 2026-08-13, después del cierre que dejó `[S-052]` (`c89ae1d`). **Tercera vuelta de la misma auditoría del día — el valor de `read` (6,5) no cambia, cambia su justificación:** `[D-073]` retira el "38% por encima del peor de diez" de `[D-072]` (`max(n=10)` es un cuantil que crece con N, no una cota) y lo sustituye por una resta del presupuesto de la ruta que no depende de ninguna medición; `[L-058]` nueva (el peor de N es un suelo que crece) y `[L-055]` ampliada (los punteros por nombre también caducan, no solo los de línea). `T-093` reformulada: ya no cronometra el peor caso de Opus 5, contesta si el presupuesto de 10 s de la ruta es correcto. Ver entrada | 8 |
| S-052 | 2026-08-13 | Trigésimo quinto tramo, sexto cierre del 2026-08-13, después del cierre que dejó `[S-051]` (`3463f2c`). **Segunda vuelta de la misma auditoría: tres hallazgos más, los tres correctos — `[D-072]` corrige la regresión de `[D-071]` (`read` sube de 4,0 a 6,5), `[L-057]` arregla la báscula que se cegaba a sí misma, y `measure_local_parts.py` deja de hablar de "techos". Ver entrada | 8 |
| S-051 | 2026-08-13 | Trigésimo cuarto tramo, quinto cierre del 2026-08-13, después del cierre que dejó `[S-050]` (`0395c1b`). **Respuesta a auditoría externa sobre esa misma ronda: los cinco hallazgos eran correctos.** El grave (hallazgo 1): `[D-070]` cerró `[A-011]` afirmando que "el cliente de Anthropic corta a los 8,0 s pase lo que pase" — **falso**, y medido gratis: `python -c "import anthropic; t=anthropic.Anthropic(api_key='x', timeout=8.0)._client.timeout; print(t.connect, t.read, t.write, t.pool)"` da `8.0 8.0 8.0 8.0`, o sea `httpx` no divide el escalar entre las cuatro fases — **lo multiplica**, y la suma real es **32 s**, no 8. `[A-011]` **REABIERTA** (segunda vez; la primera fue el 2026-08-11, `[L-043]`) — índice y entrada repuestos en `assumptions.md` con el comando de comprobación y lo que falta para cerrarla. `[D-070]` **ENMENDADA, no borrada**: tabla arriba de qué sobrevive (el 10,0 s de la ruta, los 56,3 ms locales) y qué se cayó (el techo de 8,06 s, el margen de 1,94 s), texto original conservado debajo con sus tres punteros de línea muertos tabulados contra dónde viven de verdad hoy. **`[D-071]` NUEVA:** el presupuesto del cliente se reparte fase por fase — `connect 2,0 + write 1,0 + read 4,0 + pool 1,0 = 8,0`, con `anthropic.Timeout` (mismo tipo que `httpx.Timeout`, se usa el fijado en `requirements.txt`). El arreglo de una línea que proponía la propia auditoría (`Timeout(TIMEOUT_SECONDS, connect=2.0)`) **no arregla nada — suma 26 s**; se comprobó antes de aplicarlo y se dejó como el sabotaje con el que se ve morder el test nuevo. **Contradicción del hallazgo 2 resuelta con una corrida:** de las dos cargas incompatibles de `[D-070]` ("el reembolso vive en el `except`" y "la cola no se forma por construcción"), la falsa es la segunda — un 504 devuelve el control y suelta la ficha de `anyio`, pero `respond` sigue corriendo en el hilo del pool y **el sitio no se libera**. Medido con `test_a_timed_out_tutor_keeps_its_pool_seat_with_nobody_waiting`, que llena el pool con peticiones **secuenciales** (nunca dos vivas a la vez, pool de 2, dos peticiones una detrás de otra) y ve a la tercera quedarse en cola igual — visto morder con `max_workers=3` (la tercera arranca y el test cae). **Tres lecciones nuevas:** `[L-054]` (la premisa venía citada de `[L-045]` y `[L-043]`, correctas en todo lo demás, y nadie la recomprobó — `[L-034]` aplicado a una premisa, no a una cita), `[L-055]` (los tres punteros de línea de `[D-070]` se leyeron antes de editar los archivos y el propio commit los desplazó — el desfase de cada uno era exactamente cuántas líneas insertó el commit ahí, y el único correcto apuntaba al único archivo que el commit no tocó), `[L-056]` (el invariante del pool se rompe con un solo 504, demostrado sin ninguna concurrencia). **Test nuevo** `test_the_timeout_is_split_by_phase_and_the_parts_add_up_to_the_budget`, que vigila la SUMA de las cuatro fases, no que existan cuatro números — visto morder con `assert 26.0 == 8.0` sobre el one-liner descartado. **Hallazgos 3, 4 y 5 arreglados:** el `51 ms`/"cuatro corridas" desviado de `measure_local_parts.py` corregido a `56,3 ms`/cinco corridas; los tres punteros de línea muertos, tabulados; los `[A-011]` que quedaban citados como vigentes en `app/tools.py` (comentarios de `EFFORT`, `MAX_RETRIES` y el cliente por defecto) corregidos a apuntar a `[D-070]`. Comentarios corregidos en `app/api.py` (el del pool afirmaba que la cola nunca se forma) y `app/tools.py` (afirmaba un techo de 8,0 s que no existe). `constraints.md` revisado contra el diff — sin cambios, al día. `tasks.md`: `T-079` pasa de ✅ a **🔄 (desmarcada)** — el cierre que la sostenía se cayó; `T-092` reescrita — la cita a `[A-011]` en `measure_tutor.py` vuelve a ser cierta con la reapertura, pero apareció una inexactitud nueva y distinta (el docstring sigue diciendo `timeout=8.0` cuando el cliente interno ya se construye con `tools.TIMEOUT` repartido, desde el propio diff de hoy); `T-093` nueva, con lo que falta para volver a cerrar `[A-011]`. Suite: 426 → **428 passed**, confirmado corriendo `python -m pytest -q` en este cierre. Paso 2b confirmado en este cierre: `compilar: 0`, `comparar: 0` — ningún `.ts` en el diff | 8 |
| S-050 | 2026-08-13 | Trigésimo tercer tramo, cuarto cierre del 2026-08-13, después del cierre que dejó `[S-049]` (`9ffab80`). **`T-079` CERRADA con `[D-070]`:** `TUTOR_TIMEOUT_SECONDS` se queda en 10,0 s — ni se baja ni se retira — y `[A-011]` muere, la segunda vez y ahora sí (la primera, del 2026-08-11, se había reabierto: ver `[L-043]`). `[L-045]` había acotado por lectura la última rendija que quedaba abierta: que `respond()` fuera del modelo —`count_words` + `record_practice`, que escribe en disco con candado— se comiera más de 2 s de los 10 del presupuesto. Esa rendija se cronometró hoy con `measure_local_parts.py` **nuevo** en la raíz del repo: guion que NO llama a Anthropic (coste $0) y NO escribe en `data/` (usa una carpeta temporal). Cinco corridas: peor caso **56,3 ms**, con 40 hilos escribiendo sobre el **mismo** archivo — la contención se provoca quitando sitio, no trayendo carga (regla de `[L-045]`) — contra 2 000 ms de presupuesto: **35× de margen**. `tests/test_measure_local_parts.py` **nuevo**: un solo test que vigila que el `POOL_SIZE` de la báscula (duplicado a propósito, para no levantar la app entera importando `api`) siga igualado a `api.TUTOR_POOL_SIZE` — visto morder con sabotaje (bajarlo a 4 → rojo; restaurado → verde). 🔑 **El cierre se apoya en un TECHO IMPUESTO por el cliente (8,0 s), no en una medida que mañana se pueda superar** — es la diferencia exacta con el cierre fallido de `[L-043]`, que restaba de "la peor de diez" observaciones. Comentarios actualizados: `app/api.py` (el timeout deja de decirse "sigue siendo una predicción") y `app/tools.py` (se retira el puntero muerto a `T-079`, que ya no es la tarea que decide). `[A-011]` **RETIRADA** de `assumptions.md` (fila tachada en el índice apuntando a `[D-070]`, entrada borrada de `## Entradas`); `[D-070]` **nueva** en `decisions.md` (índice + entrada) con el contra de las dos alternativas descartadas (bajarlo, retirarlo) y lo que la decisión explícitamente NO dice (los 56 ms son de una máquina de desarrollo en Windows, no del servidor; la cola sigue sin cronometrarse, cubierta solo por el invariante y su test). Suite: **426 passed** (425 + el test nuevo), confirmado corriendo `.venv/Scripts/python.exe -m pytest -q` en este cierre. `assumptions.md` y `decisions.md` ya venían escritas por la sesión principal antes del cierre — revisadas contra el diff, al día. `constraints.md` y `lessons.md` sin cambios hoy — revisados, al día. `tasks.md`: `T-079` pasa a ✅ (índice y entrada nueva); `T-092` nueva (el pendiente de `measure_tutor.py`, ver abajo). Paso 2b confirmado en este cierre: `compilar: 0`, `comparar: 0` — ningún `.ts` en el diff. ⚠️ **Pendiente detectado hoy, no tocado (cerrar la sesión no es arreglar código):** `measure_tutor.py` sigue citando `[A-011]` como abierta, en su docstring (líneas 5, 8, 10, 15) y en un `print` (línea 259) — son punteros muertos desde hoy que `T-092` deja anotados | 8 |
| S-049 | 2026-08-13 | Trigésimo segundo tramo, tercer cierre del 2026-08-13, después del cierre que dejó `[S-048]` (`f4b73b7`). **Solo `_persistence/decisions.md`, `_persistence/lessons.md` y `deploy/README.md` en el diff (`e2044e8`)** — ningún `app/`, `tests/` ni `frontend/` tocado. **Respuesta a una auditoría externa** sobre `T-091`/`T-078`/`T-019`, que confirmó 5 de 7 afirmaciones **midiendo**: 425 passed sobre `f4b73b7`, `add_point`/`read_score` sin rastro en el código (sustituidos por `read_counters`/`record_practice` desde `[S-047]`), `[A-001]`/`[A-028]` solo como comentarios de defunción en `assumptions.md`, y el dominio sirviendo `id="practice"`. **`[L-052]` nueva:** la lección que `[A-001]` mandaba escribir ("si es falsa → `lessons.md`") no se escribió — al morir se fue entera a `decisions.md` (`[D-066]`), que es el destino de la otra rama de su propia regla; dice que un maniquí no solo tapa fallos: congela decisiones de diseño y las devuelve el día caro, y `[A-001]` predijo esa factura al pie de la letra once días antes. **`[L-053]` nueva:** `curl -s` que no resuelve devuelve cuerpo vacío, y un `grep` sobre el vacío dice "no está" en vez de "no medí" — fabricó un hallazgo contra un despliegue correcto y mordió a dos terminales el mismo día; `deploy/README.md` corregido para mirar el código de estado antes que el cuerpo. **`[D-069]` corregida:** mandaba a comprobar `{"score": 1, "practice": 3}` en un disco donde ya no está (cuenta desechable `probe-format`, borrada al acabar); ahora nombra las dos corridas (3 por guion, 2 desde el navegador), declara muerto ese archivo y apunta a `jorge.json` (`{"score": 1, "practice": 2}`) como único respaldo vivo — de camino se sincronizó su índice, que decía "tres llamadas" mientras el cuerpo ya decía "cinco". **Dos citas equivocadas detectadas y corregidas durante la propia ronda:** el traspaso a la auditoría atribuyó a `[D-066]`/`[D-069]` citas a `[L-017]` y `[A-002]` que no existen ahí — descritas de memoria, no sacadas con `grep`; y al escribir `[L-053]` se citó `[L-033]`, se abrió la entrada, no decía lo atribuido, y se quitó el corchete. `assumptions.md` y `constraints.md` revisados contra el diff — sin cambios en este commit, al día. `tasks.md`: ninguna tarea cambia de estado — la ronda fue documental, sobre trabajo ya cerrado (`T-091`, `T-078`, `T-019`). Suite: **425 passed**, confirmado corriendo `.venv/Scripts/python.exe -m pytest -q` en este cierre — mismo número que `[S-048]`, no se tocó código. Paso 2b confirmado en este cierre: `compilar: 0`, `comparar: 0` — ningún `.ts` en el diff | 8 |
| S-048 | 2026-08-13 | Trigésimo primer tramo, segundo tramo del 2026-08-13, después del cierre que dejó `[S-047]` (`76e9bee`). **Único archivo modificado: `_persistence/lessons.md`** — nada de `app/`, `tests/` ni `deploy/`; el trabajo de hoy es desplegar en la máquina real, no código. **`T-091` CERRADA:** en `/opt/teapp`, `git pull` (`699f2b2`→`76e9bee`), `rm -f data/users/*.json` (obligatorio por `[D-068]`: formato viejo sin la clave `practice` es incompatible), `systemctl restart teapp`. **Verificado contra la máquina viva, no deducido:** `https://teapp.duckdns.org/` devuelve `200`; la página servida trae `<span id="practice">`; `app.js` servido contiene `practiceBox`; y una práctica real desde el navegador contra el servidor: frase incorrecta → `Words: 4 · Score: 1` — el `Score` no subió con la frase mala. **`[L-051]` nueva, escrita por la sesión principal:** el navegador mostraba la pantalla vieja cacheada (sin `Practice`) mientras los números que llegaban ya eran frescos — la mezcla engaña más que un caché normal, porque el `Score` que se veía era el correcto; se diagnosticó pidiéndole al servidor con `curl` en vez de mirar lo que pinta el navegador, y se confirmó en ventana de incógnito. De camino, se corrigió una cita equivocada a `[L-007]` detectada al escribir esa lección. `decisions.md`, `assumptions.md` y `constraints.md` revisados contra el diff — sin cambios hoy, al día. `tasks.md`: `T-091` pasa a ✅ (índice y entrada nueva). Suite: **425 passed**, confirmado corriendo `.venv/Scripts/python.exe -m pytest -q` en este cierre — mismo número que `[S-047]`, no se tocó código. Paso 2b corrido en este cierre: `compilar: 0`, `comparar: 0` — sin `.ts` en el diff | 8 |
| S-047 | 2026-08-13 | Trigésimo tramo, después del cierre que dejó `[S-046]` (`811d436`). **`T-019` CERRADA con `[D-066]`–`[D-069]`:** el marcador pasó de contar frases practicadas a contar frases CORRECTAS, con un contador `practice` aparte para los intentos. `judge_grammar` (`app/tools.py`) cambia de contrato — devuelve `GrammarVerdict(correct, message)`, no `str` — y `split_verdict` nuevo lee la primera línea fija que pide la rúbrica (`OK`/`FIX`), la recorta, y **deniega por defecto** si el modelo se salta el formato (regla 3). `read_score`/`add_point` sustituidas por `read_counters`/`record_practice`, con `Counters(score, practice)` leído y escrito en una sola operación atómica — evita el hueco entre dos escrituras donde un fallo dejaría los contadores descuadrados. `read_counters` exige la clave `practice` igual que ya exigía `score`: un archivo viejo sin ella da `ScoreFileError` legible, no un contador mudo a 0. `app/english_tutor.py`: las tres líneas de `TutorReply` que dependían del orden de evaluación de Python pasan a tres variables explícitas (`words`, `verdict`, `counters`), porque el veredicto ahora se usa dos veces y ya no cabe dentro de una sola llamada — el freno de `[D-050]` (no sube nada sin veredicto) se mantiene, solo que visible a simple vista. `app/api.py`, `frontend/app.ts`, `app/static/index.html` y `app/static/app.js` (recompilado) muestran las cuatro piezas: `verdict`, `words`, `score`, `practice`. `deploy/README.md` gana la sección "Actualizar una máquina que YA está montada", con el aviso de que `[D-068]` exige borrar `data/users/*.json` antes de reiniciar. **`[A-001]` (2026-08-02) resultó FALSA** — la prueba que pedía corrió sin buscarla: `I cooking in these morning` estaba mal y el marcador subió igual — asciende a `[D-066]`, retirada de `assumptions.md`. **`[A-028]` (nacida hoy) resultó CIERTA** — Opus 5 sí pone la primera línea — asciende a `[D-069]`, con la corrida real: `I like coffee` → Score 1/Practice 1; `I cooking in these morning` → Score 1/Practice 2; `me likes coffees` → Score 1/Practice 3. **Confirmado también desde el navegador** por quien construye: frase correcta → Score 1/Practice 1; frase mala → Score 1/Practice 2, en disco `{"score": 1, "practice": 2}`. Marcadores viejos de `data/users/` **borrados en local** — formato incompatible, decidido en `[D-068]`; el servidor sigue con archivos del formato viejo (`T-091` nueva). Suite de 410 a **425 pasando**, confirmado corriendo `.venv/Scripts/python.exe -m pytest -q` en este cierre; `npx tsc --noEmit` sin errores. `decisions.md` y `assumptions.md` ya venían escritas por la sesión principal antes del cierre (`[D-066]`–`[D-069]`, retiro de `[A-001]`/`[A-028]`) — revisadas contra el diff, al día, sin faltantes. `constraints.md` y `lessons.md` sin cambios hoy — revisados, al día. `tasks.md`: `T-019` pasa a ✅ (índice y entrada nueva), `T-091` nueva. Paso 2b corrido en este cierre: `compilar: 0`, `comparar: 0` — el `.js` está al día con `frontend/app.ts` | 8 |
| S-046 | 2026-08-13 | Vigésimo noveno tramo, segundo tramo del 2026-08-13, después del cierre que dejó `[S-045]` (`699f2b2`). **Único archivo modificado: `_persistence/decisions.md`** — nada de `app/`, `tests/` ni `deploy/`, el código del paso 8 ya estaba commiteado desde el 12. **`T-087` CERRADA (13:57 UTC):** Anthropic dejó de saturar. `check_api_key.py` contra `teapp-server` dio **salida `0`**, `requests-limit=1000`. La sonda que declaró terminado el episodio de `529` fue un control al lado —la llave del laboratorio, que a las 13:55 volvió a dar `3`—, no un reintento a ciegas. Episodio medido: **entre 9 y 19 minutos** (10×`529` entre 13:36 y 13:46 UTC; limpio a las 13:57). **`T-078` CERRADA (14:04–14:08 UTC), en la máquina real — cierre del paso 8:** EC2 encendida desde la consola, `git pull` en `/opt/teapp` (`afe2eab`→`699f2b2`, 36 commits — entró todo el paso 8, no solo la llave), `install.sh` código `0` con el portero mordiendo ANTES de escribir (`requests-limit=1000, no es la del laboratorio` → `Escribiendo ANTHROPIC_API_KEY en el .env`), `.env` resultante con permisos `600` y la llave de `teapp-server` (108 caracteres), servicios `teapp`/`caddy` `active`. **🔑 Prueba PI-4, primera práctica real del proyecto:** desde el navegador, `I cooking in these morning` → *"Almost! Say: I cooked this morning. The verb needs a past form: cooking becomes cooked."* — Words: 5, Score: 9. Rastro del otro lado: `POST https://api.anthropic.com/v1/messages` → `200`, `POST /practice` → `200`, cuota `{"day": "2026-08-13", "used": 1}` (gastada, no devuelta — correcto, `[D-051]`), marcador `data/users/jorge.json` → `{"score": 9}`. **Cómo viajó la llave:** no por la línea de comandos — `stdin` → `read -r` → `export` → `sudo -E`, comprobado en la máquina que `sudo -E` preserva el entorno. ⚠️ El mensaje de error de `install.sh` sigue recomendando la forma insegura (`sudo ANTHROPIC_API_KEY=... bash …`, visible en `ps` y en el historial) — **`T-089` nueva**. **`[L-033]` mordió y estaba escrita:** dos `ssh` por nombre fallaron con `Could not resolve hostname` mientras `nslookup` y `socket.gethostbyname` resolvían al instante; se entró por la IP fija, como manda la lección — cuarto episodio del día, primero que le pega a `ssh` en vez de a `curl`. **Falsa alarma descartada, con el código leído:** `install.sh` muestra `curl: (7) Failed to connect to 127.0.0.1 port 8000` y sigue en verde — no es un guardián mudo, `install.sh:452-463` es un bucle de diez intentos con `exit 1` al final; lo que se vio fue el intento 1 quejándose mientras la app terminaba de levantar. `decisions.md` ampliado con este tramo dentro de `[D-063]` (el párrafo viejo tachado, no borrado) y `[D-065]` (ya existía de `[S-045]`); revisado contra el diff, al día. Los otros tres archivos del porqué (`assumptions.md`, `constraints.md`, `lessons.md`) revisados — sin faltantes nuevos que anotar hoy. `tasks.md`: `T-078` y `T-087` pasan a ✅; nuevas `T-089` (mensaje inseguro de `install.sh`) y `T-090` (valorar si el paso 8 cierra del todo). Suite **no se corrió hoy** — último número medido son 410 tests, sobre `32f3314`. Ningún `.ts`/`.js` en el diff; Paso 2b confirmado igual: `compilar: 0`, `comparar: 0` | 8 |
| S-045 | 2026-08-13 | Vigésimo octavo tramo, después del cierre que dejó `[S-044]` (`32f3314`). **Solo `_persistence/` en el diff — `decisions.md`, `lessons.md`, `progress.md`.** **Condición de cierre de `[D-063]` CUMPLIDA:** `deploy/check_api_key.py` corrió por primera vez contra la red real, dos llaves con identidad leída en consola ANTES de correr — corrida A, llave de `teapp-measure`, salida `3`, `requests-limit=50`; corrida B, llave de `Default`, salida `0`, `requests-limit=1000`. `[D-063]` ampliada con la tabla, dos hipótesis muertas por el camino (la llave de `Default` NO estaba en el `.env` del curso; `Default` tiene tres llaves, ninguna recuperable) y la regla 7 enmendada: la corrida imprimió el prefijo `sk-ant-` y los cuatro caracteres finales de cada llave, así que la regla pasa de "ni un prefijo" a "cuatro caracteres finales, solo en terminal, jamás en el repositorio". **`T-078` NO se cierra:** la condición que se cumplió es la de `[D-063]` (el portero distingue), no la de `T-078` (que la llave llegue al servidor) — `install.sh` no ha corrido en la máquina. **`[A-027]` nació y murió el mismo día:** se escribió como suposición ("algo más usa la llave de `Default`") y se comprobó FALSA — es el repositorio del curso, 21 archivos `.py` cargan ese `.env` en 8 niveles (`00-setup`…`06b-memoria-skills`, excluyendo `.venv`). Ascendió a `[D-065]`: se crea `teapp-server`, con nombre propio, ANTES de correr `install.sh` — el orden lo impone `install.sh:89-95`, que nunca pisa una llave ya escrita. `assumptions.md` no cambia en el diff porque la suposición nació y murió sin llegar a escribirse ahí. **`teapp-server` creada por el usuario en `Default`.** Su comprobación contra la red dio 10×`529` entre 13:36 y 13:46 UTC — un control al lado (la llave del laboratorio, que 20 min antes había contestado `3`) confirmó que era saturación de Anthropic, no la llave nueva. `[L-046]` ampliada con el segundo episodio. **`[L-050]` nueva, hallazgo de lectura (PI-3, no se toca hoy):** `check_api_key.py:62-63` dice que el modelo "da igual" y es falso — los frenos van por modelo, el 50 es la firma del laboratorio para `claude-opus-5`; cambiar `MODEL` en el paso 9 deja al portero mudo, aceptando la llave del laboratorio sin abortar. Tercera pata de `[L-047]`. **Corrección de arrastre, `[L-049]` nueva:** el arranque presentó `T-074` como pendiente y urgente — está CERRADA desde el 2026-08-10 (`tasks.md:81`, `[S-034]`). Segundo día seguido que viajaba viva; se borró del campo *siguiente acción* de `progress.md` y de la cola de `[S-044]`, con nota en los dos. Suite: **no se corrió hoy** — el último número medido son 410 tests, sobre `32f3314`. Los cuatro archivos del porqué revisados contra el diff — al día, sin faltantes. Ningún `.ts`/`.js` tocado; Paso 2b confirmado igualmente: `compilar: 0`, `comparar: 0` | 8 |
| S-044 | 2026-08-12 | Vigésimo séptimo tramo, después del cierre que dejó `[S-043]` (`196e3eb`). Tres commits: `c071530` (solo `_persistence/`, decisiones), `d4c40eb` (código+tests de `T-078`), `be30bd4` (solo `_persistence/`, `D-064`). **`T-085` CERRADA con `[D-062]`:** tope de gasto de $2,00/mes al espacio `teapp-measure`, verificado en pantalla (*"Límite Mensual: USD 0,00 de USD 2,00"*). Aritmética: techo físico de la báscula 35 llamadas/min × $0,00234/llamada = $0,082/min; $6,48 ÷ 0,082 ≈ 79 minutos para vaciar el saldo, contra una ventana ciega de 120 min del tope de gasto (`[A-025]`, comprobada en pantalla y salió MUDA — ni `soft`, ni umbral, ni retraso documentado en la consola de primera parte). La entrada se rectificó el mismo día: decía cortar el flanco de las 26 corridas seguidas y el propio número al lado (26 × 106 × 1,72 s ≈ 79 min) lo desmentía — ese flanco **no** queda cortado. `[A-026]` nueva: correr el guion muchas veces SEGUIDAS sigue sin dueño, ni una capa lo cubre. **`T-078` implementada, sin cerrar:** `deploy/check_api_key.py` nuevo — pregunta a Anthropic de quién es la llave leyendo `anthropic-ratelimit-requests-limit`, aborta si vale 50 (firma del laboratorio, `[D-061]`), cuatro puertas de salida (0 bien, 1 sin llave, 3 laboratorio, 4 sin comprobar) para que un 529 o un corte de red no se disfracen de llave equivocada (`[L-046]`). `deploy/install.sh`: la llave entra por variable de entorno nunca por argumento, sin llave el guion no termina en verde, una llave ya escrita no se pisa, la comprobación va en la sección 2b antes de la única sección que escribe. `tests/test_check_api_key.py` nuevo, 15 tests, ninguno toca la red. Tres sabotajes: los dos primeros salieron rojos; el tercero —mover la comprobación detrás de la escritura— pasó en VERDE porque el test vigilaba la primera línea que nombrara el archivo, y esa línea era un comentario, no la llamada; corregido a líneas activas y sabotaje repetido, ahora sí muerde (`[L-048]`). `[D-063]` deja escrita la condición de cierre de `T-078`, para no confiarla a la memoria: ver morder DOS puertas con DOS llaves reales — la 3 con la del laboratorio y la 0 con la de `Default`; con una sola llave el 3 no distingue "la identificó" de "acertó por casualidad" (misma forma que `T-060b`). **`ask_anthropic` no se ha ejecutado nunca contra la red real — por eso `T-078` NO se cierra**, aunque el código esté escrito y probado. `[D-064]` nueva: la terminal que audita también puede correr `pytest -q`, disparador en presente ("cuando vaya a escribir o citar un número"), no en futuro ("cuando el número sostenga una decisión") — un disparador que exige adivinar se resuelve siempre del lado cómodo. `L-047` nueva: un acoplamiento (el `50` vive ahora en dos sitios, `[D-061]` y `[D-063]`) se anota donde va a mirar quien lo rompa, no donde lo entendió quien lo creó. **Suite de 395 a 410 pasando, confirmado corriendo `python -m pytest -q` en este cierre, sobre `be30bd4`.** Los cuatro archivos del porqué (`decisions.md`, `assumptions.md`, `constraints.md`, `lessons.md`) revisados en este cierre contra los tres commits — al día, sin faltantes; nada para ascender. Ningún `.ts`/`.js` en el diff de los tres commits del día (solo `_persistence/`, `deploy/install.sh`, `deploy/check_api_key.py`, `tests/`); Paso 2b no aplica. ⚠️ **Sigue pendiente sin tocar hoy:** `T-086` (hora de las lecturas de costo de AWS) y `T-079` (a medias, reformulada desde `[S-043]`). ✏️ **Corregido el 2026-08-13:** esta cola nombraba también `T-074` como pendiente, y era falso ya el día que se escribió — `T-074` se cerró el 2026-08-10 (`tasks.md:81`, `[S-034]`). Ver `[L-049]` | 8 |
| S-043 | 2026-08-12 | Vigésimo sexto tramo, después del cierre que dejó `[S-042]` (`89d00fd`). **`T-084` CERRADA con `[D-061]`:** espacio de trabajo `teapp-measure` creado en la consola de Anthropic, con freno de velocidad para `claude-opus-5` en `50/20.000/5.000` (peticiones, tokens de entrada, tokens de salida por minuto) y los límites de lotes y búsqueda web bajados a 1. Los números salen de `measure_tutor.py:208` (llamadas secuenciales) y de la llamada más rápida de `[A-011]` (1,72 s ⇒ techo físico 35/min), con ~1,4× de holgura. La regla 6 mordió en vivo: la documentación general decía que `Default` hereda de Anthropic `2.000.000`/`400.000` de tokens para Opus 5, pero la consola de esta cuenta contestó `1.000/500.000/80.000` — ganó la consola. La llave nueva vive en el `.env` local bajo el mismo nombre `ANTHROPIC_API_KEY`, verificada con una llamada mínima que devolvió `requests-limit: 50`, `input-tokens-limit: 20000`, `output-tokens-limit: 5000`, `requests-remaining: 49` en las cabeceras — costó 10 tokens de entrada y 4 de salida. `[D-061]` deja escrito y sin decidir si además hace falta un tope de GASTO por espacio de trabajo, contra el saldo real de $6,55; esa decisión pasa a `T-085` nueva, y va antes de `T-078` porque `T-078` es lo que le da al saldo compartido un segundo consumidor sin freno propio (hoy solo la báscula tiene `CallBudget`). **`[L-046]` nueva:** verificar la llave nueva produjo nueve `529 Overloaded` seguidos de `claude-opus-5` en ~50 s — saturación real de Anthropic, no un fallo del proyecto; con `MAX_RETRIES = 0` cada intento así se cobra de la cuota de 20 sin devolver veredicto, y los 529 no traen cabeceras `anthropic-ratelimit-*` para diagnosticar. **`[L-045]` nueva:** el plan pendiente de `T-079` —lanzar 23 peticiones a la vez para forzar cola— es un fósil: el 23 se midió contra un pool de 20 que ya no existe (`TUTOR_POOL_SIZE = 40` hoy), así que 23 peticiones entrarían todas sin cola y no medirían nada; y con el cliente cortando a 8,0 s por debajo de los 10 s de la ruta, el timeout no puede disparar ni por cola ni por modelo lento. `T-079` reformulada: la mitad que falta ya no es cronometrar, es decidir qué hacer con ese timeout. `decisions.md` y `lessons.md` escritas por la sesión principal en el momento, revisadas en este cierre — al día, sin faltantes. Ningún `.ts` ni `.js` en el diff de los cuatro commits del día (solo `_persistence/decisions.md` y `_persistence/lessons.md`); Paso 2b no aplica. ⚠️ **Pendiente sin resolver:** la lectura de AWS de hoy (`Costo Acumulado Mensual` = 1,12 US$, según traspaso) no tiene hora anotada, igual que una lectura del día 11 — ninguna de las dos está en `[A-018]`; queda `T-086` nueva | 8 |
| S-042 | 2026-08-11 | Vigésimo quinto tramo, segundo tramo del 2026-08-11 después del cierre que dejó `[S-041]` (`5815132`). **`T-082` CERRADA:** decidido con `[D-059]` cómo se separan MEDIR y SERVIR — dos capas, no una: (1) corte duro dentro de `measure_tutor.py`, que protege el saldo; (2) espacio de trabajo propio para medir, con su llave y su límite de velocidad, que separa la llave, la velocidad y la contabilidad. Se leyó la documentación de Anthropic antes de decidir: los espacios de trabajo sí admiten tope de gasto propio, pero es reparto del mismo techo de la organización, no un bolsillo aparte (*"Organization-wide limits always apply, even if workspace limits add up to more"*); el espacio por defecto —donde vive la llave de hoy— no admite ningún tope. Se descartó también Claude Platform on AWS por la regla 5: factura en CCUs a mes vencido, *"There is no CCU balance"* — cambiar un techo duro por una cuenta abierta. La decisión NO desbloqueaba `T-078` por sí sola —una decisión no frena un bucle—, así que quedó condición escrita: la capa 1 tenía que existir y verse morder. **`T-083` NUEVA Y CERRADA el mismo día con `[D-060]`, cumpliendo esa condición:** el tope de `measure_tutor.py` ahora sale del saldo, no del historial — `$0,25` por tanda ÷ `$0,00234` por llamada (medido en `[D-058]`) = **106 llamadas**, con la división escrita en el código. `CallBudget` nuevo: cobra ANTES de llamar (si cobrara después, la llamada que rebasa el tope ya se habría pagado) y vive dentro de `RecordingClient`, el paso obligado de toda llamada del guion — puesto más arriba dejaría escapar cualquier camino que no pase por el bucle. **Hallazgo de camino, `[L-044]`:** el `MAX_CALLS = 10` que el archivo ya traía no salía de ninguna medición — salía de `len(SENTENCES)`, que tiene exactamente diez frases; circuló tres veces (constante, "tope" con comentario citando tres decisiones, y argumento hablado en esta misma sesión) con aspecto de medido sin serlo. Y lo que lo hacía cumplir tampoco frenaba: `SENTENCES[:MAX_CALLS]` con tope 10 y lista de 10 no corta nada. 8 tests nuevos en `tests/test_measure_tutor.py`, tres sabotajes vistos en ROJO: cobrar después de llamar (`assert 3 == 2`), contador por cliente en vez de compartido (`DID NOT RAISE` — cada `RecordingClient` nuevo lo reiniciaría a cero sin que nadie lo notara), y `>` en vez de `>=` (`DID NOT RAISE` en cuatro tests). Suite de 387 a **395 pasando**, confirmado corriendo `python -m pytest -q` en este cierre. **`[C-008]` cerrada A MEDIAS, y las dos mitades se separan a propósito:** el fallo MUDO queda tapado —un bucle roto ya no puede vaciar el saldo en una corrida—, pero la partición en sí sigue sin existir: el saldo de la organización no se puede partir, y hoy medir y servir además comparten la llave, porque la del espacio de medir todavía no existe (`T-084`). El freno tiene su propio hueco medido y escrito: `$6,55 ÷ $0,25 = 26 corridas` a mano vacían el saldo igual — deliberado, no un olvido: el corte por corrida mata el fallo mudo, que era el objetivo. **`T-078` cambia de bloqueante:** ya no lo bloquea `T-082` (cerrada) sino `T-084` — la capa 2 nace ahí, y si `T-078` corriera antes, la única llave que hay viajaría al servidor y medir/servir compartirían también la llave. `T-084` nueva: acción del usuario en el navegador, crear el espacio de trabajo de medir. Paso 2b: `.js` compilado, al día (`compilar: 0`, `comparar: 0`) — no había ningún `.ts` en el diff | 8 |
| S-041 | 2026-08-11 | Vigésimo cuarto tramo, después del cierre que dejó `[S-040]` (`1365ed1`) — mismo día, segundo tramo. **Primera ronda de auditoría externa**, dos hallazgos comprobados en el código y anotados sin arreglar nada: `[C-008]` nueva — medir y servir salen del mismo saldo de Anthropic; hoy inofensivo porque el servidor tiene `ANTHROPIC_API_KEY` vacía, pero `T-078` quita esa protección, y después de `T-078` agotar el saldo midiendo deja la app viva dando 503 **mudo** (el camino nuevo devuelve la cuota y no toca el marcador); se decide antes de `T-078` (`T-082` nueva). `[L-042]` nueva — el camino del 504 decide la devolución con `future.cancel()` (¿arrancó la tarea?, no ¿se facturó?); ventana medida en las constantes, cliente 8,0 s contra ruta 10,0 s → bastan 2 s de cola; la auditoría acertó el bicho y se pasó en el arreglo (en el 504 `request_sent` aún no existe, arreglarlo pide `add_done_callback`, no una línea) — sin arreglar a propósito. **`T-079` corrida, primer gasto real del proyecto:** `measure_tutor.py` nuevo en la raíz, diez llamadas a `claude-opus-5` vía `judge_grammar` con el cliente de producción, tope duro de 10, `for` sobre lista acotada. Medido: `judge_grammar` 1,72 s / 3,33 s mediana / **4,72 s la peor de diez**; 247,2 tokens de entrada y 44,3 de salida por práctica. `[L-001]` mordió por tercera vez: emoji en el resumen final, `cp1252`, `UnicodeEncodeError` **después** de las diez llamadas — los números se salvaron por estar ya impresos; aviso añadido encima de `main`. **`[A-010]` CERRADA, ascendida a `[D-058]`:** el usuario leyó la consola — **$0,02** por las diez llamadas — cruzado con el precio de lista oficial de Opus 5 (leído con WebFetch, no de memoria) sobre los tokens medidos: **$0,0234**, coinciden dentro del redondeo. ⇒ $0,00234/práctica, $0,047/día, **$8,44 en 180 días** por persona a tope. 🚨 **El saldo de $6,55 no cubre a una persona a tope en la ventana: se acaba a los 140 días.** El tope de 20 no se toca — frena el abuso, no describe el uso. **`[A-011]` RETIRADA y REABIERTA el mismo día**, por una segunda ronda de auditoría — el error más importante del tramo: se cerró midiendo un reloj que no es el suyo. `measure_tutor.py` cronometra solo `judge_grammar`; `TUTOR_TIMEOUT_SECONDS = 10.0` cronometra la cola del pool más `respond()` entero. Los 4,72 s son uno de tres trozos; "5,28 s de margen" restaba de un presupuesto que la báscula no cronometró. Vuelve a `assumptions.md` como **encogida**; `[L-043]` corregida en índice y entrada, `measure_tutor.py` corregido en docstring y salida. **Reencuadre que se queda:** el timeout del cliente (8,0 s) mide un subconjunto de lo que mide el de la ruta (10 s) y además es más pequeño ⇒ en una llamada sin cola el de la ruta no puede disparar nunca — los 10 s jamás protegieron de un modelo lento, solo pueden frenar la cola; respalda no tocar el 8,0. **Lección de método en `[L-043]`:** la salvedad "no dice nada de la cola llena" ya estaba escrita cuando la entrada tituló "A-011 muere" — una salvedad en el párrafo no arregla un titular falso, el párrafo no se relee, la tabla sí. Suite de 387 (sin cambio de número, ya estaba en 387 desde `[S-040]`) confirmada de nuevo: **387 passed**, corriendo `python -m pytest -q` en este cierre. ⚠️ **`T-079` queda A MEDIAS (`T-082` nueva también):** la mitad de `[A-010]` está hecha, la de `[A-011]` (cronometrar `/practice` entera con el pool cargado, aparejo de `[L-013]`, 23 peticiones a la vez) no. Nada se afirma sobre la máquina de producción de AWS — no se tocó; la única medida externa (timeout, no RST) la hizo la terminal auditora, no este proyecto. Paso 2b: `.js` compilado, al día (`compilar: 0`, `comparar: 0`) — ningún `.ts` en el diff | 8 |
| S-040 | 2026-08-11 | Vigésimo tercer tramo, después del cierre que dejó `[S-039]`. **`T-080` CERRADA** (acción del usuario en la consola de Anthropic): saldo prepagado de 6,55 US$, recarga automática DESACTIVADA, límite de gasto mensual de 500 US$ puesto por Anthropic y ajustable. `[A-024]` era **falsa** — retirada de `assumptions.md`, vive en `[D-057]` nueva, rectificada el mismo día por decisión del usuario (el límite se queda en 500, no baja a 10: el saldo muerde muchísimo antes, bajar el límite hoy no protegía de nada; queda el disparador escrito para el día que se recargue saldo, cuando el 500 pasa a ser el único freno vivo). **`T-076` CERRADA:** `app/api.py` no cazaba `TutorUnavailableError`; ahora la importa, tiene `TUTOR_UNAVAILABLE_MESSAGE`, y un `except` que devuelve 503 y llama a `quota.refund(user)` solo si `not error.request_sent` (`[D-051]`). Seis tests nuevos en `tests/test_api.py`. Dos guardianes vistos en ROJO por sabotaje, cada uno tumbando exactamente un test: `if False` (nunca devuelve) tumbó `test_a_practice_whose_request_never_left_gets_the_quota_back`; `if True` (devuelve siempre) tumbó `test_a_practice_whose_request_did_leave_keeps_the_quota_spent`. **`T-077` CERRADA:** `FAKE_VERDICT` ya no existía (se borró en `[S-037]`); quedaban comentarios en seis archivos (`app/api.py`, `app/quota.py`, `app/tools.py`, `tests/test_api.py`, `tests/test_english_tutor.py`) diciendo que el tutor seguía siendo falso, desde `[S-039]`. Corregidos. El docstring de `app/english_tutor.py` se REESCRIBIÓ con precisión, no se borró: se verificó en `_context/scope.md` que el agente no elige herramientas a propósito, así que la secuencia fija es encargo, no deuda; se añadió también la nota que faltaba en las tres líneas de `TutorReply`, donde el orden sostiene `[D-050]`. **Corrida en vivo (PI-4), coste cero:** app local levantada con una llave de API inválida a propósito, carpeta de datos temporal, puerto 8099. `/practice` → **503** con el mensaje correcto; archivo de cuota en `{"day": "2026-08-11", "used": 0}` (gastada y devuelta); el archivo del marcador **ni se creó**; log: `El tutor no esta disponible (usuario prueba, la peticion salio: no)` con el 401 real de Anthropic. Servidor apagado después. **`[L-041]` nueva:** el log de esa corrida trae un `request_id` de Anthropic en la misma línea que dice `salio: no` — la petición SÍ salió, y aun así `request_sent=False` y la cuota se devuelve, correctamente, porque el campo decide "¿se facturaron tokens?", no "¿salió el paquete?". No se renombró hoy a propósito (PI-3: viaja por dos módulos, siete tests y cuatro decisiones); queda como `T-081` nueva. Suite de 381 a **387 pasando**, confirmado corriendo `python -m pytest -q` en este cierre. Quedan del paso 8: `T-078` (que `ANTHROPIC_API_KEY` llegue al servidor por `install.sh`) y `T-079` (medir `[A-010]`/`[A-011]` con el modelo real — ya NO bloqueada, el freno vivo es el saldo). ⚠️ **La máquina de producción de AWS no se tocó en esta sesión** — nada se afirma sobre su estado. Paso 2b: `.js` compilado, al día (`compilar: 0`, `comparar: 0`) — ningún `.ts` tocado, no había frontend en el diff | 8 |
| S-039 | 2026-08-10 | Vigésimo segundo tramo, después del cierre que dejó `[S-038]`. Una segunda auditoría externa del mismo día corrigió el proxy que `[D-054]` había dejado en `app/tools.py`: decidir con `stop_reason == "refusal"` **y** `content` vacío tenía un agujero comprobado en la documentación de Anthropic — sin streaming (como llama `judge_grammar`), un rechazo a MITAD omite el parcial, y esa respuesta llega calcada por fuera al rechazo gratis con los tokens ya pagados. `[D-055]` corrige: la rama decide ahora con `answer.usage.input_tokens > 0 or answer.usage.output_tokens > 0`, el contador que trae la propia respuesta. `tests/fake_tutor.py` gana `FakeUsage` y el campo `usage` en `FakeAnswer` (defecto **facturado**, regla 3 aplicada al valor por defecto) y el constructor `refusing_mid_output_without_partial()`. `tests/test_tools.py` gana `test_a_billed_refusal_with_no_partial_still_charges`, verificado en ROJO por sabotaje (reponiendo el proxy de `[D-054]`, el test falla con cuota regalada, y solo ese). Suite de 380 a **381 pasando**, confirmado en este cierre. `[D-055]` y `[L-040]` ya estaban escritas por la sesión principal antes del cierre; revisadas, no reescritas. 🚨 **`app/api.py` sigue sin tocar:** toda la maquinaria de `request_sent` existe y está probada, pero hoy nadie la usa — la cuota no se devuelve nunca. Paso 2b: `.js` compilado, al día (`compilar: 0`, `comparar: 0`) — no había ningún `.ts` tocado. 📎 **AMPLIADA el mismo día, después del commit `8d335fd`:** llegaron tres hallazgos más de una terminal auditora, ya escritos por la sesión principal antes de este segundo cierre — no código, solo `_persistence/`. **`[D-056]` nueva:** escalera fija para consultar documentación — `ctx7` primero, la página suelta después, la skill `claude-api` entera solo como último recurso y diciéndolo — medida en que una sola consulta de la skill llevó la sesión de 55K a ~340K tokens (vuelca ~30 documentos cuando TEAPP hace una sola llamada); no afloja la regla 6. **`[L-040]` CORREGIDA:** la entrada había declarado falsa la cita a `[L-036]`; era correcta — `[L-036]` líneas 334-335 dice literalmente la misma regla del contador/reloj propio del instrumento. El error nació de leer 13 de 119 líneas de la entrada y juzgar por el encabezado; la entrada ahora cuenta las dos capas (el proxy que confundía dos respuestas, y la corrección que se equivocó peor) con la regla nueva "abrir la entrada no es leerla: se busca la frase, no se ojea el principio". **`[A-024]` nueva:** se da por cierto que la llave de la API de Anthropic (`T-075`) no tiene tope de gasto — sin comprobar en la consola de Anthropic; bloqueante de `T-079` (la tarea que llama en bucle). Separa cuatro bolsillos que no se mezclan: AWS (`[A-018]`, con alarma), la llave de Anthropic (sin freno conocido) y la suscripción de Claude Code (`claude.ai/settings/usage`, el taller y no la obra — el aviso de límite mensual que cortó un intento de cierre hoy no entra en `[A-018]`/`[T-067]`/el presupuesto del proyecto). **`T-080` nueva** en `tasks.md`. Suite corrida de nuevo en esta ampliación: **381 passed**. Paso 2b vuelto a correr: `.js` compilado, al día (`compilar: 0`, `comparar: 0`) — solo `_persistence/` en el diff, ningún `.ts` tocado | 8 |
| S-038 | 2026-08-10 | Vigésimo primer tramo, después del cierre que dejó `[S-037]`. Dos arreglos en `app/tools.py` salidos de una auditoría externa, comprobados contra la documentación del SDK (`[D-054]`): `TIMEOUT_SECONDS = 8.0` en el constructor del cliente (el defecto del SDK son 10 min, y el aviso de que hacía falta estaba escrito en `app/api.py:130` desde el 4 de agosto sin ejecutarse); y `request_sent=not refused_before_output` en la rama del veredicto vacío, exigiendo `stop_reason == "refusal"` **y** `content` vacío — corrige la regla más corta del auditor, que habría devuelto cuota de más en un rechazo a mitad de respuesta. Los tres archivos de tests rotos desde `[S-037]` (`FAKE_VERDICT` borrado) quedaron arreglados: `tests/fake_tutor.py` nuevo (veredicto de mentira, `FakeClient`, excepciones del SDK fabricadas) y un maniquí `autouse` en `conftest.py`, porque más de 40 tests de `test_api.py` pasan por `/practice`. Suite de 362 (última corrida conocida) a **380 pasando**, confirmado en este cierre. Cinco guardianes nuevos vistos en ROJO por sabotaje: quitar el timeout, cobrar siempre el veredicto vacío, la regla corta del auditor, reordenar los `except` (`APITimeoutError` hereda de `APIConnectionError`), y reordenar las tres líneas de `TutorReply` en `app/english_tutor.py:53` (`[D-050]`). `[L-039]` nueva: el guion de sabotaje reescribió archivos con `Path.write_text()` y Windows convirtió LF a CRLF — contenido idéntico, bytes distintos, `git status` modificado con `git diff` vacío; ensuciaba el diff del día (PI-3), reparado. 🚨 **`app/api.py` sigue sin tocar:** toda la maquinaria de `request_sent` existe y está probada, pero hoy nadie la usa — la cuota no se devuelve nunca. Paso 2b: `.js` compilado, al día (`compilar: 0`, `comparar: 0`) — no había ningún `.ts` tocado | 8 |
...
### [S-052] 2026-08-13 — Segunda vuelta de la auditoría del día: `[D-072]` corrige una regresión viva de `[D-071]`

- **Paso:** 8 de 9. Trigésimo quinto tramo, sexto cierre del día, después del
  cierre que dejó `[S-051]` (`3463f2c`). Responde a la **segunda** vuelta de
  la auditoría externa sobre la misma ronda del hallazgo grave de `[S-051]`:
  tres hallazgos más, los tres correctos.
- **Hallazgo A (grave — regresión viva, no hipotética):** `[D-071]`, escrito
  dos horas antes, había puesto `read = 4,0` en el reparto por fases —
  **por debajo** de los 4,72 s ya medidos en `[L-043]` (n=10). La causa: se
  creyó que `read` cronometraba "entre el primer byte y el último". Falso, y
  comprobado en la fuente instalada, no recordado:
  `httpcore/_sync/http11.py::_receive_response_headers` usa el reloj `read`,
  y sin streaming Anthropic no manda un byte hasta terminar de generar —
  **`read` cronometra la generación entera**. El modo de fallo cobra:
  `APITimeoutError` → `request_sent=True` → `[D-051]` factura la práctica.
  **Arreglado en `[D-072]`:** `connect 1,5 + write 0,5 + read 6,5 + pool
  0,5 = 9,0`, `TIMEOUT_SECONDS` sube de 8,0 a 9,0 (sigue < 10,0 de la ruta).
  El reparto ya no es simétrico a propósito: lo liberado de `connect`,
  `write` y `pool` —sobrefinanciados— va todo a `read`, la única fase donde
  se tarda de verdad.
- **Hallazgo B:** `measure_tutor.py` heredaba `tools.TIMEOUT`, así que toda
  llamada por encima del `read` de producción dejaba de ser muestra y pasaba
  a ser error — la cola de la distribución, justo lo que hace falta para
  colocar el tope, era lo invisible. Arreglado con
  `MEASURING_READ_SECONDS = 30.0` y una excepción escrita a `[L-043]`: la
  báscula es idéntica a producción en todo menos en el tope que intenta
  medir. Lección nueva `[L-057]`.
- **Hallazgo C:** la salida de `measure_local_parts.py` seguía hablando de
  "TECHO DEL MODELO" y "MARGEN QUE QUEDA" —conceptos que `[D-070]` ya había
  retirado— y `POOL_SIZE` seguía duplicado a mano. Arreglado: la salida ya
  no resta márgenes ni habla de techos, y `POOL_SIZE` se lee de
  `api.TUTOR_POOL_SIZE`. Eso dejó sin objeto a
  `tests/test_measure_local_parts.py` (que vigilaba ese duplicado) — **se
  borró**, deliberado, no una pérdida de cobertura.
- **Dato nuevo salido al correr la báscula:** el peor caso local sube en
  cada tanda — 44,9 / 45,9 / 49,2 / 50,6 / 56,3 / 62,4 ms en seis corridas.
  Anotado en el propio guion: "el peor de N" no es un techo, es un suelo
  que crece con N.
- **Suite: 427 tests passed** (eran 428; baja uno por el archivo borrado en
  el Hallazgo C, deliberado).
- **`.js` compilado:** al día — ningún `.ts` se tocó en esta ronda.
