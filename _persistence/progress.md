...
| | |
|---|---|
| **paso** | 8 de 9 — Trigésimo quinto tramo (`[S-052]`), sexto cierre del 2026-08-13, después del cierre que dejó `[S-051]` (`3463f2c`). **Segunda vuelta de la misma auditoría del día: tres hallazgos más, los tres correctos.** El grave (A): `[D-071]`, escrito dos horas antes, había puesto `read=4,0` — por DEBAJO de los 4,72 s ya medidos en `[L-043]` (n=10), y con el modo de fallo cobrando la práctica (`APITimeoutError` → `request_sent=True` → `[D-051]`). Causa comprobada en la fuente instalada: `httpcore/_sync/http11.py::_receive_response_headers` usa el reloj `read`, y sin streaming Anthropic no manda un byte hasta terminar de generar — `read` cronometra la generación entera, no "entre el primer byte y el último". Arreglado en `[D-072]`: `connect 1,5 + write 0,5 + read 6,5 + pool 0,5 = 9,0`, `TIMEOUT_SECONDS` sube a 9,0. Hallazgo B: `measure_tutor.py` heredaba `tools.TIMEOUT`, así que la cola de la distribución —lo único que hace falta para colocar el tope— se convertía en error y desaparecía; arreglado con `MEASURING_READ_SECONDS = 30,0` y `[L-057]` (excepción escrita a `[L-043]`: la báscula es idéntica a producción en todo menos en el tope que mide). Hallazgo C: `measure_local_parts.py` seguía hablando de "TECHO" y restando márgenes que `[D-070]` ya había retirado, y duplicaba a mano `POOL_SIZE`; arreglado, y `POOL_SIZE` ahora se lee de `api.TUTOR_POOL_SIZE`, lo que dejó sin objeto a `tests/test_measure_local_parts.py` (borrado, deliberado). Dato nuevo: el peor caso local sube en cada tanda (44,9→62,4 ms en seis corridas) — "el peor de N" es un suelo que crece, no un techoran correctos, y `T-079` vuelve a estar a medias.** El grave: `[D-070]` había cerrado `[A-011]` afirmando que el cliente "corta a los 8,0 s pase lo que pase" — falso, medido: `httpx` reparte `timeout=8.0` a cuatro fases (`connect`/`read`/`write`/`pool`) con cronómetro independiente que **suman 32 s**, comprobado con un comando de una línea. `[A-011]` REABIERTA (segunda vez), `[D-070]` ENMENDADA (no borrada, con tabla de qué sobrevive y el texto original debajo). `[D-071]` NUEVA: el presupuesto se reparte fase por fase (`connect 2,0 + write 1,0 + read 4,0 + pool 1,0 = 8,0`); el arreglo de una línea que proponía la auditoría (`Timeout(8.0, connect=2.0)`) **no servía, suma 26 s** — comprobado antes de aplicarlo. Contradicción de `[D-070]` resuelta con una corrida: la carga falsa era "la cola no se forma por construcción" — un 504 suelta la ficha de `anyio` pero no el sitio del pool, medido con `test_a_timed_out_tutor_keeps_its_pool_seat_with_nobody_waiting` (peticiones **secuenciales**, nunca dos vivas a la vez, se ve morder con `max_workers=3`). Tres lecciones nuevas: `[L-054]` (la premisa heredada de `[L-045]`/`[L-043]` sin recomprobar), `[L-055]` (punteros de línea leídos antes de editar, el propio commit los desplazó), `[L-056]` (un 504 rompe el invariante del pool, demostrado sin concurrencia). Test nuevo `test_the_timeout_is_split_by_phase_and_the_parts_add_up_to_the_budget` (vigila la SUMA, visto morder con `26.0 == 8.0`). Corregidos también el `51 ms` desviado de `measure_local_parts.py`, los punteros de línea, y los `[A-011]` que seguían en `app/tools.py`. Comentarios corregidos en `app/api.py` y `app/tools.py`. Suite: 426 → **428 passed**, confirmado corriendo `python -m pytest -q` en este cierre. Paso 2b confirmado: `compilar: 0`, `comparar: 0` — ningún `.ts` en el diff. `tasks.md`: `T-079` **DESMARCADA** de ✅ a 🔄 (el cierre que la sostenía se cayó); `T-092` reescrita (la cita a `[A-011]` en `measure_tutor.py` vuelve a ser cierta, pero apareció una inexactitud nueva: el docstring sigue diciendo `timeout=8.0` cuando el cliente ya usa `tools.TIMEOUT` repartido); `T-093` nueva (lo que falta para cerrar `[A-011]` de verdad, listado dentro de su propia entrada) |
| **última sesión** | 2026-08-13 (trigésimo quinto tramo) |
| **siguiente acción** | `T-093`: terminar de cerrar `[A-011]` — cronometrar el peor caso real de Opus 5 contra un **percentil**, no contra el máximo de diez muestras (`[D-072]` ya lo deja anotado), y decidir si el reparto por fases de `[D-072]` (`read=6,5`) basta, sabiendo que `httpcore` aplica `read` por lectura de socket y no al cuerpo entero. Quedan abiertas del paso 8: `T-081` (renombrar `request_sent`, aplazada a propósito), `T-089` (mensaje inseguro de `install.sh`), `T-090` (valorar si el paso 8 cierra del todo), `T-092` (docstring de `measure_tutor.py` con `timeout=8.0` desactualizado). ⚠️ Sigue pendiente `T-086` (hora de las lecturas de costo de AWS, paso 7) y `T-088` (comentario de `MODEL` en `check_api_key.py`, para cuando toque el paso 9) |
