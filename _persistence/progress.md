...
| | |
|---|---|
| **paso** | 🚀 **PASO 9 EN CURSO, observabilidad y evals con rúbrica — quincuagésimo quinto tramo (`[S-072]`), 2026-08-19, después del cierre que dejó `[S-071]` (`fcd2d2d`).** `T-112` CORRIÓ y SALIÓ FALLIDA: 30 frases discriminantes nuevas (`measure_tutor.SENTENCES[61..90]`), etiquetadas a mano por el humano (15 `correct`/15 `wrong`/0 `unclear`), 30 llamadas reales a `claude-opus-5` (`$0,1026`, `run-df89768f_pick.jsonl`, archivado en `_persistence/replies/`). Cruce con las 60 representativas excluidas: **0 desacuerdos de 30** — tramo `0-2` de `[D-104]` = 🔴 FRACASO, la vara no discrimina al juez y se reescribe. `[D-104]` (el sello de `T-112`, cinco tramos + reparto 15/15 sellado aparte con sha256) y `[L-088]` (la lección del fracaso: forma bajo carga, no rareza gramatical, es la próxima pista) ya venían escritas por la sesión principal. `[D-103]` quedó ENMENDADA: su afirmación de que las dos condiciones del paso 9 se pagan con el mismo gasto es FALSA (`app/trace.py` no escribe `sentence`/`reply`/número de frase). `PI-6`: tres tests renombrados/derivados con autorización explícita del usuario del 2026-08-19 (`test_a_full_run_costs_what_we_decided_to_pay`, `test_no_numbers_means_all_of_them`, el `"61"` de `test_a_bad_number_stops_before_calling_anybody`), razón escrita dentro de cada uno. Archivos nuevos: `_persistence/seals/` (con portero propio, `tests/test_seals.py`, 3 tests, dos sabotajes vistos morder). **Suite: 580** — 579 passed + 1 failed antes del commit (`test_the_archived_replies_are_backed_up_by_git`, exige que el `.jsonl` de respuestas esté en Git; se pone verde al hacer `git add` en este mismo cierre). Paso 2b: `.js` al día, `tsc` compiló sobre `tsconfig.json` y coincide byte a byte con `app/static/app.js`. El paso 9 sigue ABIERTO: ninguna de las dos condiciones de `[D-103]` se cumple — la vara no discriminó, y `T-102` sigue sin correr |
| **última sesión** | 2026-08-19 (quincuagésimo quinto tramo, `[S-072]`) |
| **siguiente acción** | 🔴 **Escribir la SIGUIENTE vara discriminante, apuntando a FORMA BAJO CARGA** (`[L-088]`): frases largas, varios errores a la vez, ambigüedad de qué corregir primero — no rareza gramatical de superficie. Mismo orden fijo: escribir → etiquetar → correr el juez (`[L-083]` vigente, quien escriba será quien etiquete). Sigue abierta `T-108` (rendija del portero, cruzada hoy sin querer vía `_persistence/seals/`, sigue sin arreglarse). Sigue parado con disparador `T-102`, y `T-103`. ⚠️ Sigue pendiente `T-086` (hora de las lecturas de costo de AWS, paso 7) |
