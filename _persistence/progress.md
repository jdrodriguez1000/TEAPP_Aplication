...
| | |
|---|---|
| **paso** | 🚀 **PASO 9 EN CURSO, observabilidad y evals con rúbrica — quincuagésimo tercer tramo (`[S-070]`), 2026-08-19, después del cierre que dejó `[S-069]` (`b727ae9`).** `[D-102]` (firmada, tres enmiendas del mismo día): el nombre del corpus deja el eje `rubric-<huella>` y pasa a `run-<sello>`, con `sello = sha256(huella_rúbrica + huella_frases + huella_detector)[:8]` — un sello y no un quinto eje. La promoción a `_persistence/corpus/` deja de comparar cuatro ejes y compara uno. `~~D-092~~` tachada. **`T-109` CERRADA en sus dos mitades:** `save_replies` pasa de `open("w")` a `open("x")` y se niega con `SystemExit` si el archivo existe (el escape es borrar a mano, no un flag); `replies_file()` compone el nombre con `run_seal()`, y las tres huellas viajan además en cada fila. **`T-110` ABSORBIDA por `T-109`** — estado nuevo `🔗 absorbida` en la leyenda de `tasks.md`, porque `❌` habría dicho que el hallazgo era falso. `T-108` sigue pendiente, alcance ampliado a TRES sitios (`replies.py:83`, `tests/test_eval_rubric.py:512`, `tests/test_replies.py:25`). `T-112` desbloqueada. `[L-087]` nueva: un test que exige "seguir en verde" no demuestra que el portero mire — un sabotaje (saltarse la generación legado en `name_matches_rows`) dejó 574 tests en verde, tapado con `test_a_legacy_name_that_lies_about_the_rubric_is_still_caught`. Cinco sabotajes más vistos morder. La justificación anterior de `T-109` era falsa (decía que dos corridas el mismo día pisarían la copia archivada; medido, son dos carpetas distintas) y quedó enmendada en `[T-109]` de `tasks.md`. **Suite: 577 passed**, confirmado corriendo la suite en este cierre. Paso 2b no aplica: sin `.ts`/`.js` en el diff de hoy |
| **última sesión** | 2026-08-19 (quincuagésimo tercer tramo, `[S-070]`) |
| **siguiente acción** | 🆕 **`T-112`: escribir el conjunto DISCRIMINANTE de frases difíciles (`[D-101]`)**, ahora desbloqueada — orden fijo escribir → etiquetar → correr el juez, `[L-083]` vigente (quien las escriba será quien las etiquete, con el veredicto fresco). Sigue abierta `T-108` (rendija del portero, tres sitios). Siguen paradas con disparador `T-102` y `T-103`. ⚠️ Sigue pendiente `T-086` (hora de las lecturas de costo de AWS, paso 7) |
