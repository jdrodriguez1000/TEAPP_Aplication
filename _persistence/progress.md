...
| | |
|---|---|
| **paso** | 🚀 **PASO 9 EN CURSO, observabilidad y evals con rúbrica — cuadragésimo noveno tramo (`[S-066]`), 2026-08-18, después del cierre que dejó `[S-065]` (`9844eac`).** `T-107` CERRADA: el corpus del eval ya no se borra con cada corrida. `replies_file(picked)` compone el nombre con CUATRO ejes —modelo, fecha, huella de `GRAMMAR_RUBRIC` (`rubric_fingerprint()`) y marca `full`/`pick`— y `sentences_are_invented()` cierra `PI-8` como cerradura comprobable, no como advertencia. Las 10 filas del diagnóstico del 2026-08-17 se promovieron de `data/` a `_persistence/corpus/`, con `README.md` y un portero de tres tests con `glob` sobre la carpeta, corriendo en cada `pytest`. `[D-092]` y `[D-093]` (ésta AMPLIADA el mismo día con el portero), `[L-079]` nueva. `T-105` CERRADA: el corrector de rúbrica queda cableado a producción — `GrammarVerdict.correct: bool` pasa a `outcome: str` (tres estados: `correct`/`wrong`/`bad_format`) más `broken: frozenset[str]`; `correct` queda como propiedad derivada. `split_verdict` compone los tres y llama a `rubric_check.check_reply` con import diferido (evita ciclo). `TutorReply` y la traza (`app/api.py`, `app/trace.py`) suben los dos campos nuevos; la traza deja de escribir `correct`. `[D-094]` nueva. **Tests:** 516 → **533 passed**, confirmado corriendo la suite en este cierre. Sabotajes vistos morder en ambas tareas. Paso 2b no aplica: ningún `.ts`/`.js` en el diff |
| **última sesión** | 2026-08-18 (cuadragésimo noveno tramo, `[S-066]`) |
| **siguiente acción** | 🚀 **La corrida de 60 frases (~$0,18, saldo compartido `[C-009]`) es lo primero.** Hace tres trabajos de un pago: línea base nueva, corpus para `T-106`, y devuelve `COST_PER_CALL_USD` a estar medido (caducó con `[D-090]`/`[D-091]`). El `coste estimado` que imprime `eval_rubric.py` está caducado hacia abajo; el real se lee en la consola de Anthropic (regla 6). **Después, `T-106`** (etiquetar a mano las 60 frases), ya DESBLOQUEADA por `T-107`. Siguen paradas con disparador `T-102` y `T-103` (`[D-088]`, y ahora también `[D-081]`: cambiar `MODEL` dispara leer el límite por minuto y poner `LAB_REQUESTS_PER_MINUTE` en el mismo cambio). ⚠️ Sigue pendiente `T-086` (hora de las lecturas de costo de AWS, paso 7). 🆕 Rendija menor del portero de `_persistence/corpus/`: recorre solo `*.jsonl`, un corpus guardado como `.json`/`.txt` entraría sin que nadie lo mire — no bloquea nada |
