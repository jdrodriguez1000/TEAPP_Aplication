...
| | |
|---|---|
| **paso** | 🚀 **PASO 9 EN CURSO, observabilidad y evals con rúbrica — cuadragésimo séptimo tramo (`[S-064]`), 2026-08-17, después del cierre que dejó `[S-063]` (`25332da`).** `[D-089]` nueva: los evals arrancan por la FORMA de la rúbrica (cuatro promesas que comprueba un programa sin opinar), no por el veredicto. **Código nuevo:** `app/rubric_check.py`, `eval_rubric.py`, `tests/test_rubric_check.py`, `tests/test_eval_rubric.py`. **Tests:** 456 → **512 passed**, confirmado corriendo la suite en este cierre. Trece sabotajes vistos en rojo. Dos corridas reales pagadas: línea base de 60 llamadas (~$0,18) y diagnóstico de 10 (~$0,03) — números en `[D-089]`, no en este archivo. `[D-088]` nueva: `T-103` se PARA con disparador de acción (igual que `T-102`), firmada por el usuario tras dos votos técnicos, sin tocar código. `[L-075]` nueva: un docstring decía la regla correcta y la línea de debajo la incumplía, en el mismo minuto. `[L-069]` enmendada con una TERCERA mitad: la regla de citar la fecha desde `git log` duró 4 h 47 min — 16 apariciones de `2026-08-18` corregidas a `2026-08-17` en 6 archivos de este repo (`decisions.md`, `lessons.md`, `progress.md`, `app/english_tutor.py`, `app/trace.py`, `tests/test_trace.py`), más 1 en `Edu_TripleS/PROGRESO.md` corregida por la terminal auditora. Paso 2b no aplica: ningún `.ts`/`.js` en el diff. 🔻 **Abierta, esperando firma del usuario (preguntado, sin contestar):** si sube el tope a TRES frases en `GRAMMAR_RUBRIC`/`rubric_check.MAX_SENTENCES`, y si se afina la comprobación de comillas a solo las que envuelven una corrección |
| **última sesión** | 2026-08-17 (cuadragésimo séptimo tramo, `[S-064]`) |
| **siguiente acción** | 🚀 **Seguir dentro del paso 9.** Tres caminos abiertos, ninguno decidido: (1) firma del usuario sobre el tope de tres frases y la comprobación de comillas de `[D-089]`; (2) cablear el corrector de rúbrica a producción — que la ruta lo llame y la traza apunte el fallo de formato — escrito y NO cableado a propósito; (3) etiquetar las 60 frases de la línea base para medir si el veredicto acierta, la otra mitad del paso 9. Siguen paradas con disparador `T-102` (traza con servidor real, disparador = descenso de modelo de `[D-049]`) y `T-103` (`[D-088]`). ⚠️ Sigue pendiente `T-086` (hora de las lecturas de costo de AWS, paso 7) |
