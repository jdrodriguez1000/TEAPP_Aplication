...
| | |
|---|---|
| **paso** | 8 de 9 — 🔓 **Los dos prerrequisitos del paso 8 quedaron resueltos, y el paso arranca de verdad con `T-076`.** Decimonoveno tramo (`[S-036]`), sesión de decisión, verificación por SSH y planeación pura: ningún archivo de `app/`, `tests/` ni `deploy/` tocado en el repo — el único cambio en el diff es `_persistence/decisions.md` (`[D-049]`). `T-075` CERRADA (API key de Anthropic en el `.env` LOCAL, verificada sin imprimirla — `sk-ant-`, 108 caracteres, `.gitignore:3` la protege). `T-056` CERRADA por SSH real contra la EC2 (según traspaso, sin rastro en `git diff`): `TEAPP_REGISTRATION_OPEN = false` explícito en `/opt/teapp/.env` (permisos 600, dueño `ubuntu`) y `create_account.py` corriendo en el servidor (mensaje de uso, código 1). Hallazgo de esa verificación: `ANTHROPIC_API_KEY` existe en el servidor pero está **vacía** — abre `T-078`. `[D-049]` nueva: el paso 8 arranca con `claude-opus-5` a `effort: "low"` (el modelo MÁS caro, no el más barato) para no mezclar dos sospechosos —modelo y rúbrica— el día que se estrena el veredicto real; el descenso a Sonnet 5 y Haiku 4.5 pasa a ser trabajo MEDIDO del paso 9. Plan de archivos de `T-076` hecho y **sin ejecutar**: `app/tools.py`, `app/config.py`, `app/english_tutor.py`/`app/api.py` (solo comentarios), `tests/test_tools.py`, `tests/test_english_tutor.py`, `tests/test_api.py`, `requirements.txt`, `.env.example`, `deploy/install.sh`. Hallazgo sobre `T-077`: "borrar el agente falso" solo borra media falsedad — `respond()` seguirá llamando a las tres herramientas siempre y en el mismo orden, `scope.md` no pide que elija |
| **última sesión** | 2026-08-10 (decimonoveno tramo) |
| **siguiente acción** | ▶️ **Arrancar `T-076`: sustituir el cuerpo de `judge_grammar` por la llamada real a Claude (`claude-opus-5`, `effort: "low"`, `[D-049]`), siguiendo el plan de archivos ya escrito.** ⚠️ `tests/no_network.py` bloquea la red en toda la suite (`C-001`) — los tests nuevos tienen que inyectar un cliente falso, no llamar a Claude de verdad. ⚠️ **Sobre el estado de la máquina de producción: no se afirma aquí sin comprobarlo** (`[L-038]`) — mirar el Paso 5b de `deploy/console_steps.md` antes de asumir que está encendida o apagada. Tras `T-076`: `T-077` (borrar `FAKE_VERDICT`, con la precisión de que solo tapa media falsedad), `T-078` (la llave al servidor vía `install.sh`, ya sabido que hoy está vacía ahí) y `T-079` (medir `[A-010]`/`[A-011]` de verdad). `T-069`/`T-070`/`T-046`/`T-067` del paso 7 siguen abiertas, con dueño y fecha ya escritos en `[D-048]`/`[A-023]` |
