...
| | |
|---|---|
| **paso** | 8 de 9 — 🎉 **`T-078` CERRADA — la llave llegó al servidor y hubo primera práctica real.** Vigésimo noveno tramo (`[S-046]`), segundo tramo del 2026-08-13, después del cierre que dejó `[S-045]` (`699f2b2`). **`T-087` cerrada (13:57 UTC):** `check_api_key.py` con `teapp-server` dio salida `0`, `requests-limit=1000` — el episodio de saturación de Anthropic duró entre 9 y 19 minutos (10×`529` entre 13:36 y 13:46). **`T-078` cerrada (14:04–14:08 UTC) en la máquina real:** `git pull` `afe2eab`→`699f2b2` (36 commits), `install.sh` código 0 con el portero mordiendo antes de escribir, `.env` con permisos `600` y la llave de `teapp-server`, servicios `active`. **Prueba PI-4:** práctica real desde el navegador — `I cooking in these morning` corregida, Score 9, cuota y marcador escritos. La llave viajó por `stdin`→`read -r`→`export`→`sudo -E`, no por la línea de comandos (el mensaje de error de `install.sh` sigue recomendando la forma insegura — `T-089` nueva). `[L-033]` mordió por cuarta vez en el día (dos `ssh` por nombre fallaron, se entró por IP fija). Falsa alarma descartada: el `curl: (7)` de la salida de `install.sh` es el primer intento de un bucle de diez, no un guardián mudo. Único archivo modificado: `_persistence/decisions.md` (`[D-063]` ampliada, `[D-065]` ya existía). Suite no se corrió hoy — último número medido: 410, sobre `32f3314`. Ningún `.ts`/`.js` en el diff; paso 2b confirmado igual (`compilar: 0`, `comparar: 0`) |
| **última sesión** | 2026-08-13 (vigésimo noveno tramo) |
| **siguiente acción** | 📌 **`T-090` nueva: valorar si el paso 8 queda cerrado del todo o falta algo antes del paso 9.** Quedan abiertas dentro del paso 8: `T-079` (a medias, decidir qué hacer con el timeout de 10 s), `T-081` (renombrar `request_sent`, aplazada a propósito), `T-019` (contrato de `judge_grammar`, desbloqueada y sin decidir), `T-089` (mensaje inseguro de `install.sh`). ⚠️ Sigue pendiente `T-086` (hora de las lecturas de costo de AWS, paso 7) y `T-088` (comentario de `MODEL` en `check_api_key.py`, para cuando toque el paso 9) |
