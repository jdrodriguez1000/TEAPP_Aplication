...
| | |
|---|---|
| **paso** | 🚀 **PASO 9 EN CURSO, observabilidad y evals con rúbrica — quincuagésimo segundo tramo (`[S-069]`), 2026-08-18, después del cierre que dejó `[S-068]` (`f7a64ad`).** `[D-099]`: el corpus de las 60 respuestas del juez se ARCHIVÓ de `data/` a `_persistence/replies/` (copiar → verificar en Git → borrar), con portero propio (`replies.py`, 12 tests) y conjunto de campos cerrado. `[D-100]`: el sello de `T-111` escrito ANTES de calcular — denominador 58 (dos filas contaminadas excluidas por regla, no por lista), tabla 2×2 en vez de un solo número. **`T-111` CERRADA** (commit `ed8d162`): el cruce de las 60 etiquetas humanas contra el veredicto real del juez da **58 de 58** sobre el conjunto sellado (60 de 60 sin excluir) — cero perdones, cero correcciones de más, cero formato roto. Saboteado antes de creerlo: voltear una etiqueta o una respuesta da 57/58. `L-085` corregida (ninguna de las dos predicciones llegó a disco) y `L-086` nueva: un 100% aquí mide el EXAMEN, no el juez — `SENTENCES` no discrimina y el eval está saturado. `D-101`: la próxima vara será DISCRIMINANTE, no representativa, y `[D-049]` gana condición — no se toca `MODEL` hasta que exista una vara capaz de bajar. **Suite: 570 passed**, confirmado corriendo la suite en este cierre. Paso 2b no aplica: sin `.ts`/`.js` en el diff de hoy |
| **última sesión** | 2026-08-18 (quincuagésimo segundo tramo, `[S-069]`) |
| **siguiente acción** | 🆕 **Escribir el conjunto DISCRIMINANTE de frases difíciles** (`D-101`), con el orden escribir → etiquetar → correr el juez, y con `[L-083]` vigente — quien las escriba será quien las etiquete, con el veredicto fresco. Sin tarea numerada todavía, nace de este cierre. `T-109` (dos corridas ENTERAS el mismo día se pisan) deja de ser tarea de fondo: apunta a un insumo concreto (`_persistence/replies/`) y gana prioridad. Sigue abierta `T-108` (rendija menor del portero de `_persistence/corpus/`, `*.jsonl` por `glob`). `T-110` candidata, no decidida: meter una huella de `rubric_check.py` en la FILA del corpus — la firma el humano. Siguen paradas con disparador `T-102` y `T-103`. ⚠️ Sigue pendiente `T-086` (hora de las lecturas de costo de AWS, paso 7) |
