...
| | |
|---|---|
| **paso** | 🚀 **PASO 9 EN CURSO, observabilidad y evals con rúbrica — cuadragésimo octavo tramo (`[S-065]`), 2026-08-17, después del cierre que dejó `[S-064]` (`df616dd`).** `T-104` CERRADA en sus dos mitades, ambas firmadas por el usuario: `[D-090]` sube el tope de dos a tres frases (motivo bueno: duplicaba `"never correct more than one thing at a time"`, y `too_many_sentences` salía roja 18/60 con Opus 5 — **no** el motivo de `[D-089]`, desmentido el mismo día); `[D-091]` endurece la rúbrica prohibiendo TODAS las comillas en vez de afinar el corrector, porque la respuesta no trae delimitador de dónde está la corrección (cinco formas distintas en disco) y una heurística de fraseo se rompería junto con `[D-049]`. `MAX_SENTENCES` pasa a vivir SOLO en `app/tools.py`. **Tests:** 513 → **516 passed**, confirmado corriendo la suite en este cierre. Tres sabotajes más vistos en rojo, y el listón nuevo visto morder al revés (tope a cuatro → 3 rojos). `[L-076]` nueva: la línea base de 60 nunca se guardó — el archivo tiene 10 filas. `[L-077]` nueva: `COST_PER_CALL_USD` duplicado en `eval_rubric.py`, tres líneas bajo el comentario que decía que se importa — arreglado importándolo. `[L-078]` nueva: el criterio ante un test rojo es mirar el `assert` — ¿se quedó corto el ejemplo, o está mal puesto el listón? `[D-089]` ENMENDADA en su sitio: su hipótesis final quedó desmentida y su "línea base de 60" es falsa por omisión. `measure_tutor.py`: `COST_PER_CALL_USD` marcado CADUCADO hacia el lado malo, sin inventar número nuevo (regla 6). **T-107 nueva, y va ANTES de la corrida de 60:** el corpus del eval (`data/eval_replies.jsonl`) no sobrevive a la siguiente corrida — `save_replies` abre en `"w"` y el nombre no distingue modelo ni fecha. `T-106` pasa a BLOQUEADA por `T-107`. `T-105` sigue libre: no toca rúbrica ni corpus. Paso 2b no aplica: ningún `.ts`/`.js` en el diff |
| **última sesión** | 2026-08-17 (cuadragésimo octavo tramo, `[S-065]`) |
| **siguiente acción** | 🚀 **T-107 primero, antes que la corrida de 60.** El corpus del eval se borra con cada corrida (`eval_rubric.py:188`/`243`, `[L-076]`) — arreglar eso (nombre que distinga modelo/fecha, sin volver a `"a"`) es el paso previo a gastar. **Después, la corrida de 60 con la rúbrica ya endurecida** (`T-104` cerrada): sirve de línea base nueva Y de corpus para `T-106`, y de paso devuelve `COST_PER_CALL_USD` a estar medido (hoy caducado hacia el lado malo). Mientras tanto, `T-105` (cablear el corrector a `app/api.py`) es el camino para avanzar sin gastar. Siguen paradas con disparador `T-102` y `T-103` (`[D-088]`). ⚠️ Sigue pendiente `T-086` (hora de las lecturas de costo de AWS, paso 7) |
