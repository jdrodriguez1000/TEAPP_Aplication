...
| | |
|---|---|
| **paso** | 8 de 9 — Trigésimo tercer tramo (`[S-050]`), cuarto cierre del 2026-08-13, después del cierre que dejó `[S-049]` (`9ffab80`). **`T-079` CERRADA con `[D-070]`: `TUTOR_TIMEOUT_SECONDS` se queda en 10,0 s, y `[A-011]` muere la segunda vez y ahora sí.** `[L-045]` había acotado por lectura la única rendija que quedaba —que `respond()` fuera del modelo se coma más de 2 s—; se midió con `measure_local_parts.py` nuevo en la raíz (coste $0, no llama a Anthropic ni escribe en `data/`): **56,3 ms el peor caso, cinco corridas, con 40 hilos sobre el mismo archivo**, contra 2 000 ms de presupuesto — 35× de margen. El cierre se apoya en un TECHO IMPUESTO (el cliente corta a 8,0 s pase lo que pase), no en una observación — es la diferencia exacta con el cierre fallido de `[L-043]`. `tests/test_measure_local_parts.py` nuevo vigila que el `POOL_SIZE` de la báscula siga igualado a `api.TUTOR_POOL_SIZE`, visto morder con sabotaje (4 → rojo, restaurado → verde). Comentarios actualizados en `app/api.py` (el timeout deja de decirse "predicción") y `app/tools.py` (se retira el puntero muerto a `T-079`). `[A-011]` RETIRADA de `assumptions.md` — segunda vez; la primera, del 2026-08-11, se reabrió (`[L-043]`). Suite: **426 passed** (425 + el test nuevo), confirmado corriendo `.venv/Scripts/python.exe -m pytest -q` en este cierre. Paso 2b confirmado: `compilar: 0`, `comparar: 0` — ningún `.ts` en el diff. ⚠️ **Pendiente detectado y no tocado hoy:** `measure_tutor.py` sigue citando `[A-011]` como abierta en su docstring (líneas 5, 8, 10, 15) y en un `print` (línea 259) — punteros muertos desde hoy; queda `T-092` nueva |
| **última sesión** | 2026-08-13 (trigésimo tercer tramo) |
| **siguiente acción** | Corregir los punteros muertos a `[A-011]` en `measure_tutor.py` (`T-092` nueva). Quedan abiertas del paso 8: `T-081` (renombrar `request_sent`, aplazada a propósito), `T-089` (mensaje inseguro de `install.sh`), `T-090` (valorar si el paso 8 cierra del todo). ⚠️ Sigue pendiente `T-086` (hora de las lecturas de costo de AWS, paso 7) y `T-088` (comentario de `MODEL` en `check_api_key.py`, para cuando toque el paso 9) |
