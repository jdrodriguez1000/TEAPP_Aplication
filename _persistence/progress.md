...
| | |
|---|---|
| **paso** | 🚀 **PASO 9 EN CURSO, observabilidad y evals con rúbrica — quincuagésimo tramo (`[S-067]`), 2026-08-18, después del cierre que dejó `[S-066]` (`165f415`).** `D-095` cerró en commit propio: el archivo de respuestas se nombra por `len(records)` (lo que llegó), no por el plan; test nuevo `test_a_run_that_is_cut_short_is_named_by_what_arrived`, primero que entra en `main()`; `L-080`. En este tramo, sin commitear al arrancar: `COST_PER_CALL_USD` de `measure_tutor.py` pasa a **0,00342** (`D-096`), medido con la factura real de $0,20 de la corrida de línea base de 60 (60/60, entera, cero cortes) — `MAX_CALLS_PER_RUN` de 82 a 73. `tests/test_measure_tutor.py`: solo el EJEMPLO del docstring cambia (82/veintidós → 73/trece); el `assert` no se tocó (`L-078`). El archivo de la corrida (`data/eval_replies_claude-opus-5_2026-08-18_rubric-bbf4be38_full.jsonl`, 60 líneas) queda en `data/`, ignorado por Git — no se promueve, `[D-092]` cuelga esa promoción del commit que mueva `MODEL`/`GRAMMAR_RUBRIC`. `L-081` nueva: la huella del corpus sella `GRAMMAR_RUBRIC` (la pregunta), no `rubric_check.py` (la báscula) — comprobado contra el corpus del 17: 10 filas rotas guardadas, 1 con el detector de hoy. **Tests:** 534 passed, confirmado corriendo la suite en este cierre. Paso 2b: `.ts`/`.js` en el repo pero sin cambios en el diff — comprobado igual, al día |
| **última sesión** | 2026-08-18 (quincuagésimo tramo, `[S-067]`) |
| **siguiente acción** | 🟢 **`T-106` desbloqueada: etiquetar a mano las 60 frases de la corrida entera para medir si el veredicto del juez acierta.** Antes de arrancarla, falta decidir dónde vive el archivo de etiquetado a mano — hoy el corpus está en `data/`, sin copia y fuera de Git, y en cuanto lleve 60 juicios humanos deja de tener precio; ni `[D-092]` ni ninguna otra regla cubre el trabajo humano que ENTRA. `T-109` nueva: dos corridas ENTERAS el mismo día se pisan (modelo/fecha/huella idénticos, `save_replies` abre en `"w"`) — `D-095` mató el caso cortada-contra-entera, no el de entera-contra-entera, y el juez no es determinista. `T-110` candidata, no decidida: meter una huella de `rubric_check.py` en la FILA del corpus, patrón como `rubric` — la firma el humano (`L-081`). Siguen paradas con disparador `T-102` y `T-103`. ⚠️ Sigue pendiente `T-086` (hora de las lecturas de costo de AWS, paso 7). Sigue abierta `T-108` (rendija menor del portero de `_persistence/corpus/`) |
