...
| | |
|---|---|
| **paso** | 🏁 **8 de 9 CERRADO el 2026-08-15 (`[D-081]`), listo para cruzar al paso 9.** Lo de abajo es el estado del último cierre del 14, que se conserva. — Cuadragésimo segundo tramo (`[S-059]`), cuarto cierre del 2026-08-14, después del cierre que dejó `[S-058]` (`954b296`). **`T-089` CERRADA y `T-097` nueva y cerrada.** Auditoría externa señaló que `install.sh` recomendaba `sudo VAR=... bash …` para pasar la llave; al leer el archivo entero apareció algo peor: la cabecera traía el mismo patrón **con la llave completa**, a tres líneas de un aviso que decía "NUNCA como argumento". 🧪 **Medido en la EC2, 18:54 UTC:** `sudo FOO=secreto123 sleep 30` visible en `ps aux` con dueño `root`, desde la cuenta `ubuntu`. No hubo que rotar nada: `grep -c "sk-ant"` sobre los dos `.bash_history` dio 0 y 0 — el despliegue del 13 ya usó la forma segura. Arreglo en `install.sh`: `export` → `read -r -s` → `sudo -E`, y el arreglo también se midió (`sudo -E` sobrevive a `Defaults env_reset`). `T-097` (número propio, a pedido del usuario): la misma forma, sin secreto dentro, retirada también de `deploy/README.md`, `deploy/console_steps.md` y el otro mensaje de error de `install.sh` — motivo: es la plantilla que se copia, no una fuga. `[L-061]` nueva (escrita por la sesión principal). **`T-090` CERRADA: el paso 8 NO queda cerrado** — razón dada en el traspaso (`T-089` pasó de cosmético a clase de seguridad medida en segundos, cerrar a ciegas sin mirar lo pendiente no era razonable). ✅ **Anotada en `[D-080]`**, commit `6c7b5a7`, posterior a este sello. 🔴 **Corregido el 2026-08-15:** esta casilla decía *"esa decisión no está anotada — falta que el usuario la dicte"*, y era falsa cuando se leyó — `progress.md` se selló en `8b9b37f` y `[D-080]` se escribió después, en un commit que solo tocó `decisions.md`. Ver `[L-062]`. `T-079` queda como primera tarea del próximo día, con `[D-077]` abierto, sin tocar hoy a propósito (cuarta sesión del día). **Corrección de arrastre:** el resumen del cierre anterior dijo que la lectura de `T-095` fue "en AWS"; era la consola de **Anthropic** (`teapp-measure`) — error nacido y muerto en el chat, `progress.md`/`tasks.md` de ese cierre ya estaban bien escritos; `T-086` sigue pidiendo la próxima lectura de AWS. Suite: 440 passed (sin cambio, no se tocó código de app). Paso 2b confirmado en este cierre: `compilar: 0`, `comparar: 0` — ningún `.ts` en el diff |
| **última sesión** | 2026-08-15 (cuadragésimo tercer tramo, `[S-060]`) |
| **siguiente acción** | 🏁 **PASO 8 CERRADO el 2026-08-15 con `[D-081]`** — las cuatro miradas de `[D-080]` hechas: `T-089` ✅ midiendo, `T-079` 🟡 condición viva, `T-081` 🔲 aplazada con motivo, `T-088` ✅ desarmada. **Queda UNA pendiente, no dos.** 🔻 **DISPARADOR — antes de cambiar `MODEL`, CADA VEZ:** leer en la consola el límite por minuto de ese modelo en `teapp-measure` y ponerlo en `LAB_REQUESTS_PER_MINUTE` **en el mismo cambio**; si no, el portero acepta la llave del laboratorio sin dar error (`[D-081]`, `T-088`). ⚠️ **El paso 9 es "Observabilidad y evals con rúbrica"** (`_context/roadmap.md:23`), y `[D-049]` mete dentro el descenso a **Sonnet 5 y Haiku 4.5** — o sea que el disparador salta **al menos dos veces**, no una al abrir el paso. 💣 **`T-098` armada, disparador = el próximo arranque:** el guion de inicio lee la prosa en vez del campo de estado y no se salta lo tachado. 🟢 **`T-079` resuelta el 2026-08-15 sin gastar nada:** revisión DOCUMENTAL de `[D-077]`, el ✅ baja a 🟡 con disparador escrito (si vuelven los cortes, se repite la tanda de 60 ANTES de tocar `TUTOR_TIMEOUT_SECONDS`). No se repitió la tanda: otra daría otro verde igual de condicionado por otros $0,18. `T-090` ✅ cerrada, ver `[D-080]`. ⏳ **Pendiente y sin bloquear: leer el SALDO PREPAGADO actual** en la consola de Anthropic (un clic, $0), con la hora UTC anotada ANTES del número (`[D-079]`) — el 6,55 US$ de `[D-057]` es del 2026-08-11 y desde entonces corrieron `T-093` y `T-095`. 🚨 **Se lee ANTES del próximo bucle de llamadas, sea cual sea.** Quedan abiertas del paso 8: `T-081` (renombrar `request_sent`, aplazada a propósito), `T-088` (comentario de `MODEL` en `check_api_key.py`, para el paso 9), `T-096` (prioridad baja, ~5 llamadas sin dueño en `teapp-measure`). ⚠️ Sigue pendiente `T-086` (hora de las lecturas de costo de AWS, paso 7) |

## Índice

| id | fecha | qué avanzó | paso |
|---|---|---|---|
| S-060 | 2026-08-15 | Cuadragésimo tercer tramo, primero del 2026-08-15, después del cierre que dejó `[S-059]` (`6c7b5a7`). 🏁 **PASO 8 CERRADO con `[D-081]`**, tras mirar las cuatro pendientes una por una como mandaba `[D-080]`: `T-089` ✅ (cerrada el 14 midiendo), `T-079` 🟡 (baja de ✅ a condición viva), `T-081` 🔲 (aplazada con motivo, PI-3), `T-088` ✅ (**desarmada**). 🔑 **La sesión entera nació de una auditoría de apertura que desmintió el resumen de arranque:** `T-090` se ofreció como pendiente estando decidida y commiteada en `[D-080]` desde el día anterior, y `~~A-024~~` viajó como *"sin comprobar"* estando retirada desde el 11. **`T-088` era el hallazgo gordo:** su ficha decía *"corregir cuando toque el paso 9"* y resultó estar **ARMADA para** el paso 9 — `anthropic-ratelimit-requests-limit` es por modelo, así que cambiar `MODEL` sin tocar `LAB_REQUESTS_PER_MINUTE` hace que `main()` devuelva `EXIT_OK` **aceptando la llave del laboratorio sin dar error**; denegar por defecto convertido en aceptar por accidente. Arreglada con dos comentarios y cero lógica. **`T-098` nueva y cerrada el mismo día:** el guion de arranque leía la prosa en vez del campo de estado y no se saltaba lo tachado — arreglado en `protocol-start` y `session-starter`, y **verificado corriendo el arranque TRES veces**, cada corrida destapando lo que la anterior tapaba (convención de tachado ausente en `decisions.md`; el disparador del paso 9 perdido y colgado de la tarea equivocada; y el roadmap desmintiendo que el paso 9 sea *"bajar a Haiku"* — es *"Observabilidad y evals con rúbrica"*, `_context/roadmap.md:23`). Adoptada la convención `~~D-nnn~~` con dos marcas (🔻 SUPERADA / ✅ CUMPLIDA), aplicada a `~~D-071~~` y `~~D-080~~`. **Cinco lecciones nuevas, `[L-062]`–`[L-066]`**, todas de la misma familia: una afirmación que fue cierta y a la que nadie fue a apagar. Código tocado: solo `deploy/check_api_key.py`, solo comentarios. Suite: **440 passed**, corrida dos veces. Cinco commits, todos subidos a `origin` (`a54baa3`). Paso 2b no aplica: ningún `.ts` en el diff | 8 |
| S-059 | 2026-08-14 | Cuadragésimo segundo tramo, cuarto cierre del 2026-08-14, después del cierre que dejó `[S-058]` (`954b296`). **`T-089` CERRADA — `install.sh` recomendaba `sudo VAR=... bash` para la llave, y la cabecera del propio archivo la traía COMPLETA a tres líneas de un aviso que decía "NUNCA como argumento".** Medido en la EC2 (18:54 UTC): el argumento de `sudo` es visible en `ps aux` para cualquier usuario, dueño `root`. No hubo que rotar la llave (0 rastro en los dos `.bash_history`). Arreglo: `export`/`read -r -s`/`sudo -E`, medido que sobrevive a `env_reset`. `[L-061]` nueva. **`T-097` nueva y cerrada** (número propio, a pedido del usuario): la misma forma retirada de los tres sitios que la traían sin secreto — la plantilla, no la fuga. **`T-090` decidida — el paso 8 NO cierra** (razón: `T-089` resultó ser seguridad real, no cosmética); decisión anotada en `[D-080]` (commit `6c7b5a7`, posterior a este sello) — 🔴 **este renglón decía "sin anotar, queda para el usuario"; corregido el 2026-08-15, ver `[L-062]`.** `T-079` pasa a primera tarea de mañana, sin tocar hoy. **Corrección de arrastre:** el cierre anterior atribuyó la lectura de `T-095` a AWS; era Anthropic (`teapp-measure`) — error solo en el chat, los archivos ya estaban bien. Suite: 440 passed (heredado, sin cambio de código). Paso 2b confirmado: `compilar: 0`, `comparar: 0` — ningún `.ts` en el diff | 8 |
