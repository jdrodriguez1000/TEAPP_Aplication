...
| | |
|---|---|
| **paso** | 7 de 9 — 🚀 **`T-073` sigue cerrada; el instalador gana `is-enabled` tras revisión externa. Decimosexto tramo (`[S-033]`):** `install.sh` comprobaba el temporizador solo con `is-active`, que no distingue "corre ahora" de "volverá tras apagar y encender" — exactamente el modo de fallo que `T-074` no puede ver por sí sola. Se añade `is-enabled` al lado, más el quinto guardián en `tests/test_deploy_shutdown.py`. De 360 a **362** tests. `[L-034]` nueva (un control que mide el ahora no mide el mañana) y gana su antepasado real, `[L-017]` — mismo archivo, mismo bloque, misma orden `is-active`, cuatro días antes. `[A-022]` nueva: la zona horaria dentro de `OnCalendar` está sin comprobar, se mide con `systemd-analyze calendar`, hoy y otra vez en `T-069`. Corregida una colisión de identificadores entre una lección de este repo y su homóloga del repo supervisor, que se citaban indistintamente — 16 citas arregladas y el prefijo (`[LM.nn]` doble letra / `[L-nnn]` guion) escrito en `CLAUDE.md`. 🚨 **`T-074` SIGUE PENDIENTE Y VENCE HOY A LAS 23:00 UTC (18:00 Colombia):** nada de este tramo la mide — sigue siendo la única parte de comportamiento, no configuración |
| **última sesión** | 2026-08-09 (decimosexto tramo) |
| **siguiente acción** | 🚨 **T-074 — mirar el temporizador disparar de verdad, hoy 2026-08-09 a las 23:00 UTC (18:00 Colombia):** con alguien viendo la consola de AWS, confirmar que la instancia pasa a `stopped` sola y anotar la hora exacta. Si a las 18:02 Colombia sigue en `running`, apagarla a mano y anotarlo como hallazgo, no como contratiempo — significaría que el temporizador no disparó y que `T-073` no está cerrada del todo. Al día siguiente, encender la máquina a mano desde la consola antes de trabajar, y correr el Paso 5b de `deploy/console_steps.md` (cinco controles). Después: `[A-022]` con `systemd-analyze calendar` en la máquina viva, `T-066` (dos dispositivos), `T-067` (gasto real bajo la ventana de `[D-045]`, tres tarifas). **Entrar por SSH usando la IP fija `32.199.55.191`**, no el nombre — el DNS falla intermitente y la causa está en el cliente (`[L-033]`) |
