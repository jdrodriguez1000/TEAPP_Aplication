...
| | |
|---|---|
| **paso** | 8 de 9 — Trigésimo segundo tramo (`[S-049]`), tercer cierre del 2026-08-13, después del cierre que dejó `[S-048]` (`f4b73b7`). **Ronda de auditoría externa sobre `T-091`/`T-078`/`T-019`, atendida en el commit `e2044e8`** — sin código, solo `_persistence/decisions.md`, `_persistence/lessons.md` y `deploy/README.md`. La auditoría confirmó 5 de 7 afirmaciones midiendo: 425 passed sobre `f4b73b7`, `add_point`/`read_score` sin rastro en el código, `[A-001]`/`[A-028]` solo como comentarios de defunción, y el dominio sirviendo `id="practice"`. **`[L-052]` nueva:** la lección que `[A-001]` mandaba escribir al morir se fue entera a `decisions.md` (`[D-066]`), destino de la otra rama de su propia regla — un maniquí no solo tapa fallos, congela decisiones de diseño y las devuelve el día caro. **`[L-053]` nueva:** `curl -s` sin resolver devuelve cuerpo vacío, y un `grep` sobre el vacío dice "no está" en vez de "no medí" — fabricó un hallazgo contra un despliegue correcto; `deploy/README.md` corregido para no invitar al error. **`[D-069]` corregida:** dejó de mandar a comprobar el formato en un disco donde ya no está (cuenta desechable `probe-format`, borrada); ahora nombra las dos corridas y apunta a `jorge.json` como único respaldo vivo, con su índice sincronizado ("tres llamadas" → "cinco"). **Dos citas equivocadas detectadas y corregidas en la propia ronda:** el traspaso atribuía a `[D-066]`/`[D-069]` citas a `[L-017]`/`[A-002]` que no existen ahí; y al escribir `[L-053]` se citó `[L-033]`, se abrió, no decía lo atribuido, y se quitó el corchete. Suite: **425 passed**, confirmado corriendo `.venv/Scripts/python.exe -m pytest -q` en este cierre. Paso 2b confirmado: `compilar: 0`, `comparar: 0` — ningún `.ts` en el diff |
| **última sesión** | 2026-08-13 (trigésimo segundo tramo) |
| **siguiente acción** | Quedan abiertas del paso 8: `T-079` (a medias, decidir qué hacer con el timeout de 10 s), `T-081` (renombrar `request_sent`, aplazada a propósito), `T-089` (mensaje inseguro de `install.sh`), `T-090` (valorar si el paso 8 cierra del todo). ⚠️ Sigue pendiente `T-086` (hora de las lecturas de costo de AWS, paso 7) y `T-088` (comentario de `MODEL` en `check_api_key.py`, para cuando toque el paso 9) |
