...
| | |
|---|---|
| **paso** | 8 de 9 — 🔄 **`T-076` sigue A MEDIAS. Una segunda auditoría externa corrigió el proxy de `[D-054]`: la cuota ya no se decide por la forma de `content`, se decide por `usage`.** Vigésimo segundo tramo (`[S-039]`), después del cierre que dejó `[S-038]`. `[D-054]` había resuelto la rama del veredicto vacío exigiendo `stop_reason == "refusal"` **y** `content` vacío — un proxy con un agujero comprobado: sin streaming (como llama `judge_grammar`), un rechazo a MITAD omite el parcial, así que llega calcado por fuera al rechazo gratis con los tokens ya pagados. `[D-055]` lo corrige: `app/tools.py` decide ahora con `answer.usage.input_tokens > 0 or answer.usage.output_tokens > 0` — el contador de la propia respuesta, no una forma inferida. `tests/fake_tutor.py` gana `FakeUsage` y el campo `usage` en `FakeAnswer`, con defecto **facturado** (regla 3 en el valor por defecto), más el constructor `refusing_mid_output_without_partial()`. `tests/test_tools.py` gana el guardián `test_a_billed_refusal_with_no_partial_still_charges`, verificado en ROJO por sabotaje (reponiendo el proxy viejo, el test falla con cuota regalada). Suite de 380 a **381 pasando** (corrida en este cierre, sigue en 381 tras la ampliación). `[D-055]` y `[L-040]` ya escritas por la sesión principal, revisadas en este cierre. `app/api.py` sigue SIN TOCAR: toda la maquinaria de `request_sent` existe y está probada, pero nadie la usa — la cuota no se devuelve nunca. 📎 **Ampliación del mismo cierre, después del commit `8d335fd`:** una terminal auditora trajo tres hallazgos más, ya escritos por la sesión principal y solo revisados aquí — `[D-056]` (escalera de documentación: `ctx7` primero, la skill `claude-api` último recurso, medido en 55K→~340K tokens de una sola consulta), `[L-040]` CORREGIDA (la cita a `[L-036]` que se había declarado falsa era correcta; el error fue leer 13 de 119 líneas y juzgar por el encabezado — regla nueva: "abrir la entrada no es leerla") y `[A-024]` (la llave de la API de Anthropic no tiene tope de gasto conocido, a comprobar antes de `T-079`) |
| **última sesión** | 2026-08-10 (vigésimo segundo tramo, con ampliación del mismo día tras el commit `8d335fd`) |
| **siguiente acción** | 🚨 **`app/api.py` tiene que cazar `TutorUnavailableError`, mirar `request_sent` y llamar a `quota.refund`.** Es lo único que falta para cerrar `T-076`: la excepción ya se lanza con la información correcta (`[D-050]`, `[D-051]`, `[D-055]`), pero hoy nadie la atrapa y la cuota jamás se devuelve, ni siquiera cuando la petición nunca salió. 🚨 **Antes de `T-079` (que llama en bucle): mirar en la consola de Anthropic si la llave admite tope de gasto o alerta de uso** (`[A-024]`, `T-080`) — hoy no hay freno conocido ahí, a diferencia de AWS. Después: `T-077`, `T-078`. ⚠️ **Sobre el estado de la máquina de producción: no se afirma aquí sin comprobarlo** (`[L-038]`) |
