...
| | |
|---|---|
| **paso** | 8 de 9 — Trigésimo sexto tramo (`[S-053]`), séptimo cierre del 2026-08-13, después del cierre que dejó `[S-052]` (`c89ae1d`). **Tercera vuelta de la misma auditoría del día: el número de `read` NO cambia, cambia el porqué.** `[D-072]` justificaba `read = 6,5` como *"un 38% por encima de los 4,72 s de la peor de diez"* — razonamiento roto: `max(n=10)` no estima una cota, estima un cuantil que crece con N, lo mismo que demostraron las propias seis tandas locales de la báscula (44,9→62,4 ms, +39% y subiendo). `[D-073]` nueva: `read` deja de estimarse y pasa a calcularse por **resta** del presupuesto — `10,00 (ruta) − 2,50 (connect+write+pool) − 0,07 (trabajo local) − 0,50 (margen) = 6,93`; se deja en 6,5, holgura de regalo. El motivo de fondo: el coste no es simétrico — pasarse cuesta cero (muerde la ruta), quedarse corto cobra una práctica y culpa a Anthropic en el log. **Rescata a `T-093`:** si `read` es máximo por construcción, la tanda de 30-40 frases ya no sirve para ajustarlo — sirve para contestar si los 10 s de la ruta son el presupuesto correcto, la pregunta que `[A-011]` llevaba haciendo desde el 4 de agosto sin saber formular. `[L-058]` nueva: "el peor de N" es un suelo que crece, no un techo — lo transferible es que el hallazgo salió midiendo algo que no decidía nada (el trabajo local, sobra por 30×) y hubo que llevarlo a donde sí decidía. `[L-055]` AMPLIADA: decía "los punteros de LÍNEA se releen al final", y por ese hueco se coló un puntero **por nombre** — `measure_local_parts.py` citaba en presente `test_the_local_scale_uses_the_real_pool_size` como guardián vivo en el mismo commit que lo borraba; la regla ahora vale para cualquier puntero. `measure_local_parts.py` corregido en dos cabos más: el docstring llevaba números viejos (`TIMEOUT_SECONDS = 8.0` cuando ya vale 9,0; `56,3 ms` cuando la sexta tanda dio 62,4) y el puntero al test borrado — ahora la prosa no repite ningún número, dice que viven en `app/tools.py`. Suite: **427 passed**, sin tests tocados en este diff — mismo número medido sobre `c89ae1d` (el `428` que dejó anotado `[S-052]` no se reprodujo al remedir: puede ser un desliz de esa entrada, no hay evidencia de qué test cambió), confirmado corriendo `.venv/Scripts/python.exe -m pytest -q` en este cierre. Paso 2b confirmado: `compilar: 0`, `comparar: 0` no aplica — ningún `.ts`/`.js` en el diff. `tasks.md`: `T-093` **reformulada** — ya no es "cronometrar el peor caso de Opus 5", es "¿son 10 s el presupuesto correcto de la ruta?", con percentil y tasa de corte decididos ANTES de medir |
| **última sesión** | 2026-08-13 (trigésimo sexto tramo) |
| **siguiente acción** | `T-093` (reformulada): correr la tanda de 30-40 frases contra `claude-opus-5` para medir el p95 del tiempo de generación, **decidiendo ANTES** qué percentil se compra y qué tasa de corte se acepta (p95 = 1 de cada 20 cortada y cobrada) — sin decidirlo antes, se cae en `max(40)`, el mismo error de `[L-058]` con más muestras. Con p95 ~7 s los 10 s de la ruta valen y `[A-011]` se cierra; con p95 ~12 s el presupuesto de la ruta es el que está mal, no el reparto de fases. Quedan abiertas del paso 8: `T-081` (renombrar `request_sent`, aplazada a propósito), `T-089` (mensaje inseguro de `install.sh`), `T-090` (valorar si el paso 8 cierra del todo), `T-092` ya cerrada. ⚠️ Sigue pendiente `T-086` (hora de las lecturas de costo de AWS, paso 7) y `T-088` (comentario de `MODEL` en `check_api_key.py`, para cuando toque el paso 9) |
