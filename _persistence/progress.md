...
| | |
|---|---|
| **paso** | 🚀 **PASO 9 EN CURSO, observabilidad y evals con rúbrica — quincuagésimo primer tramo (`[S-068]`), 2026-08-18, después del cierre que dejó `[S-067]` (`8a7c44f`).** `[D-097]`: el etiquetado manual de las 60 frases vive en `_persistence/labels/`, HERMANA de `corpus/` y no dentro (vidas opuestas: `corpus/` guarda lo que ya no es producción, las etiquetas nacen contra la rúbrica viva), con portero propio (`labels.py`, `tests/test_labels.py`, 17 comprobaciones) desde el primer commit. Commit `206ca43`: esqueleto de 60 filas con `verdict: null`, `_persistence/labels/README.md`. **`T-106` CERRADA** en commit `28ed31b`: el humano etiquetó las 60 a mano — 27 `correct`, 33 `wrong`, 0 `unclear`, 0 notas. El portero cazó la fila 21 corrupta (`"I did not saw him yeshagmasterday"`) y se restauró el texto contra `SENTENCES` sin tocar el veredicto. `[D-098]`: la vara del etiquetado es el inglés escrito de libro, deducida de las etiquetas (22, 30, 54 salieron `wrong`), no preguntada; la 55 se revisó bajo esa vara y no cambió. `L-082` (citar un mecanismo por su nombre no comprueba su alcance) y `L-083` (ilustrar un criterio con frases del propio conjunto ancla las etiquetas) nuevas. **Suite: 551 passed**, según el mensaje de los dos commits — no se volvió a correr en este cierre. Paso 2b no aplica: `git status`/`git diff` de este cierre salen vacíos, ningún `.ts`/`.js` tocado hoy |
| **última sesión** | 2026-08-18 (quincuagésimo primer tramo, `[S-068]`) |
| **siguiente acción** | 🆕 **Cruzar las 60 etiquetas humanas (`_persistence/labels/sentence_labels.jsonl`) contra el veredicto real del juez, usando `app.tools.split_verdict` — sin tarea numerada todavía, nace de este cierre.** Sigue abierta `T-108` (rendija menor del portero de `_persistence/corpus/`, `*.jsonl` por `glob`). Sigue abierta `T-109` (dos corridas ENTERAS el mismo día se pisan). `T-110` candidata, no decidida: meter una huella de `rubric_check.py` en la FILA del corpus — la firma el humano. Siguen paradas con disparador `T-102` y `T-103`. ⚠️ Sigue pendiente `T-086` (hora de las lecturas de costo de AWS, paso 7) |
