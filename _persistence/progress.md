...
| | |
|---|---|
| **paso** | 7 de 9 — 🚀 **TEAPP en producción, con dos deudas de despliegue cerradas hoy.** **Decimocuarto tramo (`[S-031]`):** `T-050` (llave estable) y `T-051` (cookie `Secure`) pasan de 🔄 a ✅ — redespliegue real en la máquina (`git pull` `aff4350`→`0dfdbba`, `install.sh` código 0, huella del `.env` y `data/users/jorge.json` intactas) y cookie `Secure`/`HttpOnly`/`SameSite=Lax` medida en navegador real con F5 sin recredenciales. Ascienden `[A-005]` y `[A-008]` → `[L-032]`; `[A-009]` → `[L-031]`. **`[D-045]` nueva:** la EC2 deja de estar encendida de continuo — ventana de uso 12:00–23:00 UTC, apagado automático desde dentro de la máquina (temporizador `shutdown -P` ya armado para hoy 23:00 UTC, `MODE=poweroff` verificado), encendido manual; revoca el descarte de `[D-029]` y deja caducada `[D-044]`. `[A-018]` gana sexta lectura (`Costo Acumulado Mensual` 0,37 US$, `Importe utilizado` sigue 0,00) y corrección: son **tres** fuentes de gasto, no dos — falta el volumen EBS. `[A-017]` gana episodios 4 y 5: causa localizada en el **cliente**, no en DuckDNS (resolución partida entre `ssh`/`nslookup`/`curl` en el mismo minuto). ⚠️ **La máquina se apaga sola hoy a las 23:00 UTC** — mañana hay que encenderla a mano desde la consola antes de trabajar. Ningún archivo de `app/` ni `tests/` cambia de comportamiento hoy — los tocados (`app/config.py`, `app/sessions.py`, `deploy/install.sh`, `tests/*`) son solo comentarios re-etiquetando anclas retiradas. Suite no corrida hoy, sigue en 351 |
| **última sesión** | 2026-08-09 (decimocuarto tramo) |
| **siguiente acción** | Escribir la pieza de apagado automático desde dentro de la máquina que `[D-045]` exige (hoy solo hay un disparo único, no sobrevive a un `stop`/`start`). Medir el primer `stop`/`start` bajo esa ventana: marcador vivo, `200` sin tocar nada, certificado sin reemitir, estado `stopped` en consola. `T-066` (dos dispositivos a la vez) sigue pendiente y es la siguiente tarea natural. `T-067` se mide bajo la ventana de `[D-045]`, separando tres tarifas (Elastic IP, horas de instancia, volumen EBS). **Entrar por SSH usando la IP fija `32.199.55.191`**, no el nombre — el DNS falla intermitente y la causa está en el cliente |
