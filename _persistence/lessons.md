...
|---|---|---|---|
| L-088 | 2026-08-19 | 🎯 **Una vara que dos lectores humanos leen igual es la señal —disponible ANTES de pagar— de que no va a discriminar a un modelo grande.** Las 30 frases discriminantes de `T-112` se escribieron *raras de superficie pero inequívocas* (subjuntivos, inversiones, colectivos en singular, `information` incontable) y dieron **0 desacuerdos de 30** contra `claude-opus-5`: tramo `0-2` de `[D-104]`, **fracaso**. 🔑 **Lo que duele es que el aviso llegó gratis y una hora antes que la factura:** el cruce entre el reparto sellado y la etiqueta humana había dado **30 de 30**, o sea *«las frases son inequívocas para dos lectores independientes»* — y de ahí a que también lo sean para un modelo grande hay muy poco trecho. **Se leyó como *«la vara no es ambigua»* (buena noticia) y no como *«la vara no es difícil»* (la predicción del fracaso).** El mismo número decía las dos cosas. ⚠️ **La confusión de fondo: difícil-para-un-humano y difícil-para-el-juez no son la misma dificultad**, y `[D-101]` ya lo había dicho con otras palabras —*discriminante* busca dónde se rompe el corrector, no parecerse a lo que escribe quien practica—; se cumplió la letra (frases raras) y no el espíritu (frases que **rompan al juez**). 🧭 **La pista de dónde sí hubo movimiento, marcada como pista y NO como resultado porque no estaba sellada:** en el eje de la **forma**, `2 de 30` rompieron `too_many_sentences` contra `0 de 60` de la vara representativa, misma huella de rúbrica (`bbf4be38`). La próxima vara apunta ahí — **forma bajo carga** (frases largas, varios errores a la vez, ambigüedad de qué corregir primero), no rareza gramatical. ✅ **Y lo que SÍ funcionó, que es la mitad que se salvó:** los cinco tramos de `[D-104]` estaban escritos antes de correr, así que un `30 de 30` con la mecánica intacta —dos brazos poblados, cero ambigüedad, cruce limpio— **no pudo reportarse como *«el corrector acierta el 100 % incluso en frases difíciles»***, que es verdad literal y conclusión falsa. **Sin el sello, esta sesión habría cerrado celebrando.** | `[D-101]`, `[D-104]`, `[D-049]`, `[D-100]`, `[L-071]`, `[L-085]`, `T-112`, `measure_tutor.py`, `_persistence/replies/`, paso 9 |
...
| L-080 | 2026-08-18 | 🏷️ **Un aviso en la consola no protege un archivo: cuando el informe y el nombre se contradicen, el que sobrevive es el nombre.** `eval_rubric.py` sabía perfectamente que la tanda se había cortado —`report_lines` imprime *"faltan N respuestas... no valen como línea base"*— y aun así guardaba esas 30 filas en un archivo llamado `full`, porque el nombre se calculaba con el plan (`calls = len(plan)`) y no con lo que llegó. 🔑 **La forma general: la evidencia y su etiqueta tienen vidas distintas.** El informe dura lo que dura la ventana de la terminal; el nombre del archivo dura hasta que alguien lo divida dentro de seis semanas — así que **el aviso hay que ponerlo en la parte duradera**, no en la que se cierra sola. ⚠️ **Y es el mismo bicho que el cuarto eje se inventó para matar, con el disfraz al revés:** `[L-071]` era una selección de 10 fallos que leída entera daba `100%`; esto es una corrida cortada cuyo nombre promete línea base. 🔻 **Y la segunda mitad no era etiquetado sino pérdida:** `save_replies` abre en `"w"` y los otros tres ejes no cambian en el mismo día — una segunda corrida cortada **se llevaba por delante la línea base ya pagada**, que es `[L-076]` exacto **viviendo dentro del arreglo de `[L-076]`**. 📌 **Lo que enseña sobre los tests, y es la parte que más duele:** había un test llamado `test_a_partial_run_is_named_pick_not_full` y **el nombre bastaba para dejar de mirar**. Probaba `replies_file(10)` —una tanda que se **pidió** parcial—, nunca el camino de la que se cortó sola, porque ese número solo existe dentro de `main()`, y `main()` hace red. 🧭 **La regla que queda: un test cuyo nombre describe el riesgo y cuyo cuerpo no llega hasta él es peor que no tenerlo**, porque ocupa su sitio en la lista. Cazado por la terminal auditora, antes de pagar la corrida | `eval_rubric.py`, `tests/test_eval_rubric.py`, `[D-095]`, `[D-092]`, `[L-071]`, `[L-076]`, `PI-4`, auditoría externa del 2026-08-18 |
| L-079 | 2026-08-18 | ⏳ **Un criterio de conservación que se evalúa DESPUÉS deja la evidencia esperando en el sitio menos duradero, justo mientras se la considera "todavía no valiosa".** Propuse guardar un corpus *"cuando su rúbrica ya no exista en producción"*. Suena comprobable y lo es — **pero solo se puede comprobar más tarde**: en el momento de crear un corpus la rúbrica está viva **por definición**, porque acabas de correr con ella. 🔑 **Así que bajo ese criterio nada se guarda nunca al nacer, y la sala de espera era `data/`**: ignorado por Git, un solo disco, sin copia. El criterio protegía la evidencia **exactamente después** del periodo en que estaba expuesta. ⚠️ **Y el segundo defecto era de cobertura: nombré la rúbrica y olvidé el modelo.** `[D-049]` va a mover `MODEL` tres veces a propósito — un corpus de Opus 5 con la rúbrica intacta deja de ser repetible el día que se baje a Sonnet, y **es la línea base contra la que se mide el descenso**: el archivo que más duele perder era justo el que mi criterio dejaba fuera. 🧭 **Las dos reglas que quedan.** (1) **Un criterio de conservación se evalúa al CREAR, no al caducar** — si no, hay una ventana en la que el dato vale y nadie lo protege. (2) **El disparador se cuelga de un evento que ocurre seguro y se nota seguro** —el commit que cambia la configuración— **no de que alguien caiga en la cuenta**; es el patrón que `[D-081]` ya usaba y que yo no reconocí teniéndolo al lado. 📌 **La salida buena era más simple que mi criterio: los ejes del propio nombre.** No se estira, lo comprueba un programa, y cubre el modelo. Cazado por la terminal auditora | `eval_rubric.py`, `_persistence/corpus/`, `[D-092]`, `[D-093]`, `[D-049]`, `[D-081]`, `[L-071]`, `[L-076]`, `T-107`, auditoría externa del 2026-08-18 |
...
| L-069 | 2026-08-17 | 📅 **Un día entero inventado, escrito 40 veces en 6 archivos, y el testigo estaba a un comando.** Toda la sesión `[S-060]` se fechó **2026-08-15**; sus commits —`abd1a87`, `a54baa3`, `13d2d79`— son del **2026-08-14**, el último a las 15:16 -0500 (20:16 UTC, sigue siendo el 14). No es zona horaria y no es el reloj: el repo supervisor fechó bien el 16 desde la misma máquina. **Confirmado por el usuario: el 15 no se trabajó.** 🔑 **Lo que enseña no es el error, es la PROPAGACIÓN:** el 15 se coló en `progress.md`, `tasks.md`, `decisions.md`, `lessons.md` y **las dos skills del protocolo**, y de ahí a cinco lecciones —`[L-062]`–`[L-067]`— que existen precisamente para cazar afirmaciones que nadie fue a apagar. **Una fecha inventada dentro de una lección sobre datos caducados.** ⚠️ **Y es de la clase muda:** ninguna herramienta valida una fecha en prosa, no rompe ningún test y se lee natural — solo se cae al cruzarla con `git log`, que es lo que nadie hizo en cinco tramos. 🧭 **Regla: la fecha que entra en `_persistence/` se copia de `git log`, no de la idea que uno tenga de qué día es.** Es `[L-063]` aplicado al calendario: con cita se está leyendo, sin cita se está recordando — y la fecha se recuerda igual de mal que un número. 🚨 **Segunda mitad, y es la cara:** al corregirlo se cayó también el remedio. `[L-067]` y `protocol-close` mandaban comprobar *"si la fecha de la última fila no es la de hoy, falta la entrada"* — y **ese criterio habría dado VERDE en el caso exacto que lo originó**, porque hubo cinco cierres el mismo 14 y la fila de arriba ya llevaba la fecha de hoy siendo de otra sesión. **Cambiado a comparar el ID**, que es lo único que distingue dos tramos del mismo día. 📌 Una regla que no caza su propio caso es una nota — misma familia que `[L-068]` el mismo día. 🔻 **TERCERA MITAD (2026-08-17): la regla duró 4 h 47 min.** Entró al repo en `41c753f` (**10:54:25**) y `25332da` (**15:41:03**) escribió **16** apariciones de `2026-08-18` de golpe —`decisions.md` ×6, `lessons.md` ×3 con el título de `[L-074]` dentro, `progress.md` ×3, `english_tutor.py` ×2, `trace.py`, `test_trace.py`— más **1 en `Edu_TripleS/PROGRESO.md`**: **17 en total**. 🔑 **Y no hizo falta `git log` para cazarla: el mensaje de `41c753f` ya decía "auditoría del 2026-08-17"** — el dato bueno y la lección estaban los dos escritos, y ninguno se cruzó con lo que se escribía. 🧭 **Corrección a la regla: *"se copia de `git log`"* dice de dónde y NO dice cuándo**, y una instrucción sin momento se cumple cuando uno se acuerda — el momento es **al abrir el archivo para escribir**, no al cerrar, que es cuando ya está copiada a seis sitios. 🚨 **Segunda cara, de otro mecanismo: el conteo que corrige y pierde.** Se reportó *"son 16, no 13"* con tabla por archivo — cierto dentro de TEAPP y **perdió la única que estaba fuera**, la del supervisor, que el auditor había listado con ruta y línea. **Subir un número no es cubrir un conjunto.** 🔑 **La fecha se coló por no dar error; ésta por venir DENTRO de un arreglo** — un dato que acaba de mejorar deja de auditarse, y la tabla desglosada hace el trabajo de parecer exhaustiva. Es `[L-070]` en un conteo en vez de en un puntero, y `LM.15` con otra piel: allí el silencio se leyó como confirmación, aquí *"16, no 13"* como cobertura completa. 🧭 **Un conteo se publica con su ÁMBITO al lado, y si corrige otro, con la diferencia de ámbito dicha.** 📌 Sin culpa de nadie: **`Edu_TripleS` y `Test_Edu_TripleS` son HERMANAS**, ninguna terminal cruza ese límite por costumbre, y la fecha sí lo cruzó | `[L-063]`, `[L-067]`, `[L-062]`, `[L-066]`, `[L-068]`, `[L-070]`, `[L-074]`, `[D-088]`, `LM.15`, `[S-060]`, `progress.md`, `tasks.md`, `decisions.md`, `protocol-close`, `protocol-start`, `git log`, `41c753f`, `25332da`, auditoría externa del 2026-08-17 |
...
- **🔻 Y el daño se cobró.** Las dos predicciones salieron `56/58` por aritmética y
  ...
  (`[L-086]`).
- **🚨 Tercera pieza, corregida el mismo día y la más barata de arreglar: NINGUNA
  de las dos predicciones llegó a disco.**
  - El `54` **sí lo escribió el usuario**, en la terminal auditora, y esta terminal
    no puede leer aquella — así que no era un número inventado. El error fue
    **atribuirlo a la terminal ejecutora**, que efectivamente nunca lo escribió.
    Corregido aquí para que el registro no arrastre la versión fuerte.
  - El `56` de la auditoría tampoco: vivía en el chat, con su marca de anclaje, y
    en el disco no.
  - 🔑 **Un sello que vive en una conversación no es un sello.** Se sellaron las
    reglas del cruce (`[D-100]`) y **no los números que esas reglas existían para
    proteger**. El protocolo se ejecutó por la mitad sin que nadie lo notara,
    porque la mitad que faltaba era la que no tenía archivo.
...
- **Qué pasó:** al explicarle al humano cuándo marcar `unclear`, esta terminal usó
  como ejemplos las frases **54 y 55 de las 60 que él iba a etiquetar** — dijo de
  la 55 que era *"el caso puro"*. Cuando etiquetó, esas dos filas ya venían con una
  opinión ajena encima.
- **Por qué pasó:** los ejemplos buenos se buscan donde están los datos, porque son
  concretos y están a mano. 🔑 **Y no se siente como contaminar: se siente como
  enseñar bien.** El cuidado se había puesto entero en el archivo —portero, campos
  cerrados, `note` declarada como no auditada— y la opinión entró por el canal que
  no tiene portero, que es la conversación.
- **Qué se hace distinto:** para ilustrar un criterio de etiquetado, **inventar las
  frases del ejemplo**, nunca sacarlas del conjunto que se va a juzgar. Cuesta lo
  mismo. ⚠️ Y cuando ya se hizo, **decirlo en voz alta al revisar** en vez de
  presentar la discrepancia como hallazgo limpio: una fila anclada no se distingue
  después de una fila decidida.

### [L-082] 2026-08-18 — Citar un mecanismo por su nombre no comprueba su alcance

- **Qué pasó:** al preparar `[D-097]` se apoyaron dos afirmaciones en mecanismos
  que ya existen, y las dos eran falsas al abrirlos. Esta terminal dijo que
  `T-108` bloqueaba la decisión porque su portero dejaría pasar el archivo de
  etiquetas; pero `T-108` endurece `CORPUS_DIR.glob("*.jsonl")`, y las etiquetas
  van a `_persistence/labels/`, que ese glob **no mira esté roto o arreglado**. Y
  la terminal auditora pidió reutilizar el lector de `OK`/`FIX` de
  `app/rubric_check.py`; pero ese es `learner_message`, cuyo docstring dice que
  devuelve **si** la primera línea traía palabra clave y **descarta cuál era** —
  el dato que hacía falta. El bueno era `app.tools.split_verdict`.
- **Por qué pasó:** el nombre de un mecanismo describe su tema, no su alcance.
  `check_reply`, `learner_message`, "el portero de `corpus/`" — los tres suenan a
  que cubren lo que uno quiere que cubran. 🔑 **Y una frase de la forma "esto ya
  lo hace X" se audita sola en la cabeza de quien la oye**: nombra código real,
  que existe y funciona, así que no dispara la sospecha que dispararía un dato
  inventado.
- **Qué se hace distinto:** antes de escribir "se reutiliza X" o "X ya lo cubre",
  **abrir X y leer qué recibe y qué devuelve** — igual que `CLAUDE.md` exige abrir
  la entrada antes de citarla. ⚠️ Aquí las dos se cazaron **antes** de escribir
  código porque se fue a mirar el disco; el precio de no mirarlo habría sido un
  esquema construido contra la puerta equivocada, y en verde.

### [L-081] 2026-08-18 — La huella sella la pregunta, no la báscula que responde

- **Qué pasó:** el corpus congelado del 17 guarda `10` filas rotas. Pasado por el
  `rubric_check` de hoy da `1`. Nueve discrepancias, todas `too_many_sentences`.
- **Por qué pasó:** `rubric_fingerprint()` hashea `tools.GRAMMAR_RUBRIC` y nada
  más. Eso identifica **lo que se le pidió al modelo**, no **cómo se corrige la
  respuesta**. Los cuatro ejes del nombre —modelo, fecha, huella, `full`/`pick`—
  dejan `rubric_check.py` entero fuera del sello.
  ⚠️ **El caso concreto que lo destapó sí estaba cubierto:** cambió
  `MAX_SENTENCES`, que vive dentro de la rúbrica, así que la huella se movió y
  avisó. El agujero es real igualmente, pero se demuestra leyendo la función —no
  con ese ejemplo—: un cambio que viva **solo** en `rubric_check.py` no la mueve.
- **Qué se hace distinto:** cuando un campo guardado es el **veredicto de un
  programa** y no un dato crudo, la identidad del corpus tiene que incluir **ese
  programa**. Y al reportar: separar *"la deriva ocurre"* de *"nada la detecta"*,
  porque un ejemplo cubierto contado como prueba da por medido lo que solo se ha
  nombrado.

### [L-080] 2026-08-18 — El aviso vivía en la consola y la mentira vivía en el disco

- **Qué pasó:** `eval_rubric.py` nombraba el archivo de respuestas con el número
  de frases **planeadas**, no con las que llegaron. Como el bucle tiene dos
  `break` previstos, una tanda de 60 podía escribir 30 filas en un archivo
  llamado `..._full.jsonl`. El informe **sí** avisaba de que faltaban 30.
- **Por qué pasó:** el aviso y la etiqueta tienen vidas distintas. El informe se
  muere al cerrar la terminal; el nombre del archivo sobrevive hasta que alguien
  lo divida meses después. **El aviso estaba en la parte que se borra sola.** Y
  como `save_replies` abre en `"w"` y los otros tres ejes no cambian dentro del
  mismo día, una segunda corrida cortada borraba la línea base ya pagada.
- **Qué se hace distinto:** la etiqueta se calcula sobre **lo que se obtuvo**,
  nunca sobre lo que se pidió. Y en los tests: **un nombre que describe el riesgo
  no es prueba de que el cuerpo llegue hasta él** — aquí había un
  `test_a_partial_run_is_named_pick_not_full` que solo probaba la tanda pedida
  parcial, y su nombre bastó para que nadie mirara el otro camino durante un día.

### [L-079] 2026-08-18 — Un criterio de conservación que se evalúa después deja la evidencia esperando en el sitio menos duradero

- **Qué pasó:** propuse guardar un corpus del eval *"cuando su rúbrica ya no exista
  en producción"*. Es comprobable, y por eso lo di por bueno. La terminal auditora
  lo desarmó por dos sitios.
- **Por qué pasó:** el criterio **solo se puede evaluar más tarde**. Al crear un
  corpus la rúbrica está viva por definición —acabas de correr con ella—, así que
  nada se guarda nunca al nacer; se guardaría cuando alguien cayera en que aquella
  rúbrica murió. Y mientras tanto la evidencia espera en `data/`: ignorado por Git,
  un solo disco, sin copia. **El criterio protegía el dato exactamente después del
  periodo en que estaba expuesto.** El segundo defecto era de cobertura: nombré la
  rúbrica y olvidé el modelo, que es el eje que `[D-049]` va a mover tres veces —y
  el corpus que más duele perder, la línea base del descenso, era justo el que mi
  criterio dejaba fuera.
- **Qué se hace distinto:**
  - **Un criterio de conservación se evalúa al CREAR, no al caducar.** Si solo se
    puede evaluar después, hay una ventana en la que el dato ya vale y nadie lo
    protege.
  - **El disparador cuelga de un evento que ocurre seguro y se nota seguro** —aquí,
    el commit que cambia `MODEL` o `GRAMMAR_RUBRIC`—, nunca de que alguien caiga en
    la cuenta. Es el patrón que `[D-081]` ya usaba, y no lo reconocí teniéndolo al
    lado.
  - **Antes de inventar un criterio, mirar si el artefacto ya lo lleva encima.** La
    salida buena era más simple que la mía: los ejes del propio nombre.
...
- **Qué pasó.** `_leaks_keyword` (`app/rubric_check.py`) comprueba si al alumno se
  le escapó la palabra clave del programa. Su docstring decía, con estas palabras:

  > *"Sin mayúsculas, el `ok` de "that's ok" sería un fallo y no lo es — la
  > rúbrica prohíbe la palabra clave del programa, no la palabra inglesa."*

  Y la línea de debajo era `{word.strip(...).upper() for word in message.split()}`:
  subía **todo** a mayúsculas, o sea hacía exactamente lo que el docstring acababa
  de prohibir. Resultado: *"That's ok. Well done."* —el tono cálido que la rúbrica
  **pide**— salía marcado como fallo.

- **Por qué pasó.** Las dos cosas se escribieron en el mismo minuto y **la
  explicación salió bien**. No fue no entender la regla: la regla estaba escrita
  entera, dos líneas más arriba. 🔑 **Escribir la razón se siente como haberla
  implementado.** El docstring cierra el asunto en la cabeza de quien lo escribe, y
  con esa sensación de cierre puesta, la línea de código se teclea en automático.

- ⚠️ **Y es de la clase muda por partida doble.** El corrector seguía devolviendo
  un conjunto plausible, el informe habría salido con números creíbles, y
  **releer la función lo habría confirmado**: quien la relee lee primero la
  explicación correcta y llega al código dándolo por bueno. **La prosa buena
  blinda al código malo que tiene debajo.**

- 🧭 **Regla: cuando un docstring dice "aquí NO se hace X", el caso de X se
  escribe como test, y antes de correr nada.** La frase del docstring **es** el
  enunciado del test. Sin test, es una intención.

- 📌 **Pariente de `[L-074]`, y complementaria — no la misma.** Allí dos frases
  **ciertas** llevaron a conclusiones falsas porque describían un estado anterior:
  había que actualizar la prosa. Aquí la frase es cierta, **sigue siéndolo**, y el
  fallo es que **el código no la obedece**. En una sobraba prosa vieja; en esta
  sobraba confianza en prosa nueva.

- 🪞 **La misma forma otra vez, dos horas después, en el sabotaje.** Para ver
  morder el freno de ASCII de `[L-001]` se metió la palabra *"medicion"* en una
  línea impresa — **sin tilde**. El sabotaje salió **verde**, y por un momento eso
  se leyó como *"el freno no muerde"*, cuando lo que no mordía era la prueba.
  Repetido con tilde de verdad: `UnicodeEncodeError`. 🔑 **Un sabotaje que no rompe
  nada no es un sabotaje, igual que un docstring que no se comprueba no es un
  freno.** En los dos casos la intención era correcta y el acto no la cumplía.

- **A raíz de:** un rojo en `tests/test_rubric_check.py` al construir el eval de la
  forma, 2026-08-17.
- **Relacionadas:** `[D-089]`, `[L-074]`, `[L-001]`, `[D-086]`, `[L-048]`,
  `app/rubric_check.py`, `PI-4`, `PI-6`.

---

### [L-074] 2026-08-17 — La caja que se describe más pequeña de lo que es, y la propuesta que se lee en un tope

**Dos veces el mismo día, y las dos veces una FRASE CIERTA llevó a una
conclusión falsa.** No hay ningún dato equivocado en ninguna de las dos.

---

#### 1. El docstring que se quedó pequeño

`TutorReply` decía *"lo que el tutor contesta, **en tres piezas separadas**"*.
Tenía **cinco** campos. La frase fue cierta y dejó de serlo sin que nadie la
tocara: `correct` entró el 2026-08-17, y antes `score` y `practice`.

🔑 **El daño no fue el número mal.** Fue que **construí un argumento encima**: me
opuse a meter `model_seconds` en la clase porque *"le cambiaría el significado, de
qué dijo el tutor a qué dijo y cuánto tardó"*. Sonaba a principio. Era falso —
**tres de los cinco campos ya no venían del juez**: `words` lo cuenta `respond` en
local, `score` y `practice` salen del archivo de contadores. La clase dejó de ser
*"el veredicto"* en `[D-066]` y pasó a ser *"lo que la ruta necesita saber de esta
práctica"*, donde un reloj encaja perfectamente.

🚨 **Y la propuesta cara salió de ahí:** para no *"contaminar"* la caja, propuse
meter el cronómetro dentro de `judge_grammar` y hacer subir el dato tres pisos —
`GrammarVerdict` incluido. Sobraba: `respond` ya tiene la llamada al modelo
aislada en su propia línea, y un `perf_counter` alrededor de esa línea mide lo
mismo. **Defender una pureza inexistente costó una propuesta de tres módulos donde
cabía una de uno.**

📌 **Y estaba en el archivo de al lado también:** la cabecera de
`tests/test_trace.py` enumeraba *"que apunte, que añada, y que NO guarde la
frase"* — **tres de cinco tests**. Encontrada por buscarla, no por casualidad.

---

#### 2. El tope que se lee como medida

`app/tools.py` reparte `connect`/`write`/`pool`/`read`, y `[D-085]` escribió: *"la
arquitectura ya piensa en fases; el registro no las escribe"*.

**Cierto palabra por palabra.** Y de ahí salió la propuesta de guardar esas cuatro
fases en la traza — que **no se puede hacer**: son un presupuesto que se le
*entrega* a la librería, no un cronómetro. `httpx 0.28.1` y `httpx2 2.9.1` dan
**un solo `elapsed`** cada una, el total de la respuesta.

🔑 **La frase no dice que existan. Dice que no están escritos, que es lo que uno
dice de algo que existe.** *"El registro no las escribe"* describe igual de bien
un dato disponible que nadie apunta y un dato que no existe — y el lector elige
la primera, porque es la única que deja algo que hacer.

---

#### 🧭 Las dos reglas

1. **Una descripción que enumera se actualiza con lo que enumera, o no enumera.**
   Un docstring que dice *"tres piezas"* no es una errata cuando hay cinco: es un
   argumento esperando a que alguien lo use.
2. **Una afirmación sobre lo que NO se ha hecho tiene que decir si se PUEDE
   hacer.** *"No está escrito"* y *"no existe"* se leen igual y llevan a sitios
   opuestos.

⚠️ **Y cómo se corrige importa:** la frase de `[D-085]` se **sustituyó en su
sitio**, no se matizó debajo. **Una regla con un asterisco debajo se sigue leyendo
como regla** — y quien la leyó y propuso las cuatro fases no habría bajado a leer
el asterisco.

📌 **Emparenta con `[L-069]`** —la prosa copiada que ninguna herramienta valida—
pero es peor de cazar: allí el dato viajado era **falso**, y aquí **los dos datos
son verdaderos.** No hay nada que un `grep` pueda encontrar.

🪞 **Y la simetría, que es lo transferible:** en las dos, la frase describía un
estado **anterior** que se había quedado escrito. `TutorReply` sí tuvo tres
piezas. `tools.py` sí es lo único parecido a fases que hay. **Ninguna de las dos
mintió nunca; las dos dejaron de ser toda la verdad, y ninguna avisó.**

- **Relacionadas:** `[D-087]`, `[D-085]`, `[D-066]`, `[L-069]`, `[L-073]`,
  `[L-070]`, `app/english_tutor.py`, `app/trace.py`, `tests/test_trace.py`,
  auditoría externa del 2026-08-17.
...
- 🔻 **TERCERA MITAD, añadida el 2026-08-17: la regla se escribió por la mañana y
  se rompió por la tarde, en el mismo repo y el mismo día.** No es una recaída a
  las semanas. Los números, sacados de `git log` y no de la memoria:

  | qué | cuándo | commit |
  |---|---|---|
  | entra esta lección al repo | **10:54:25 -0500** | `41c753f` |
  | entran **16** apariciones de `2026-08-18` | **15:41:03 -0500** | `25332da` |

  **4 h 47 min de vida tenía la regla.** Y las 16 no se fueron filtrando: entraron
  **todas en un solo commit** —`decisions.md` ×6, `lessons.md` ×3 (incluido el
  título de `[L-074]`), `progress.md` ×3, `app/english_tutor.py` ×2, `app/trace.py`,
  `tests/test_trace.py`—, más **1 en `Edu_TripleS/PROGRESO.md`**, del otro lado.
  **Total 17.**

  > 🔑 **Y la fecha buena ya estaba en el repo, escrita por nosotros.** El mensaje
  > de `41c753f` dice *"auditoría del 2026-08-17"*. No faltaba el dato ni faltaba
  > la lección: **las dos estaban escritas y ninguna se cruzó con lo que se
  > escribía.** Eso es lo que esta mitad añade — la de arriba decía *"solo se cae
  > al cruzarla con `git log`"* y era optimista: aquí no hizo falta `git log`,
  > bastaba con leer el commit de la mañana.

- 🧭 **Y de ahí la corrección a la regla, porque la de arriba no bastó:** *"se
  copia de `git log`"* describe de dónde sacar la fecha, y **no dice cuándo**. Una
  instrucción sin momento se cumple cuando uno se acuerda, que es la definición de
  no cumplirse. **El momento es al abrir un archivo de `_persistence/` para
  escribir, antes de teclear la fecha** — no al cerrar, que es cuando ya está
  copiada a seis sitios.

- 🚨 **La segunda cara del día, y es de otro mecanismo — no la caza la regla de
  arriba ni la nueva.** Al reportar el arreglo se dijo *"son 16, no 13"*, con
  tabla por archivo. **Cierto dentro de TEAPP, y perdió la única que estaba
  fuera:** la del repo supervisor, que el auditor había listado con ruta y línea
  en el mensaje anterior. Sus 13 eran 12 de TEAPP + 1 de allí; los 16 eran TEAPP y
  nada más. **Subir un número no es cubrir un conjunto**, y cambiar el universo
  del conteo sin decirlo convierte una corrección en una pérdida.

  > 🔑 **Por qué se colgó de una corrección y no de un descuido: un dato que acaba
  > de mejorar deja de auditarse.** La fecha se coló por **no dar error**; esta se
  > coló por **venir dentro de un arreglo, con una tabla desglosada que hace el
  > trabajo de parecer exhaustiva**. Es `[L-070]` (*"el arreglo que sale de una
  > auditoría no está auditado"*) en un conteo en vez de en un puntero, y `LM.15`
  > con otra piel: allí el silencio se leyó como confirmación, aquí *"16, no 13"*
  > se leyó como cobertura completa.

  🧭 **Regla: un conteo se publica con su ÁMBITO escrito al lado**, y si corrige
  otro conteo, **con la diferencia de ámbito dicha**. 📌 Y el reparto lo explica
  sin culpa de nadie: cada terminal barrió su casa. **`Edu_TripleS` no está dentro
  de `Test_Edu_TripleS` — son hermanas** en `Company_TripleS/`, y ninguna de las
  dos cruza ese límite por costumbre. **La fecha sí lo cruzó, porque la escriben
  los dos.**

- **A raíz de:** auditoría externa del 2026-08-17, hallazgo H-2, confirmado por
  el usuario. Tercera mitad: reincidencia del mismo día, con el conteo corto
  cazado por la auditoría el 2026-08-17.
- **Relacionadas:** `[L-063]`, `[L-067]`, `[L-062]`, `[L-066]`, `[L-068]`,
  `[L-070]`, `[L-074]`, `[D-088]`, `LM.15`, `[S-060]`, `protocol-close`,
  `protocol-start`, `41c753f`, `25332da`.
