...
### [A-024] 2026-08-10 — La llave de la API de Anthropic no tiene tope de gasto

- **Se supone que:** la llave creada en `T-075` corre **sin ningún límite de
  gasto ni alerta de uso**. Nadie ha entrado a la consola de Anthropic a mirarlo;
  se supone lo malo por defecto, que es la regla 3 aplicada al dinero.
- **Cómo se comprobaría:** entrar a la consola de Anthropic y ver si la llave (o
  su organización) admite un límite de gasto o una alerta. 🚨 **Y hacerlo ANTES
  de `T-079`**, que es la tarea que empieza a llamar de verdad y en bucle para
  medir `[A-010]` y `[A-011]`. Si admite tope, se pone; si no, `T-079` se corre
  con un contador de llamadas y un corte duro escrito en el propio guion.
- **Si es falsa:** nada — sería la buena noticia. El riesgo vive en que sea
  **cierta**: un bucle de medición con un fallo tonto —un `while` que no sale, un
  reintento mal puesto— llama sin techo y **sin aviso**.

🔑 **La asimetría es el hallazgo.** La cuenta de AWS tiene alarma (`[A-018]`, con
su propia duda sobre si avisa a tiempo). La llave de Anthropic no tiene nada. Un
freno que no existe no es un freno flojo: es que **no está**, y esa diferencia se
lee mal desde dentro porque el proyecto ya "tiene una alarma".

⚠️ **Cuatro bolsillos, y no se mezclan.** `[A-018]` ya se rompió una vez el día
que la EC2 se sumó a la IP y la aritmética dejó de ser limpia:

| qué | dónde se mira | freno |
|---|---|---|
| IP elástica + EC2 + volumen EBS | consola de AWS | `[A-018]`, con alarma |
| **llave de la API de Anthropic** | consola de Anthropic | 💥 ninguno conocido |
| suscripción de Claude Code | `claude.ai/settings/usage` | el del plan |

📌 **La suscripción de Claude Code es el TALLER, no la obra.** El aviso de límite
mensual que cortó un cierre el 2026-08-10 salió de ahí: **no entra** en `[A-018]`,
ni en `[T-067]`, ni en el presupuesto del proyecto. Se anota aquí solo para que
nadie vuelva a sumarlo donde no va.

📌 Señalado por auditoría externa, no por el proyecto.
