...
| `~~D-nnn~~` ⛔ **RENUNCIADA** | no se ejecutó y ya no se va a ejecutar; el alcance la dejó fuera | `~~D-049~~` → `[D-105]` |
...
🔴 **La tercera marca nació el 2026-08-19, y nació porque las dos primeras
habrían mentido.** `[D-105]` renuncia a `[D-049]`: no la superó otra decisión
—sus números siguen siendo los del código— y desde luego no se cumplió. Marcarla
✅ **CUMPLIDA** habría escrito en el archivo que el descenso de modelo se hizo.
Una renuncia es un tercer desenlace y necesitaba su propia palabra.
...
|---|---|---|---|
| D-105 | 2026-08-19 | 🏁 **PASO 9 CERRADO, y con él TEAPP terminada. Las dos condiciones de `[D-103]` quedan cumplidas y se RENUNCIA a `[D-049]`: el descenso a Sonnet 5 y Haiku 4.5 no se hace, ni hoy ni después.** Firmado por el usuario. **(1) Condición 1 —vara discriminante escrita, etiquetada y corrida— CUMPLIDA:** `T-112` se escribió (`SENTENCES[61..90]`), se etiquetó a mano y se corrió (`$0,1026`). Salió **fallida** —0 desacuerdos de 30, tramo `0-2` de `[D-104]`— y el aprendizaje quedó en `[L-088]`. 🔑 **El ejercicio se hizo y el resultado fue un fracaso DOCUMENTADO, que no es lo mismo que no haberlo hecho.** La condición pedía una vara escrita, etiquetada y corrida, y las tres ocurrieron; no pedía que mordiera. No se repite. **(2) Condición 2 —traza vista escribir con servidor levantado y modelo real— CUMPLIDA el 2026-08-19 a las 18:16 UTC**, en la EC2 real, entrando por `https://teapp.duckdns.org/` desde el navegador. ⚠️ **Antes hubo que desplegar, y eso casi se pasa por alto:** la máquina corría `699f2b2` (2026-08-13) y `app/trace.py` nació en `c292210` (2026-08-17) — o sea que **el código desplegado era anterior a la traza, y practicar allí no habría escrito nada**. `git pull` + `systemctl restart teapp` (40 commits; `requirements.txt` y `.env.example` sin cambios desde el despliegue, así que no hizo falta reinstalar). 🔑 *"El servidor está arriba"* era cierto y aun así la prueba habría salido vacía: la afirmación no había caducado, es que nunca dijo lo que parecía decir. 📏 **La línea, entera y sin recortar:** `{"at": "2026-08-19T18:16:26.397065+00:00", "broken": [], "model": "claude-opus-5", "model_seconds": 3.468, "outcome": "correct", "practice": 9, "queue_seconds": 0.0, "rest_seconds": 0.002, "score": 6, "seconds": 3.47, "user": "jorge", "words": 9}`. ✅ **Lo que demuestra, y son tres cosas. (a) `PI-8` y `[D-085]` se cumplen en PRODUCCIÓN, no en un test:** doce campos y **ninguno contiene la frase** —dice `words: 9`, nunca las nueve palabras—. Eso solo se podía creer mirando datos reales dentro. **(b) La fila no se contradice a sí misma:** `rest_seconds` no se recibe, se calcula, y `3,47 − 0,0 − 3,468 = 0,002` cuadra. **(c) 💰 Medición nueva, con corrida detrás: de los 3,47 s de la práctica, 3,468 son el modelo. El código propio entero —cuota, marcador, escritura en disco— tarda 2 ms, el 0,06%.** Optimizar código nuestro no compraría nada: toda la espera es el modelo. Y deja holgado el freno de `[A-011]`, 3,5 s contra un techo de 10. 🚨 **POR QUÉ SE RENUNCIA, y esta es la mitad que no se puede resumir sin estropearla: TEAPP es EDUCATIVA, no comercial.** Existe para que quien la construye aprenda cómo se hace una aplicación con IA de punta a punta, y ese recorrido está hecho: los diez pasos del roadmap se caminaron. No se vende, no tiene clientes, no tiene usuarios fuera de su autor. Por tanto **no existe la presión que justifica el descenso de modelo**: ni factura que optimizar a escala, ni márgenes, ni regresión en producción que vigilar. Comparar tres modelos sobre una vara saturada enseñaría hoy menos de lo que cuesta, y **el aprendizaje central ya está cobrado en `[L-088]`** por `$0,1026`. ⛔ **LO QUE HAY QUE DEJAR ESCRITO SIN AMBIGÜEDAD, porque es lo único que impide que esta entrada se lea al revés dentro de seis meses: esta decisión es válida PORQUE el proyecto es educativo. En una aplicación COMERCIAL, lo que hoy se decide no hacer SÍ habría que hacerlo — y no como buena práctica recomendable, sino como parte del trabajo.** En concreto sería obligatorio: **(i)** construir una vara que discrimine ANTES de cambiar el modelo — un eval saturado da 100 antes y 100 después, así que no es freno sino adorno; era exactamente lo que `[D-049]` y `[D-101]` exigían, y la exigencia era correcta; **(ii)** medir cada modelo candidato en las DOS dimensiones por separado, la FORMA (¿obedece el contrato de salida?) y el VEREDICTO (¿acierta?), que se rompen por caminos distintos y se arreglan en direcciones opuestas —una apunta a la rúbrica, la otra al modelo—; **(iii)** distinguir los dos errores en vez de promediarlos: *corregir de más* molesta y *perdonar* enseña mal, caen dentro de la misma tasa de acierto y no valen lo mismo (por eso `cross_check.py` saca una tabla 2×2 y no un número — eso se conserva y está bien hecho); **(iv)** medir el coste real de cada modelo antes de elegir, no deducirlo de la lista de precios (`COST_PER_CALL_USD` de este repo está MEDIDO y ya caducó una vez, `[D-096]`); **(v)** tener una regresión que corra sola en cada cambio de modelo. 🔑 **El riesgo concreto que esto conjura:** que alguien —incluido el propio autor— lea *"el paso 9 se cerró sin comparar modelos"* y concluya que comparar modelos es prescindible. **No lo es. Se saltó porque este proyecto no lo necesita, y la diferencia entre las dos cosas es el asunto entero.** 🔴 **DEUDA QUE QUEDA VIVA Y DORMIDA, hallada al preparar esta decisión: hay DOS constantes de modelo y solo UNA está guardada.** `app/tools.py:41` (`MODEL_NAME`, la de la app) y `deploy/check_api_key.py:104` (`MODEL`, la del portero). `tests/test_check_api_key.py:91` clava el par `(check_api_key.MODEL, LAB_REQUESTS_PER_MINUTE)`: clava la **segunda**. Ningún test clava la primera — `tests/test_tools.py:145`, `tests/test_trace.py:56` y `tests/test_eval_rubric.py:516` la **derivan**, así que siguen verdes con cualquier valor. **Consecuencia: cambiar el modelo de la app no pone ningún test en rojo; es un cambio MUDO.** Y tocar `app/tools.py` sin tocar `check_api_key.py` dejaría al portero validando contra el límite de Opus mientras la app corre otra cosa — `T-088` reentrando por otra puerta. ⚠️ **Al renunciar a `[D-049]` esta deuda queda DORMIDA, no resuelta.** Se registra a propósito: es la trampa que se armaría sola el día que alguien tocara el modelo. En un producto comercial se arregla antes de cerrar; aquí se deja escrita y se deja quieta. 🔻 **Lo que sigue fuera, igual que ya lo estaba por `[D-103]`:** `T-103`, `T-108` y `T-081`. Deuda pequeña y real; no deciden el cierre. **Contra:** reescribir la vara apuntando a **forma bajo carga**, según el diagnóstico de `[L-088]` (descartado por el usuario: repetir el ejercicio compra una lección marginal más pequeña por el mismo dinero); **correr las 90 frases YA etiquetadas sobre Haiku 4.5 y Sonnet 5, sin escribir ni una frase nueva** —propuesto en esta sesión con el argumento de que la saturación de `[L-086]` se midió **solo con Opus**, que *"Opus saca 90/90 luego Haiku sacará 90/90"* es una suposición y no una medición, y con techo de gasto derivado `< $0,62`— (descartado por el usuario por alcance: el proyecto es educativo y ya cumplió su fin); cerrar el paso 9 también sin `T-102` (descartado por el usuario: cuesta `~$0,01` y es la mitad que da nombre al paso) | `_context/roadmap.md`, `app/trace.py`, `app/tools.py:41`, `deploy/check_api_key.py:104`, `tests/test_check_api_key.py:91`, `~~D-049~~`, `[D-085]`, `[D-096]`, `[D-101]`, `[D-103]`, `[D-104]`, `[L-086]`, `[L-088]`, `[A-031]`, `T-081`, `T-088`, `T-102`, `T-103`, `T-108`, `T-112`, `PI-4`, `PI-8`, paso 9, 🏁 **cierre del proyecto** |
...
### [D-105] 2026-08-19 — 🏁 Paso 9 cerrado y TEAPP terminada: se renuncia al descenso de modelo porque el proyecto es educativo

- **Se eligió:** dar por **cumplidas las dos condiciones de `[D-103]`**, cerrar el
  paso 9 y con él el proyecto, y **renunciar a `[D-049]`** — el descenso a
  Sonnet 5 y Haiku 4.5 no se hace, ni hoy ni después. No se reescribe la vara
  discriminante y no se comparan modelos. TEAPP queda terminada en
  `claude-opus-5`. Firmado por el usuario.

#### Condición 1 — la vara discriminante: CUMPLIDA, con resultado fallido

`T-112` se escribió (`measure_tutor.SENTENCES[61..90]`), se etiquetó a mano y se
corrió por `$0,1026`. Salió **fallida**: 0 desacuerdos de 30, el tramo `0-2` que
`[D-104]` había rotulado de antemano como fracaso. El aprendizaje quedó en
`[L-088]`.

> 🔑 **El ejercicio se hizo, y el resultado fue un fracaso DOCUMENTADO — que no
> es lo mismo que no haberlo hecho.** La condición pedía una vara *escrita,
> etiquetada y corrida*, y las tres cosas ocurrieron. No pedía que mordiera.

Se cierra sin repetir.

#### Condición 2 — la traza en producción: CUMPLIDA el 2026-08-19, 18:16 UTC

Vista escribir en la EC2 real, entrando por `https://teapp.duckdns.org/` desde
el navegador, con una llamada de verdad a `claude-opus-5`.

⚠️ **Antes hubo que desplegar, y eso casi se pasa por alto.** La máquina corría
`699f2b2` (2026-08-13) y `app/trace.py` nació en `c292210` (2026-08-17): **el
código desplegado era anterior a la traza**, así que practicar allí habría
funcionado y no habría escrito ni una línea. Se resolvió con `git pull` +
`systemctl restart teapp` — 40 commits, sin reinstalar, porque
`requirements.txt` y `.env.example` no habían cambiado desde el despliegue.

> 🔑 ***"El servidor está arriba"* era cierto, y aun así la prueba habría salido
> vacía.** La afirmación no había caducado: es que nunca dijo lo que parecía
> decir. Misma familia que `[L-062]`…`[L-066]`.

**La línea, entera y sin recortar:**

```json
{"at": "2026-08-19T18:16:26.397065+00:00", "broken": [], "model": "claude-opus-5", "model_seconds": 3.468, "outcome": "correct", "practice": 9, "queue_seconds": 0.0, "rest_seconds": 0.002, "score": 6, "seconds": 3.47, "user": "jorge", "words": 9}
```

✅ **Demuestra tres cosas, y conviene no mezclarlas:**

1. **`PI-8` y `[D-085]` se cumplen en PRODUCCIÓN, no en un test.** Doce campos y
   **ninguno contiene la frase**: dice `words: 9`, nunca las nueve palabras.
   Dice `outcome: "wrong"` o `"correct"`, nunca qué estaba mal. Esa promesa solo
   se podía creer mirando datos reales dentro.
2. **La fila no se contradice a sí misma.** `rest_seconds` no se recibe, se
   calcula, y las cuentas cuadran: `3,47 − 0,0 − 3,468 = 0,002`.
3. 💰 **Medición nueva, con corrida detrás: de los 3,47 s de la práctica, 3,468
   son el modelo. El código propio entero —cuota, marcador, escritura en disco—
   tarda 2 ms. El 0,06%.** Optimizar código nuestro no compraría nada: toda la
   espera es el modelo. Y deja holgado el freno de `[A-011]`: 3,5 s contra un
   techo de 10.

#### Por qué se renuncia: TEAPP es educativa, no comercial

TEAPP existe para que quien la construye aprenda cómo se hace una aplicación con
IA de punta a punta —navegador, identidad, servidor propio, frenos de
producción, despliegue en la nube, modelo real enchufado y observabilidad—, y
ese recorrido está hecho: los diez pasos del roadmap se caminaron.

No se vende, no tiene clientes, no tiene usuarios fuera de su autor. **Por tanto
no existe la presión que justifica el descenso de modelo:** ni factura que
optimizar a escala, ni márgenes, ni regresión en producción que vigilar.

Comparar tres modelos sobre una vara saturada enseñaría hoy menos de lo que
cuesta en tiempo y en dinero, y **el aprendizaje central ya está cobrado**:
`[L-088]` —una vara que dos humanos leen igual (30/30) puede no discriminar a un
modelo (0 de 30)— es la lección más valiosa del arco de evals y costó `$0,1026`.

#### ⛔ Lo que hay que dejar escrito sin ambigüedad

> 🚨 **Esta decisión es válida PORQUE el proyecto es educativo. En una aplicación
> COMERCIAL, lo que hoy se decide no hacer SÍ habría que hacerlo — y no como
> buena práctica recomendable, sino como parte del trabajo.**

En un producto de verdad sería obligatorio:

1. **Construir una vara que discrimine ANTES de cambiar el modelo.** Un eval
   saturado da 100 antes y 100 después: no es un freno, es un adorno. Era
   exactamente lo que `[D-049]` y `[D-101]` exigían, y la exigencia era correcta.
2. **Medir cada modelo candidato en las DOS dimensiones, por separado:** la
   FORMA (¿obedece el contrato de salida?) y el VEREDICTO (¿acierta?). Se rompen
   por caminos distintos y se arreglan en direcciones opuestas — una apunta a la
   rúbrica, la otra al modelo. Mezclarlas en una sola tasa esconde las dos.
3. **Distinguir los dos errores, no promediarlos.** En un producto que enseña,
   *corregir de más* molesta y *perdonar* enseña mal; caen dentro de la misma
   tasa de acierto y no valen lo mismo. Por eso `cross_check.py` saca una tabla
   2×2 y no un número: eso se conserva y está bien hecho.
4. **Medir el coste real de cada modelo antes de elegir**, no deducirlo de la
   lista de precios. `COST_PER_CALL_USD` de este repo está MEDIDO y ya caducó
   una vez (`[D-096]`).
5. **Tener una regresión que corra sola en cada cambio de modelo.** Aquí no la
   hay y no hará falta, porque el modelo no se vuelve a tocar.

> 🔑 **El riesgo concreto que este apartado conjura:** que alguien —incluido el
> propio autor, dentro de seis meses— lea *"el paso 9 se cerró sin comparar
> modelos"* y se lleve la conclusión de que comparar modelos es prescindible.
> **No lo es. Se saltó porque este proyecto no lo necesita, y la diferencia
> entre las dos cosas es el asunto entero.**

#### 🔴 Deuda que queda viva y DORMIDA: dos constantes de modelo, una sola guardada

Hallada al preparar esta decisión.

| dónde | constante | ¿la clava algún test? |
|---|---|---|
| `app/tools.py:41` | `MODEL_NAME` — **la de la app** | ❌ **no** |
| `deploy/check_api_key.py:104` | `MODEL` — la del portero | ✅ `tests/test_check_api_key.py:91` |

`tests/test_check_api_key.py:91` clava el par `(check_api_key.MODEL,
LAB_REQUESTS_PER_MINUTE)` — la **segunda**. Ningún test clava la primera:
`tests/test_tools.py:145`, `tests/test_trace.py:56` y
`tests/test_eval_rubric.py:516` la **derivan**, así que siguen verdes con
cualquier valor.

🚨 **Consecuencia: cambiar el modelo de la app no pone ningún test en rojo. Es un
cambio MUDO.** Y tocar `app/tools.py` sin tocar `check_api_key.py` dejaría al
portero validando contra el límite de Opus mientras la app corre otra cosa —
`T-088` reentrando por otra puerta.

⚠️ **Al renunciar a `[D-049]`, esta deuda queda DORMIDA, no resuelta.** Se
registra a propósito: es la trampa que se armaría sola el día que alguien tocara
el modelo. En un producto comercial se arregla antes de cerrar; aquí se deja
escrita y se deja quieta.

#### Lo que sigue fuera, igual que ya lo estaba por `[D-103]`

`T-103`, `T-108` y `T-081`. Deuda pequeña y real; no deciden el cierre.

- **Contra:**
  - **Reescribir la vara apuntando a *forma bajo carga***, según el diagnóstico
    de `[L-088]`. Descartado por el usuario: repetir el ejercicio compra una
    lección marginal más pequeña por el mismo dinero.
  - **Correr las 90 frases YA etiquetadas sobre Haiku 4.5 y Sonnet 5, sin
    escribir ni una frase nueva.** Propuesto en esta sesión con un argumento que
    sigue siendo válido y que se anota para que no se pierda: **la saturación de
    `[L-086]` se midió SOLO con Opus**, y *"Opus saca 90/90 luego Haiku sacará
    90/90"* es una suposición, no una medición. Techo de gasto derivado del
    número medido de Opus: `< $0,62` las dos corridas. Descartado por el usuario
    **por alcance**, no por el argumento: el proyecto es educativo y ya cumplió
    su fin.
  - **Cerrar el paso 9 también sin `T-102`.** Descartado por el usuario: cuesta
    `~$0,01` y es la mitad que da nombre al paso.
- **Toca:** `_context/roadmap.md`, `app/trace.py`, `app/tools.py:41`,
  `deploy/check_api_key.py:104`, `tests/test_check_api_key.py:91`, `~~D-049~~`,
  `[D-085]`, `[D-096]`, `[D-101]`, `[D-103]`, `[D-104]`, `[L-086]`, `[L-088]`,
  `[A-031]`, `T-081`, `T-088`, `T-102`, `T-103`, `T-108`, `T-112`, `PI-4`,
  `PI-8`, paso 9, y 🏁 **el cierre del proyecto**.
...
- **Se eligió:** el paso 9 queda cerrado cuando se cumplen **las dos**:
  1. **Existe una vara DISCRIMINANTE** —escrita, etiquetada y corrida contra el
     juez, en ese orden (`[D-101]`, `[L-083]`)—. No basta con que exista el archivo:
     tiene que haber pasado por el juez y haberse cruzado.
  2. **La traza se ha visto escribir con servidor levantado y modelo real**, no con
     `TestClient` y juez de mentira.

  Y se cierra **con una entrada de `decisions.md`**, como los ocho pasos anteriores.

- **Contra:**
  - **Cerrar por conteo de tareas.** Descartado: es lo que `[T-098]` prohibió al
    protocolo de arranque, y **una lista vacía no es una condición** — dice que
    nadie apuntó más, no que esté hecho.
  - **Cerrar solo con `T-112`.** Descartado: deja la observabilidad sin correr, y el
    paso se llamaría por su mitad ausente.
  - **Incluir las cuatro tareas abiertas.** Descartado: `T-103` y `T-108` son deuda
    pequeña y real, se hacen de paso y no deciden nada. Meterlas habría hecho el
    criterio más difícil de cumplir **sin hacerlo más honesto**.

- **Por qué:** 🚨 **el paso 9 no tenía criterio de cierre, y eso se descubrió
  buscándolo, no recordándolo.** `roadmap.md:23` le dedica una línea;
  `_context/scope.md` no lo menciona. Los ocho anteriores se cerraron con una
  entrada de `decisions.md` —el 8 con `[D-081]`—, que es lo que el arranque usa para
  declarar un paso cerrado.

  🔑 **Sin criterio escrito un paso no se cierra: se abandona.** Y se abandona por
  cansancio, justo cuando la lista de tareas pequeñas parece vacía — que no es lo
  mismo que estar terminado.

  ⚠️ **Aquí el riesgo tenía cara concreta.** Las dos mitades del paso están muy
  desiguales: los *evals* casi enteros (`T-104`…`T-111`, y hoy `T-109`), y la
  **observabilidad —la que da nombre al paso— nunca vista funcionar**.
  `app/trace.py` tiene tests, pero `T-102` dice que solo se ha corrido con
  `TestClient` y el juez de mentira. Es `PI-4` sin cumplir en la mitad que titula el
  paso, y era **exactamente** lo que un cierre por cansancio se habría llevado por
  delante sin que nadie lo notara.

- 📌 **Las dos condiciones se pagan con el MISMO gasto, y por eso son dos y no
...
- 🧭 **El orden de `T-112` no se relaja por esto:** escribir → etiquetar → correr,
  ...
  que cierre esa puerta (`[L-083]`).

- **Toca:** `_context/roadmap.md`, `_persistence/tasks.md`, `[D-081]`, `[D-101]`,
  `[D-049]`, `[C-009]`, `[L-083]`, `T-102`, `T-103`, `T-108`, `T-112`, `PI-4`,
  paso 9.
...
- **Se eligió:** **(1)** el próximo conjunto de frases se declara **hoy**, antes de
  escribirlo, y será **discriminante**; **(2)** `[D-049]` —bajar `MODEL` a Sonnet
  para abaratar— gana una condición que antes no tenía: **no se toca `MODEL` hasta
  que exista una vara capaz de bajar.**
- **🔑 Por qué la declaración va antes de escribir las frases.** *"Frases difíciles a
  propósito"* significa dos cosas distintas, y sus números no se parecen:
  - **Discriminante:** escrito para encontrar **dónde se rompe** el corrector.
    Contesta *"¿dónde está el borde?"*.
  - **Representativo:** escrito para parecerse a lo que de verdad escribe alguien
    practicando. Contesta *"¿qué tan bien le sirve esto a quien lo usa?"*.

  Los dos son legítimos. Pero si las frases se escriben sin declarar cuál es, **el
  número que salga se reinterpreta después, con el resultado delante**, y sale a
  favor de quien mide. Es `[D-040]` aplicado al siguiente **artefacto** en vez de a
  la siguiente lectura.
- **Por qué discriminante:** la pregunta viva hoy es `[D-049]`, el descenso de
  modelo. No es la experiencia de quien usa la app — ésa tendrá su vara cuando sea
  la pregunta.
- **🔴 Por qué la condición sobre `MODEL` es más dura de lo que parecía.** Se había
  escrito que con el eval saturado *"la caída no se verá hasta que sea enorme"*. Es
  peor: un eval en el techo **da 100 antes del cambio y 100 después**, así que **no
  puede funcionar como freno de regresión** — ni para `[D-049]` ni para ningún otro
  cambio. Para esa pregunta concreta no mide poco: **no mide.**
- **Encaja con lo que ya estaba armado:** `[D-081]` puso un freno delante de `MODEL`
  por prudencia. Ahora ese freno tiene **una razón medida detrás**, que es distinto
  y mejor.
- **📌 La causa del techo, localizada y no supuesta:** `eval_rubric.py:129` importa
  `SENTENCES` de `measure_tutor.py`. Esas sesenta frases **se escribieron para medir
  al tutor**, en otro trabajo, y se tomaron prestadas para examinar al juez de
  gramática. 🔑 Eso convierte el hallazgo de `[L-086]` en algo más fuerte que una
  observación sobre esta corrida: **el techo estaba puesto desde el día en que se
  reutilizó el archivo**, y hoy simplemente se vio.
- **⚠️ El orden del protocolo se conserva, porque hoy funcionó:** **escribir →
  etiquetar → recién entonces correr el juez.** Nunca al revés. Etiquetar con el
  veredicto delante no es etiquetar.
- **🚨 Y `[L-083]` sigue viva, sin portero que la cierre.** Quien escriba las frases
  difíciles será quien las etiquete, y ahora mismo tiene el veredicto del juez
  fresco en la cabeza. Un portero de archivo no alcanza a eso: es la puerta de la
  conversación, la misma por la que entró `[L-085]`.
- **Contra:**
  - **Conjunto representativo.** Descartado hoy, no por malo: contesta otra
    pregunta, y la viva es el descenso de modelo.
  - **Escribir las frases ya, a última hora de la sesión.** Descartado: sin el tipo
    declarado, el número nace reinterpretable — y cansado se declara peor.
