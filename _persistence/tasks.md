...
| T-078 | Que `ANTHROPIC_API_KEY` llegue al servidor: `install.sh` tiene que colocarla en el archivo de entorno de la máquina, con permisos cerrados, sin escribirla nunca en el repo. ⚠️ **Enlaza con `[A-023]`:** es exactamente la pieza que hace que el `deploy/` de septiembre no sea el de hoy, y por tanto condiciona el ensayo de `T-069`. 🔻 **Ya NO bloqueada por `T-084` — cerrada el 2026-08-12 (`[D-061]`).** 🚨 **Bloqueada ahora por `T-085`:** ponerle llave de SERVIR al servidor le da al saldo compartido un segundo consumidor sin freno propio, y esa decisión todavía no está tomada. ⚠️ Y de camino conviene mirar `[L-042]`: el 504 sigue decidiendo la devolución con `future.cancel()`, que no responde "¿se facturó?" | 🔲 | 8 |
...
| T-085 | 💰🆕 **Decidir si el espacio `teapp-measure` lleva además tope de GASTO, medido contra el saldo real de $6,55 — no contra el tope mensual de $500 de la organización.** Queda abierto por `[D-061]`. Va **antes** de `T-078`: hoy solo gasta la báscula, que tiene `CallBudget` (`[D-060]`); el lado que sirve no tiene freno propio más allá de la cuota de 20/día por persona. `T-078` es lo que le da al saldo un segundo consumidor — la condición que desbloqueó `T-078` ("la capa 1 existe y mordió") protege al laboratorio, no al lado que sirve | 🔲 | 8 |
