...
| T-078 | Que `ANTHROPIC_API_KEY` llegue al servidor: `install.sh` tiene que colocarla en el archivo de entorno de la máquina, con permisos cerrados, sin escribirla nunca en el repo. ⚠️ **Enlaza con `[A-023]`:** es exactamente la pieza que hace que el `deploy/` de septiembre no sea el de hoy, y por tanto condiciona el ensayo de `T-069`. 🔻 **Ya NO bloqueada por `T-085` — cerrada el 2026-08-12 (`[D-062]`).** 🔄 **Implementada el 2026-08-12, SIGUE ABIERTA A PROPÓSITO (`[D-063]`):** `deploy/check_api_key.py` y los cambios de `deploy/install.sh` están escritos y probados (15 tests, `tests/test_check_api_key.py`), pero `ask_anthropic` no se ha ejecutado nunca contra la red real. Condición de cierre escrita en `[D-063]`: ver morder la puerta 3 (con la llave del laboratorio) y la puerta 0 (con la de `Default`) — con una sola llave el 3 no distingue "la identificó" de "acertó por casualidad". ⚠️ Y de camino conviene mirar `[L-042]`: el 504 sigue decidiendo la devolución con `future.cancel()`, que no responde "¿se facturó?" | 🔄 | 8 |
...
- **Estado:** 🔄 a medias
- **Dónde quedó:** el código está escrito y probado (`deploy/check_api_key.py`,
  cambios en `deploy/install.sh`, 15 tests en `tests/test_check_api_key.py`,
  ninguno toca la red). `ask_anthropic` **no se ha ejecutado nunca contra la
  red real** — falta correr el guion de verdad, no falta escribir nada.
- **Condición de cierre, escrita en `[D-063]` para no confiarla a la
  memoria:** ver morder DOS puertas con DOS llaves reales — la puerta 3 con
  la llave del laboratorio (tiene que reconocerla por `requests-limit: 50` y
  abortar) y la puerta 0 con la llave de `Default` (tiene que pasar, antes de
  que `install.sh` la use). Con una sola llave la puerta 3 no distingue "la
  identificó" de "acertó por casualidad" — misma forma que `T-060b`.
- **No cerrar esta tarea sin ambas corridas reales**, aunque los 15 tests
  estén en verde: prueban la lógica contra una Anthropic de mentira.
