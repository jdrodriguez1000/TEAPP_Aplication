...
| T-105 | 🆕 **Cablear el corrector de rúbrica (`app/rubric_check.py`) a producción.** Escrito y NO cableado a propósito (`[D-089]`): que la ruta llame al corrector y la traza apunte el fallo de formato es un cambio en `app/api.py`, decidido aparte. Hoy `split_verdict` (`tools.py:601`) hace lo correcto cuando el modelo rompe el formato pero no se lo cuenta a nadie — la traza guarda `correct=False` tanto si el juez rompió el formato como si el alumno se equivocó. **NO bloqueada por `T-107`:** no toca rúbrica ni corpus, es el camino para avanzar sin gastar | 🔲 | 9 |
| T-106 | 🔴 **BLOQUEADA por `T-107`.** Etiquetar a mano las 60 frases de la línea base (o leerlas) para medir si el veredicto del juez ACIERTA, si corrigió uno o tres errores, o si se fue del tema — las tres promesas de `GRAMMAR_RUBRIC` que un programa no puede comprobar solo. Etiquetar cruza sesiones, y hoy el corpus lo puede borrar cualquier corrida (`[L-076]`): sin `T-107` resuelta, lo que se etiquete hoy puede desaparecer mañana | 🔲 | 9 |
| T-107 | 🆕 **El corpus de respuestas del eval no sobrevive a la siguiente corrida.** `replies_file()` (`eval_rubric.py:188`) devuelve un nombre fijo `data/eval_replies.jsonl`, y `save_replies` abre en `open("w")` (línea 243): una tanda de 10 borra una línea base de 60 — ya ocurrió una vez (`[L-076]`). 🚨 **NO se arregla cambiando a `"a"`:** el motivo escrito para sobrescribir es bueno (no mezclar dos modelos o dos rúbricas en un archivo, `[L-071]`) y se conserva; lo que falla es que el **nombre** no distingue modelo ni fecha, que es justo lo que `[D-049]` va a mover tres veces. De paso permite tener las tres líneas base (Opus/Sonnet/Haiku) a la vez. 🔻 **Se hace ANTES de la corrida de 60** — al revés se paga por un corpus que la siguiente tanda puede volver a borrar | 🔲 | 9 |
