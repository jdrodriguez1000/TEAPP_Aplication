...
| T-112 | 🔴 **CORRIÓ el 2026-08-19 y SALIÓ FALLIDA — tramo `0-2` de `[D-104]`. Ya no está bloqueada por `T-109` (cerrada en `799cc00`) ni le falta firma o prioridad.** 30 frases discriminantes nuevas (`measure_tutor.SENTENCES[61..90]`), etiquetadas a mano por el humano (15 `correct`/15 `wrong`/0 `unclear`), 30 llamadas reales a `claude-opus-5` (`$0,1026`, `eval_replies_claude-opus-5_2026-08-19_run-df89768f_pick.jsonl`). Cruce con las 60 representativas excluidas: **0 desacuerdos de 30** — la vara no discriminó al juez, aunque sí discriminó entre dos lectores humanos (30/30). **Se REABRE con el diagnóstico de `[L-088]`:** la próxima vara apunta a la FORMA BAJO CARGA (frases largas, varios errores a la vez, ambigüedad de qué corregir primero), no a la rareza gramatical de superficie — ese eje sí movió algo (2/30 rompieron `too_many_sentences` contra 0/60), pero quedó marcado como pista sin sellar, no como resultado. Ver `[D-104]`, `[L-088]` | 🔲 | 9 |
