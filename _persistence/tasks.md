...
| T-104 | 🆕 **ABIERTA, esperando FIRMA del usuario — preguntado el 2026-08-17, sin contestar, nada tocado.** Dos decisiones de `[D-089]`: (1) ¿sube el tope a TRES frases en `GRAMMAR_RUBRIC` y en `rubric_check.MAX_SENTENCES`? — el propio texto de la rúbrica teme una *lista de errores*, no la longitud, y esa promesa ya la cubre *"never correct more than one thing at a time"`; (2) ¿se afina la comprobación de comillas a solo las que envuelven una corrección, o se endurece la rúbrica para prohibirlas todas? 🚨 Cambiar la rúbrica CADUCA `COST_PER_CALL_USD` (`$0,00304`), así que no se toca antes de decidir | 🔲 | 9 |
| T-105 | 🆕 **Cablear el corrector de rúbrica (`app/rubric_check.py`) a producción.** Escrito y NO cableado a propósito (`[D-089]`): que la ruta llame al corrector y la traza apunte el fallo de formato es un cambio en `app/api.py`, decidido aparte. Hoy `split_verdict` (`tools.py:601`) hace lo correcto cuando el modelo rompe el formato pero no se lo cuenta a nadie — la traza guarda `correct=False` tanto si el juez rompió el formato como si el alumno se equivocó | 🔲 | 9 |
| T-106 | 🆕 **Etiquetar a mano las 60 frases de la línea base de `[D-089]`** (o leerlas) para medir si el veredicto del juez ACIERTA, si corrigió uno o tres errores, o si se fue del tema — las tres promesas de `GRAMMAR_RUBRIC` que un programa no puede comprobar solo. Es la otra mitad del paso 9 por evals, sin empezar | 🔲 | 9 |
