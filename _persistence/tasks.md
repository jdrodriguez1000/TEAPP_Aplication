...
| T-111 | 🆕 **Cruzar las 60 etiquetas humanas (`_persistence/labels/sentence_labels.jsonl`) contra el veredicto real del juez sobre esas mismas frases, usando `app.tools.split_verdict`.** Es el paso que mide si el corrector ACIERTA — hoy solo existe la verdad de referencia (`T-106`), falta correrla contra el veredicto y comparar. No se corrió todavía; nace de este cierre, sin firma ni prioridad decidida | 🔲 | 9 |
