...
| T-093 | 🆕 **Cerrar `[A-011]` de verdad.** ⚠️ **Actualizada el 2026-08-13, segunda ronda:** el ítem (3) de más abajo YA NO aplica — `[D-072]` subió `read` a 6,5, que queda 38% POR ENCIMA del peor de diez medido, no por debajo. Lo que queda pendiente: (1) el reparto por fases sigue sin ser techo duro — `httpcore` aplica `read` por lectura de socket, no al cuerpo entero; (2) nadie ha cronometrado el peor caso real de Opus 5 contra un **percentil** — el `6,5` se coloca contra el máximo de solo diez muestras, que no es la cola de la distribución (falta una tanda de 30-40 frases, ~$0,09 con `[D-058]`). Sin estas dos, el `10,0` de la ruta sigue siendo la única garantía de reloj de pared, y así se queda | 🔲 | 8 |
