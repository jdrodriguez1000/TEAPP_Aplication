...
| T-079 | Medir de verdad los dos frenos que hoy son predicción, con el modelo real y facturas encima: `[A-010]` (20 prácticas/día por persona) y `[A-011]` (10 s de timeout al tutor). 🟡 **CERRADA CON CONDICIÓN VIVA el 2026-08-15 — las dos mitades están hechas: `[A-010]` con `[D-058]` desde el 2026-08-11, `[A-011]` con `[D-077]` vía `T-093`.** 🔴 **Llevaba ✅ y baja a 🟡:** la medición es real (60 llamadas, cero cortes, criterio fijado antes de mirar), pero **un ✅ afirma que no queda nada que vigilar** y `[D-077]` deja viva una condición que **no depende de nosotros** —*"vale mientras Anthropic responda como el 2026-08-14"*— y que **no se cierra midiendo**: otra tanda daría otro verde igual de condicionado por otros $0,18. **Disparador: si vuelven los cortes al practicar, se repite la tanda de 60 ANTES de tocar `TUTOR_TIMEOUT_SECONDS`.** ⚠️ De paso, `[D-077]` corrige una frase escrita el 15 que decía que las cuatro fases son "relojes en paralelo" y que el 9,0 cabe "por construcción": son **secuenciales**, el 9,0 es una **suma nuestra** que el SDK no impone, y la única garantía de reloj de pared es el `10,0` de `app/api.py:730` (`app/tools.py:239`). Ver entrada | 🟡 | 8 |
...
| T-088 | ✅ **CERRADA el 2026-08-15, y adelantada a propósito: NO era un remate del paso 9, estaba ARMADA PARA el paso 9.** Corregir el comentario de `MODEL` en `deploy/check_api_key.py`. Decía *"da igual cuál sea el modelo"* y es falso: `anthropic-ratelimit-requests-limit` es **por modelo**, así que la firma del laboratorio no es el 50, es **el par (espacio, modelo)** — cambiar `MODEL` sin tocar `LAB_REQUESTS_PER_MINUTE` hace que la comparación salga falsa, y el guion devuelve `EXIT_OK` **aceptando la llave del laboratorio, sin dar error** (`[L-050]`, tercera pata de `[L-047]`). 🔑 **Por qué no podía esperar:** el paso 9 es *bajar a Haiku*, o sea que el disparador de esta trampa era **la primera acción del paso siguiente**, y el comentario falso estaba puesto justo delante de quien iba a cambiar `MODEL`, diciéndole que daba igual. Aplazarla habría sido dejar un disparador sin dueño — ver `[L-064]`. ✅ Hecho: aviso completo en `MODEL` con el disparador escrito (leer en la consola el límite del modelo nuevo y ponerlo arriba **en el mismo cambio**), y puntero desde `LAB_REQUESTS_PER_MINUTE` para que el par se vea desde los dos extremos. Solo comentarios, sin tocar lógica; suite en verde | ✅ | 8 |
...
| T-096 | 🆕 **Prioridad BAJA. Localizar ~5 llamadas sin dueño en `teapp-measure`, con el número dentro: `1.834` tokens de entrada + `338` de salida ≈ `$0,018`.** Es lo que la semana 10–16/08 trae de más sobre el día 14 (`23.502 \| 3.297` menos `21.668 \| 2.959`). 🚨 **No cuadra con las sondas que teníamos anotadas:** `check_api_key.py` y la llamada mínima de `[D-061]` suman decenas de tokens, no 1.834 — algo más llamó con la llave del laboratorio, **en un día que no es el 14**. ⚠️ No bloquea nada (son centavos y no tocan el día de la corrida), pero es el tipo de cabo que aquí se cobra tres sesiones después. 🔍 **Se resuelve barato y gratis:** la misma vista de USO, *"Ver por: Día"*, recorriendo del 10 al 16 y viendo cuál trae el excedente | 🔲 | 8 |
| T-098 | 💣 **ARMADA, no aplazada — su disparador es el PRÓXIMO ARRANQUE. El guion de inicio resume la PROSA en vez de leer el CAMPO DE ESTADO, y no se salta lo tachado.** 🧪 **Daño ya medido, no supuesto:** `T-090` se ofreció **tres veces** como trabajo por hacer estando hecha (dos por prosa caducada, una por la columna `🔲` que no se actualizó), y `[A-024]` viajó como *"sin comprobar"* estando **RETIRADA desde el 2026-08-11** — `assumptions.md` la tiene tachada y el guion la leyó como viva. 🔑 **Y el problema no se acaba con esas dos filas:** `~~A-010~~`, `~~A-011~~` y `~~A-014~~` están tachadas y esperan el mismo turno. 🔧 **Qué tiene que cambiar:** el arranque lee la **columna de estado** de `tasks.md` para saber qué falta —no el párrafo—, y **omite toda entrada tachada** (`~~A-nnn~~`) de `assumptions.md` en vez de resumirla. 📌 Nace de `[L-062]` (el estado se pudre por detrás de las entradas) y de `[L-063]` (citar el sitio, no recordarlo); es `[L-064]` aplicándose a su primer caso. Puede tocar la skill `protocol-start` y el agente `session-starter` | 🔲 | 8 |
