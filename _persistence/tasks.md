...
| T-072 | 🚨 **Nueva.** Existe un camino que escribe en `data/` real **sin pasar por `conftest.py`** — evidencia en `[A-020]`: el 2026-08-06 a las 14:48:33 aparecieron `data/users/otronombrelargo.json` y `data/quota/otronombrelargo.json` con el mismo nanosegundo, de una cuenta que no existe en `data/accounts.json`. No fue pytest — el portero de `T-071` vive **dentro** de la suite y no ve nada que corra fuera de ella. Investigar qué script o proceso lo escribió y cerrarlo. NO se mezcla con `T-071` | 🔲 | 7 |
