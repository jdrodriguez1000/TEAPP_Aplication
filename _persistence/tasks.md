...
| T-102 | ✅ **CERRADA el 2026-08-19, 18:16 UTC — la traza se vio escribir en producción.** EC2 real, entrando por `https://teapp.duckdns.org/` desde el navegador, llamada real a `claude-opus-5`. Línea completa citada en `[D-105]`; doce campos, ninguno con la frase (`PI-8`, `[D-085]` en producción). Costó ~`$0,0034`, una práctica. Condición 2 de `[D-103]` cumplida. Cierra el paso 9 junto con la condición 1 (`T-112`) | ✅ | 9 |
...
| T-108 | 🎯 **CRUZADA Y MEDIDA el 2026-08-19, sin querer y de paso — no solo teórica.** `replies.py:83`, `tests/test_eval_rubric.py:512` y `tests/test_replies.py:25` recorren `*.jsonl` con `glob`: un corpus guardado como `.json` o `.txt` entra sin que nadie lo mire. La rendija de `_persistence/labels/` la cruzó **la propia sesión ejecutora al primer intento**, al sellar el reparto de brazos de `T-112` como `_persistence/seals/hard_arms_sealed.json` — un `.json`, no un `.jsonl`, y ningún `glob *.jsonl` lo miró nunca (por eso hizo falta un portero aparte, `tests/test_seals.py`). ⚠️ **Alcance ampliado el 2026-08-19 antes de esto:** la redacción anterior nombraba solo `corpus/` — la carpeta que guarda la evidencia del 58/58 quedaba fuera de la tarea que la iba a proteger; el tercer sitio (`replies/`) lo encontró la sesión ejecutora. Sigue sin arreglarse: se tapa recorriendo `*` y exigiendo que todo lo que no sea `README.md` acabe en `.jsonl`. No bloquea nada | 🔲 | 9 |
| T-109 | 🔼 **PRIORIDAD. El nombre del archivo no registra QUÉ EXAMEN se tomó — y `T-112` cambia justo eso.** Los cuatro ejes sellan modelo, fecha, `GRAMMAR_RUBRIC` y `full`/`pick`; **no sellan el conjunto de frases ni `rubric_check.py`**. Corriendo las frases discriminantes de `T-112`: mismo modelo, misma huella de rúbrica, mismo `full` → **nombre idéntico al de la corrida que sostiene el 58/58**, y `save_replies` abre en `"w"`. Dos mitades independientes: el sello `run-` en el nombre (`[D-102]`) y `open("x")` en `save_replies`. **Absorbe `T-110`.** ⚠️ **La justificación anterior de esta tarea era FALSA y está enmendada** — ver `[T-109]` ✅ **CERRADA el 2026-08-19 en sus DOS mitades** (commit `799cc00`): `save_replies` abre en `"x"` y `replies_file()` compone `run-<sello>` con las tres huellas. Seis sabotajes vistos morder; uno de ellos no lo cazó la suite y nació `[L-087]`. Suite `575 → 577` | ✅ | 9 |
| T-112 | ✅ **CERRADA el 2026-08-19 — NO se reabre.** Corrió y salió FALLIDA — tramo `0-2` de `[D-104]`. 30 frases discriminantes nuevas (`measure_tutor.SENTENCES[61..90]`), etiquetadas a mano por el humano (15 `correct`/15 `wrong`/0 `unclear`), 30 llamadas reales a `claude-opus-5` (`$0,1026`, `eval_replies_claude-opus-5_2026-08-19_run-df89768f_pick.jsonl`). Cruce con las 60 representativas excluidas: **0 desacuerdos de 30** — la vara no discriminó al juez, aunque sí discriminó entre dos lectores humanos (30/30). 🔑 **`[D-105]` da la condición 1 de `[D-103]` por CUMPLIDA con este resultado:** la vara pedía escribirse, etiquetarse y correrse — las tres pasaron; no pedía que mordiera. El diagnóstico de `[L-088]` (apunta a FORMA BAJO CARGA, no a rareza gramatical) queda escrito para quien retome evals en el futuro, pero no se ejecuta: el proyecto renunció al descenso de modelo (`~~D-049~~`) y no hay próxima vara que escribir. Ver `[D-104]`, `[D-105]`, `[L-088]` | ✅ | 9 |
| T-113 | 🆕 **DORMIDA. Dos constantes de modelo y solo una guardada por un test.** `app/tools.py:41` (`MODEL_NAME`, la de la app) no la clava ningún test —`tests/test_tools.py:145`, `tests/test_trace.py:56` y `tests/test_eval_rubric.py:516` la DERIVAN y siguen verdes con cualquier valor—; `deploy/check_api_key.py:104` (`MODEL`, la del portero) sí la clava `tests/test_check_api_key.py:91`. Cambiar el modelo de la app es hoy un cambio MUDO: ningún test se pone en rojo. Hallada al preparar `[D-105]`, queda DORMIDA (no resuelta) al renunciar a `[D-049]` — se reabre el día que alguien vuelva a tocar el modelo | 🔲 | 9 |
...
- **Estado:** ✅ **hecha del todo** el 2026-08-19, commit `799cc00`. Desbloquea `T-112`.
