...
| T-078 | Que `ANTHROPIC_API_KEY` llegue al servidor: `install.sh` tiene que colocarla en el archivo de entorno de la máquina, con permisos cerrados, sin escribirla nunca en el repo. ⚠️ **Enlaza con `[A-023]`:** es exactamente la pieza que hace que el `deploy/` de septiembre no sea el de hoy, y por tanto condiciona el ensayo de `T-069`. 🔻 **Ya NO bloqueada por `T-082` — cerrada el 2026-08-11.** 🚨 **Bloqueada ahora por `T-084`:** `[D-059]` asignó la llave nueva a MEDIR y la de hoy (`T-075`) a SERVIR, y es la de hoy la que viaja al servidor en esta tarea — pero la llave nueva nace en `T-084`. Si esta tarea corre antes, la única llave que hay viajaría al servidor y medir/servir compartirían también la llave, perdiendo la revocación por separado y la contabilidad por `workspace_id` que la capa 2 compra. ⚠️ Y de camino conviene mirar `[L-042]`: el 504 sigue decidiendo la devolución con `future.cancel()`, que no responde "¿se facturó?" | 🔲 | 8 |
| T-079 | Medir de verdad los dos frenos que hoy son predicción, con el modelo real y facturas encima: `[A-010]` (20 prácticas/día por persona) y `[A-011]` (10 s de timeout al tutor). 🔄 **A MEDIAS desde el 2026-08-11:** `measure_tutor.py` corrido, diez llamadas reales a `judge_grammar`. La mitad de `[A-010]` quedó CERRADA (ascendida a `[D-058]`: $0,00234/práctica, $8,44 en 180 días por persona a tope, no cubierto por el saldo de $6,55). La mitad de `[A-011]` NO: el guion cronometró `judge_grammar` suelto, no `/practice` entera con el pool cargado — `[A-011]` quedó REABIERTA como encogida. **Lo que falta:** cronometrar `/practice` entera con concurrencia (aparejo de `[L-013]`, 23 peticiones a la vez) | 🔄 | 8 |
...
| T-084 | 🆕 **Acción del usuario en el navegador: crear el espacio de trabajo para MEDIR, con su llave propia y su límite de VELOCIDAD (no de gasto).** Es la capa 2 de `[D-059]`. 🚨 **Bloquea `T-078`**, no por el saldo sino por el reparto de llaves: `[D-059]` asignó la llave nueva a medir y la de hoy (`T-075`) a servir, pero la llave nueva nace aquí. Si `T-078` corre antes, al servidor viaja la única llave que hay —la misma que usa `measure_tutor.py`— y medir y servir pasan a compartir también la llave, perdiendo lo único que la capa 2 compraba: poder revocarla sola y la contabilidad separada por `workspace_id` | 🔲 | 8 |
