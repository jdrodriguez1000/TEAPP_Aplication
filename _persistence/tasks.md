...
| T-078 | Que `ANTHROPIC_API_KEY` llegue al servidor: `install.sh` tiene que colocarla en el archivo de entorno de la máquina, con permisos cerrados, sin escribirla nunca en el repo. ⚠️ **Enlaza con `[A-023]`:** es exactamente la pieza que hace que el `deploy/` de septiembre no sea el de hoy, y por tanto condiciona el ensayo de `T-069`. 🚨 **Bloqueada por `T-082`:** en cuanto la llave llegue al servidor, `[C-008]` deja de ser teórica — decidir la partición entre medir y servir va ANTES. ⚠️ Y de camino conviene mirar `[L-042]`: el 504 sigue decidiendo la devolución con `future.cancel()`, que no responde "¿se facturó?" | 🔲 | 8 |
...
| T-082 | 💰🚨 **Decidir cómo se separan MEDIR y SERVIR, antes de `T-078`.** Hoy comparten el mismo saldo de Anthropic y es inofensivo porque el servidor tiene la llave vacía (`[C-008]`); en cuanto `T-078` la ponga en el servidor, una tanda de medición como `T-079` puede dejar la app viva dando 503 — y MUDA, porque el camino que devuelve cuota no toca el marcador. Sin comprobar: si la consola de Anthropic admite espacios de trabajo con saldo o tope propio (partición de verdad); si no, tocaría partir por orden (medir siempre antes de tocar producción) o por guion (corte duro de llamadas). Señalada por auditoría externa el 2026-08-11, ver `[C-008]` | 🔲 | 8 |
