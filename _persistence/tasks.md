...
| T-054 | Tope de tamaño de cuerpo en el servidor de delante (el reverse proxy de la nube). `MAX_SENTENCE_LENGTH` frena el gasto, no la subida: un cuerpo de 5 MB se sube entero antes del 422 (`C-002`). ✅ Hecha el 2026-08-06 en su mitad medible: `deploy/Caddyfile.template` ya tenía `max_size 16KB` desde `T-063`, y la báscula que faltaba quedó hecha — 500 caracteres en cinco alfabetos, peor caso legítimo 6016 bytes, contra un techo real de 16000 (`[D-035]`, `tests/test_deploy_limits.py`). ⚠️ **Falta que Caddy devuelva el 413 de verdad** — necesita el binario, llega con `T-061`. La deuda del entero real de `16KB` (vía `caddy adapt`, hoy leído en documentación) queda en `[A-019]` | ✅ | 7 |
...
| T-071 | El aislamiento de datos vive en un `fixture` local, no en `conftest.py`. `conftest.py` desvía cuentas (`ACCOUNTS_FILE`), cuota (`QUOTA_DIR`) y el contador de `login_guard`, **pero no el marcador** (`USERS_DIR`). Ese aislamiento está duplicado en dos `fixture` locales: `test_api.py` y `test_deploy_limits.py`. 🚨 **La trampa sigue armada:** el próximo archivo de tests que llame a `/practice` escribirá en `data/` de verdad, y nadie lo notará — `git status` no mira esa carpeta (`[L-020]`). Forma de catálogo: *si el aislamiento de datos vive en un fixture local en vez de en `conftest.py`, el siguiente archivo de tests escribe en producción y nadie se entera.* | 🔲 | 7 |
